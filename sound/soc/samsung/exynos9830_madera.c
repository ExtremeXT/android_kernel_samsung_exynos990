/*
 *  Driver for Madera CODECs on Exynos9830
 *
...
static const struct snd_soc_dapm_widget exynos9830_supply_widgets[] = {
...
static int exynos9830_probe(struct snd_soc_card *card)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(exynos9830_supply_widgets); i++) {
		const struct snd_soc_dapm_widget *w;
		struct regulator *r;

		w = &exynos9830_supply_widgets[i];
		switch (w->id) {
...
static struct snd_soc_dai_link exynos9830_dai[100] = {
	{
		.name = "RDMA0",
		.stream_name = "RDMA0",
...
static const struct snd_kcontrol_new exynos9830_controls[] = {
...
static const struct snd_soc_dapm_widget exynos9830_widgets[] = {
...
static const struct snd_soc_dapm_route exynos9830_routes[] = {
...
static struct snd_soc_card exynos9830_madera = {
	.name = "Exynos9830-Madera",
	.owner = THIS_MODULE,
	.dai_link = exynos9830_dai,
	.num_links = ARRAY_SIZE(exynos9830_dai),

	.probe = exynos9830_probe,
	.late_probe = exynos9830_late_probe,

	.controls = exynos9830_controls,
	.num_controls = ARRAY_SIZE(exynos9830_controls),
	.dapm_widgets = exynos9830_widgets,
	.num_dapm_widgets = ARRAY_SIZE(exynos9830_widgets),
	.dapm_routes = exynos9830_routes,
	.num_dapm_routes = ARRAY_SIZE(exynos9830_routes),
...
static void exynos9830_register_card_work_func(struct work_struct *work)
{
	struct snd_soc_card *card = &exynos9830_madera;
	int ret;
...
DECLARE_WORK(exynos9830_register_card_work, exynos9830_register_card_work_func);

static int exynos9830_audio_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &exynos9830_madera;
	struct madera_drvdata *drvdata = card->drvdata;
	...
	schedule_work(&exynos9830_register_card_work);
...
static const struct of_device_id exynos9830_audio_of_match[] = {
	{ .compatible = "samsung,exynos9830-madera", },
	{},
};
MODULE_DEVICE_TABLE(of, exynos9830_audio_of_match);
#endif /* CONFIG_OF */

static struct platform_driver exynos9830_audio_driver = {
	.driver		= {
		.name	= "exynos9830-audio",
		.owner	= THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = of_match_ptr(exynos9830_audio_of_match),
	},

	.probe		= exynos9830_audio_probe,
	.remove		= exynos9830_audio_remove,
};

module_platform_driver(exynos9830_audio_driver);

MODULE_DESCRIPTION("ALSA SoC Exynos9830 Audio Driver");
MODULE_AUTHOR("Charles Keepax <ckeepax@opensource.wolfsonmicro.com>");
MODULE_AUTHOR("Gyeongtaek Lee <gt82.lee@samsung.com>");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos9830-madera");
