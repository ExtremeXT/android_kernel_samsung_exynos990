...
static struct vts_platform_data *vts_get_drvdata(struct snd_pcm_substream *substream)
{
       struct snd_soc_pcm_runtime *rtd = substream->private_data;
       struct snd_soc_dai *dai = rtd->cpu_dai;
       struct snd_soc_dai_driver *dai_drv = dai->driver;
       struct vts_data *data = dev_get_drvdata(dai->dev);
       struct vts_platform_data *platform_data = platform_get_drvdata(data->pdev_vtsdma[dai_drv->id]);
...
static int vts_platform_new(struct snd_soc_pcm_runtime *runtime)
{
	...
	struct vts_platform_data *data = platform_get_drvdata(vtsdata->pdev_vtsdma[dai_drv->id]);
