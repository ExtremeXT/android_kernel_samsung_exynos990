...
#INSTALLED_DPM_DTB_TARGET := $(PRODUCT_OUT)/dpm.img
...
ifeq ($(TARGET_DTB_NAME),)
KERNEL_DTB := $(KERNEL_DTB_DIR)/$(TARGET_SOC).dtb
else
KERNEL_DTB := $(KERNEL_DTB_DIR)/$(TARGET_DTB_NAME).dtb
endif
ifeq ($(TARGET_DTBO_CFG),)
...
#MAKE_CONFIG_CMD := $(MAKE) -C $(TARGET_KERNEL_SOURCE) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(KERNEL_DEFCONFIG) CC=$(CC)
...
#MAKE_CONFIG_CMD += $(MAKE) -C $(TARGET_KERNEL_SOURCE) ARCH=$(KERNEL_ARCH) KCONFIG_ALLCONFIG=.config alldefconfig CC=$(CC)
...
#	$(MAKE) -C $(TARGET_KERNEL_SOURCE) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) CLANG_TRIPLE=$(CLANG_TRIPLE) CC=$(CC) -j$(N_KERNEL_BUILD_THREAD)
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) CLANG_TRIPLE=$(CLANG_TRIPLE) -j$(N_KERNEL_BUILD_THREAD)
...
#	cp $(DPM_DTB) $(INSTALLED_DPM_DTB_TARGET)
