...
__attribute__((unused)) static int load_fw_utc_vector(struct npu_session *sess, struct vs4l_param *param)
{
	...
	if (vector_len >= test_buf->size) {
		npu_err("Vector file[%s] is too large(%zuB). Maximum DRAM buf is %zuB\n",
			vector_path, vector_len, test_buf->size);
		ret = -EINVAL;
		goto err_exit;
	}
	...
	npu_info("Loading FW test vector[%s] to vaddr[%pK], len = %zuB\n",
		vector_path, test_buf->vaddr, vector_len);
	if (!test_buf->vaddr) {
		npu_err("DRAM buffer is not initialized.\n");
		ret = -EFAULT;
		goto err_exit;
	}
	ret = npu_binary_read(&vector_binary, test_buf->vaddr, vector_len);
	if (ret) {
		npu_err("npu_binary_read([%s], %pK, %zu) failed : %d\n",
			vector_path, test_buf->vaddr, vector_len, ret);
		ret = -EFAULT;
		goto err_exit;
	}
	...
	/* Cache invalidation */
	__dma_map_area(test_buf->vaddr, vector_len, DMA_TO_DEVICE);
...
{
	BUG_ON(!system);
	BUG_ON(!system->fw_npu_unittest_buffer);
	...
	ft_handle->unittest_buf = system->fw_npu_unittest_buffer;
