...
static int alloc_profile_data(struct npu_profile_control *profile_ctl)
{
	...
	npu_info("profiling buffer allocated, size(%lu) / dv(%pad) / kv(" NPU_KPTR_LOG ")",
	...
	nw_cmd_e cmd, struct npu_memory_buffer *buf_handle)
{
	...
	case NPU_NW_CMD_PROFILE_START:
		if (buf_handle) {
			...
			npu_dbg("prepare profile start command: cmd(%u) v(" NPU_KPTR_LOG ") d(%pad) size(%zu)\n",
...
static int npu_profile_stop(struct npu_profile_control *profile_ctl)
{
	...
	npu_dbg("fw profile invalidate addr(" NPU_KPTR_LOG "), size(%ld)\n",
...
int npu_profile_probe(struct npu_system *system)
{
	...
	probe_info("PWM timer address = " NPU_KPTR_LOG "\n", profile_ctl->pwm);
