...
bool check_trace_code(enum kbase_ktrace_code code)
{
	...
	case KBASE_KTRACE_CODE(KBASE_DEVICE_SUSPEND_DUMMY):
	case KBASE_KTRACE_CODE(KBASE_DEVICE_RESUME_DUMMY):
	case KBASE_KTRACE_CODE(KBASE_DEVICE_PM_SUSPEND):
	case KBASE_KTRACE_CODE(KBASE_DEVICE_PM_RESUME):
	case KBASE_KTRACE_CODE(KBASE_DEVICE_PM_WAIT_WQ_RUN):
	case KBASE_KTRACE_CODE(KBASE_DEVICE_PM_WAIT_WQ_QUEUE_WORK):
	case KBASE_KTRACE_CODE(LSI_JM_IRQ):
	case KBASE_KTRACE_CODE(LSI_JM_IRQ_E):
	case KBASE_KTRACE_CODE(LSI_MMU_IRQ):
	case KBASE_KTRACE_CODE(LSI_MMU_IRQ_E):
	case KBASE_KTRACE_CODE(LSI_GPU_IRQ):
	...
	case KBASE_KTRACE_CODE(LSI_HWCNT_ON_DVFS):
	case KBASE_KTRACE_CODE(LSI_HWCNT_OFF_DVFS):
	case KBASE_KTRACE_CODE(LSI_HWCNT_ON_GPR):
	case KBASE_KTRACE_CODE(LSI_HWCNT_OFF_GPR):
	case KBASE_KTRACE_CODE(LSI_HWCNT_BT_ON):
	case KBASE_KTRACE_CODE(LSI_HWCNT_BT_OFF):
	case KBASE_KTRACE_CODE(LSI_HWCNT_VSYNC_SKIP):
	case KBASE_KTRACE_CODE(LSI_CHECKSUM):
