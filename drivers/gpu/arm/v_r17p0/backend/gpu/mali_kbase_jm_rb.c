...
/**
+ * other_ctx_active - Determine if any job slots other than @js are currently
+ *                    running atoms of a different context than a given one
+ * @kbdev: Device pointer
+ * @js:    Job slot
+ * @kctx:  Kbase context to match against
+ *
+ * Return: true if any slots other than @js are busy, false otherwise
+ */
static inline bool other_ctx_active(struct kbase_device *kbdev, int js, struct kbase_context *kctx)
{
	int slot;
	struct kbase_jd_atom *katom[SLOT_RB_SIZE];
	int idx;
	...
	for (slot = 0; slot < kbdev->gpu_props.num_job_slots; slot++) {
		...
		{
			katom[idx] = kbase_gpu_inspect(kbdev, slot, idx);

			if (katom[idx] != NULL && katom[idx]->kctx != kctx &&
	...
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
		...
		for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
			...
			case KBASE_ATOM_GPU_RB_READY:
				...
				/* if there is something on the other slot check whether
				 * it is an atom from a different context and if so
				 * disallow this submittion. */
				if (other_ctx_active(kbdev, js, katom[idx]->kctx))
				{
					break;
				}
