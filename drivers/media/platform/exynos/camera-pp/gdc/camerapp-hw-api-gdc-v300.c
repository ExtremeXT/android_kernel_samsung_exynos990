...
#include "camerapp-hw-reg-gdc-v300.h"
#include <linux/smc.h>
...
{
	camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_INT_STATUS], 0x0007);
}
...
void camerapp_hw_gdc_update_param(void __iomem *base_addr, struct gdc_dev *gdc)
{
	...
	if (d_frame->pixel_size == CAMERAPP_PIXEL_SIZE_8_2BIT)
		output_dither = 1;
	camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_GDC_DITHER],
