...
#include "camerapp-hw-reg-gdc-v310.h"
#include <linux/smc.h>

u32 camerapp_hw_gdc_get_ver(void __iomem *base_addr)
{
	return camerapp_sfr_get_reg(base_addr, &gdc_regs[GDC_R_GDC_VER]);
}
...
{
	camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_INT_STATUS], 0x0003);
}
 ...
 */
	...
	camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_WAXI_STRIDE_HEADER_LUM],
		&gdc_fields[GDC_F_WRITE_STRIDE_HEADER_LUMA], output_stride_lum_w_2bit);
	camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_WAXI_STRIDE_HEADER_CHROMA],
...
void camerapp_hw_gdc_set_dma_address(void __iomem *base_addr, struct gdc_frame *s_frame, struct gdc_frame *d_frame)
{
	...
	camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_GDC_DST_HEADER_BASE_LUM],
		&gdc_fields[GDC_F_GDC_DST_HEADER_BASE_LUM], d_frame->addr.y_2bit);
	camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_GDC_DST_HEADER_BASE_CHROMA],
 ...
 * 4 : 128 byte(10bit 80%)
 */
void camerapp_hw_gdc_set_compressor(void __iomem *base_addr, struct gdc_frame *d_frame)
{
	if (d_frame->extra == COMP) {
		camerapp_sfr_set_field(base_addr, &gdc_regs[GDC_R_GDC_COMPRESSOR],
...
void camerapp_hw_gdc_update_param(void __iomem *base_addr, struct gdc_dev *gdc)
{
	...
	/* gdc SBWC */
	camerapp_hw_gdc_set_compressor(base_addr, d_frame);
...
void camerapp_hw_gdc_status_read(void __iomem *base_addr)
{
	...
	isPixelBusy = camerapp_sfr_get_field(base_addr,
		&gdc_regs[GDC_R_GDC_DEBUG_BUSY], &gdc_fields[GDC_F_PXCBUSYLUM]);
	isPortBusy = camerapp_sfr_get_field(base_addr,
