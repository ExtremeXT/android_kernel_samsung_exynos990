...
static ssize_t camera_awb_master_show(char *buf, enum is_cam_info_index cam_index)
{
	struct is_rom_info *finfo;
	struct is_cam_info *cam_info;
	int position;
	...
	if (rom_cal_index == 1) {
		memcpy(buf, &cal_buf[finfo->rom_sensor2_awb_master_addr], IS_AWB_MASTER_DATA_SIZE);
	} else {
		memcpy(buf, &cal_buf[finfo->rom_awb_master_addr], IS_AWB_MASTER_DATA_SIZE);
	}

	return IS_AWB_MASTER_DATA_SIZE;

err_awb_master:
	memset(buf, 0xFF, IS_AWB_MASTER_DATA_SIZE);

	return IS_AWB_MASTER_DATA_SIZE;
}


static ssize_t camera_awb_module_show(char *buf, enum is_cam_info_index cam_index)
{
	struct is_rom_info *finfo;
	struct is_cam_info *cam_info;
	int position;
	...
	if (rom_cal_index == 1) {
		memcpy(buf, &cal_buf[finfo->rom_sensor2_awb_module_addr], IS_AWB_MODULE_DATA_SIZE);
	} else {
		memcpy(buf, &cal_buf[finfo->rom_awb_module_addr], IS_AWB_MODULE_DATA_SIZE);
	}

	return IS_AWB_MODULE_DATA_SIZE;

err_afcal:
	memset(buf, 0xFF, IS_AWB_MODULE_DATA_SIZE);
...
static ssize_t camera_paf_offset_show(char *buf, bool is_mid, int f_index)
{
	...
	case FNUMBER_1ST :
		...
		if (is_mid)
			paf_offset_base = ROM_PAF_CAL_DATA_START_ADDR + IS_PAF_OFFSET_MID_OFFSET;
		else
			paf_offset_base = ROM_PAF_CAL_DATA_START_ADDR + IS_PAF_OFFSET_PAN_OFFSET;
#endif
		break;
	case FNUMBER_2ND :
#ifdef ROM_F2_PAF_CAL_DATA_START_ADDR
		if (is_mid)
			paf_offset_base = ROM_F2_PAF_CAL_DATA_START_ADDR + IS_PAF_OFFSET_MID_OFFSET;
		else
			paf_offset_base = ROM_F2_PAF_CAL_DATA_START_ADDR + IS_PAF_OFFSET_PAN_OFFSET;
#endif
		break;
	case FNUMBER_3RD :
#ifdef ROM_F3_PAF_CAL_DATA_START_ADDR
		if (is_mid)
			paf_offset_base = ROM_F3_PAF_CAL_DATA_START_ADDR + IS_PAF_OFFSET_MID_OFFSET;
		else
			paf_offset_base = ROM_F3_PAF_CAL_DATA_START_ADDR + IS_PAF_OFFSET_PAN_OFFSET;
#endif
		break;
	default :
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (force_caldata_dump == false) {
		ret = is_sec_run_fw_sel(ROM_ID_FRONT);
		if (ret) {
			err("is_sec_run_fw_sel is fail(%d)", ret);
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	memcpy(data, &cal_buf[ROM_REAR3_PAF_CAL_DATA_START_ADDR + IS_PAF_CAL_ERR_CHECK_OFFSET], 4);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (check_ois_power) {
		is_ois_auto_test_rear2(sysfs_core, ois_threshold_rear2,
			&x_result, &y_result, &sin_x, &sin_y,
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (check_ois_power) {
		is_ois_get_offset_data(sysfs_core, &raw_data_x, &raw_data_y);
		...
		struct device_attribute *attr, char *buf)
{
	...
	memcpy(data, &cal_buf[ROM_PAF_CAL_DATA_START_ADDR + IS_PAF_CAL_ERR_CHECK_OFFSET], 4);
	paf_err_data_result = *data | ( *(data + 1) << 8) | ( *(data + 2) << 16) | (*(data + 3) << 24);
		...
		struct device_attribute *attr, char *buf)
{
	...
	memcpy(data, &cal_buf[ROM_F2_PAF_CAL_DATA_START_ADDR + IS_PAF_CAL_ERR_CHECK_OFFSET], 4);
	paf_err_data_result = *data | ( *(data + 1) << 8) | ( *(data + 2) << 16) | (*(data + 3) << 24);
		...
		struct device_attribute *attr, char *buf)
{
	...
	memcpy(data, &cal_buf[ROM_F3_PAF_CAL_DATA_START_ADDR + IS_PAF_CAL_ERR_CHECK_OFFSET], 4);
	paf_err_data_result = *data | ( *(data + 1) << 8) | ( *(data + 2) << 16) | (*(data + 3) << 24);
...
#ifdef ROM_PAF_CAL_DATA_START_ADDR_FRONT
