...
int is_af_power(struct is_device_af *af_device, bool onoff)
{
	...
	/* VDDAF_COMMON_CAM */
	ret = is_af_ldo_enable("VDDAF_COMMON_CAM", onoff);
	if (ret) {
	...
	/* OIS_VDD_2.8V */
	ret = is_af_ldo_enable("VDDD_2.8V_OIS", onoff);
	if (ret) {
	...
	/* VDD_VM_2.8V_OIS */
	ret = is_af_ldo_enable("VDD_VM_2.8V_OIS", onoff);
	if (ret) {
	...
	/* OIS_VDD_1.8V */
	ret = is_af_ldo_enable("VDDD_1.8V_OIS", onoff);
	if (ret) {
...
int16_t is_af_enable(void *device, bool onoff)
{
	...
	if (!camera_running) {
		if (onoff) {
			...
			af_regulator = is_check_regulator_status("VDDAF_COMMON_CAM");
...
int16_t is_af_move_lens(struct is_core *core)
{
	...
	info("is_af_move_lens : camera_running = %d\n", camera_running);
	if (!camera_running) {
		if (actuator->vendor_use_standby_mode) {
...
int16_t is_af_move_lens_rear2(struct is_core *core)
{
	int ret = 0;
	struct is_actuator *actuator = NULL;
	struct i2c_client *client = NULL;
	...
	info("is_af_move_lens : camera_running_rear2 = %d\n", camera_running_rear2);
