...
int is_mcu_empty_check_clear(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
retry:
	...
	if (empty_check > 0)
		is_mcu_empty_check_clear(subdev);
	else
		is_mcu_disconnect(subdev, core);
...
p_err:
	...
	is_mcu_set_aperture_onboot(core);
...
int is_ois_init_mcu(struct v4l2_subdev *subdev)
{
	...
	if ((ois_wide_init == true && module->position == SENSOR_POSITION_REAR) ||
	 ...
	 if (!ois_hw_check) {
		...
		if (val == 0x01) {
			ret = is_ois_i2c_write_multi(client, 0x0254, ois_pinfo.wide_xgg, 6);
			ret |= is_ois_i2c_write_multi(client, 0x0258, ois_pinfo.wide_ygg, 6);
			ret |= is_ois_i2c_write_multi(client, 0x0554, ois_pinfo.tele_xgg, 6);
			ret |= is_ois_i2c_write_multi(client, 0x0558, ois_pinfo.tele_ygg, 6);
			if (ret < 0)
				err("ois gyro data write is fail");

			ret = is_ois_i2c_write_multi(client, 0x0442, ois_pinfo.wide_xcoef, 4);
			ret |= is_ois_i2c_write_multi(client, 0x0444, ois_pinfo.wide_ycoef, 4);
			ret |= is_ois_i2c_write_multi(client, 0x0446, ois_pinfo.tele_xcoef, 4);
			ret |= is_ois_i2c_write_multi(client, 0x0448, ois_pinfo.tele_ycoef, 4);
			if (ret < 0)
...
#endif
			ret = is_ois_i2c_write(client, 0x0240, wx_pole);
			ret |= is_ois_i2c_write(client, 0x0241, wy_pole);
			ret |= is_ois_i2c_write(client, 0x0242, gyro_orientation);
#ifdef CAMERA_2ND_OIS
...
bool is_ois_gyro_cal_mcu(struct is_core *core, long *x_value, long *y_value)
{
	int ret = 0;
	u8 val = 0, x = 0, y = 0;
	int retries = 30;
	...
	if ((val & 0x23) == 0x0) {
		...
		ret = is_ois_i2c_write(client, 0x03, 0x01);
		if (ret) {
			err("i2c write fail\n");
		}
		msleep(170);
		retries = 20;
		do {
			ret = is_ois_i2c_read(client, 0x03, &val);
			if (ret != 0) {
				val = -EIO;
				err("i2c read fail\n");
				break;
		...
		ret = is_ois_i2c_read(client, 0x0027, &val);
		if (ret != 0) {
			val = -EIO;
			err("i2c read fail\n");
			goto exit;
...
bool is_ois_offset_test_mcu(struct is_core *core, long *raw_data_x, long *raw_data_y)
{
	int ret = 0, i = 0;
	u8 val = 0, x = 0, y = 0;
...
void is_ois_get_offset_data_mcu(struct is_core *core, long *raw_data_x, long *raw_data_y)
{
	int i = 0;
	int ret = 0;
	u8 val = 0, x = 0, y = 0;
...
int is_mcu_probe(struct i2c_client *client,
...
MODULE_DEVICE_TABLE(of, exynos_is_mcu_match);
