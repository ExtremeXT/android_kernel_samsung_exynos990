...
#include "is-dt.h"
#include "is-cis-2p6.h"
#include "is-cis-2p6-setA.h"
#include "is-cis-2p6-setB.h"
...
int sensor_2p6_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cis->use_pdaf == true) {
		...
		ret = is_sensor_write16(cis->client, 0x0B0E, 0x0000);
		if (ret < 0) {
			err("2p6 sensor write fail !!!");
			goto p_err;
		}
		...
		if (cis->cis_data->is_data.paf_stat_enable == false) {
			...
			ret = is_sensor_write16(cis->client, 0x602A, 0x1BB0);
			if (ret < 0) {
				err("2p6 sensor write fail !!!");
				goto p_err;
			}
	...
	if (cis->cis_data->is_data.wdr_enable == false) {
		...
		ret = is_sensor_write16(cis->client, 0x0216, 0x0000);
		if (ret < 0) {
			err("2p6 sensor write fail !!!");
			goto p_err;
		}
...
int sensor_2p6_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (is_vender_wdr_mode_on(cis_data)) {
		/* wdr on */
		ret = is_sensor_write16(client, 0x0216, 0x0100);
		if (ret < 0)
			err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0216, 0x0100, ret);
		/* Apply seperating long and short dgain */
		ret = is_sensor_write16(client, 0x31FA, 0x0001);
		if (ret < 0)
			err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x31FA, 0x0001, ret);
	} else {
		/* wdr off */
		ret = is_sensor_write16(client, 0x0216, 0x0000);
		if (ret < 0)
			err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0216, 0x0000, ret);
		/* Apply common value long and short dgain */
		ret = is_sensor_write16(client, 0x31FA, 0x0000);
		if (ret < 0)
			err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x31FA, 0x0000, ret);
	}
#endif
...
int sensor_2p6_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (is_vender_wdr_mode_on(cis_data)) {
		...
		ret = is_sensor_write16(client, 0x0C82, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
static const struct of_device_id exynos_is_cis_2p6_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-2p6",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_cis_2p6_match);
