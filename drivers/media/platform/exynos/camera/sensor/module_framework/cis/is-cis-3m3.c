...
#include "is-dt.h"
#include "is-cis-3m3.h"
#include "is-cis-3m3-setA.h"
#include "is-cis-3m3-setB.h"
...
int sensor_3m3_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (device->image.framerate == 24
		...
		is_sensor_write16(client, 0x30DA, 0x00A9);
		is_sensor_write16(client, 0x30DC, 0x00A9);
	} else {
		is_sensor_write16(client, 0x30DA, 0x0016);
		is_sensor_write16(client, 0x30DC, 0x0016);
	}
#endif
...
int sensor_3m3_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* WDR mode */
	if (is_vender_wdr_mode_on(cis_data)) {
		is_sensor_write16(client, 0x0216, 0x0100);
	} else {
		is_sensor_write16(client, 0x0216, 0x0000);
	}
...
int sensor_3m3_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* DGain WDR Mode */
	if (is_vender_wdr_mode_on(cis_data)) {
		is_sensor_write16(client, 0x31FC, 0x0001);
	} else {
		is_sensor_write16(client, 0x31FC, 0x0000);
	}
	...
	/* Long digital gain */
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = is_sensor_write16(client, 0x3062, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_3m3_cis_set_adjust_sync(struct v4l2_subdev *subdev, u32 sync)
{
	...
	ret = is_sensor_write8(client, 0x30D7, 0x01);
...
#ifdef CAMERA_REAR2_SENSOR_SHIFT_CROP
static int sensor_3m3_cis_update_pdaf_tail_size(struct v4l2_subdev *subdev, struct is_sensor_cfg *select)
{
	struct is_cis *cis = NULL;
	struct i2c_client *client;
	char *cal_buf;
