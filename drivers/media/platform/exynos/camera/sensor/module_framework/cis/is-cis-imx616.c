...
int sensor_imx616_cis_get_fineIntegTime(struct is_cis *cis)
{
	...
	ret = is_sensor_read16(client, REG(FINE_INTEG_TIME), &fine_integ_time);
	if (ret < 0) {
...
int sensor_imx616_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret = is_sensor_write16(client, REG(MID_COARSE_INTEG_TIME), middle_coarse_int);
		CHECK_GOTO(ret < 0, p_i2c_err);
...
int sensor_imx616_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret = is_sensor_write16(client, REG(MID_DGAIN), middle_gain);
		CHECK_GOTO(ret < 0, p_i2c_err);

		ret = is_sensor_write16(client, REG(ST_DGAIN), short_gain);
		CHECK_GOTO(ret < 0, p_i2c_err);
	}
