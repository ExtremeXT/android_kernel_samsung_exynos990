...
#include "is-dt.h"
#include "is-cis-imx219.h"
#include "is-cis-imx219-setA.h"
#include "is-cis-imx219-setB.h"

#include "is-helper-i2c.h"

#define SENSOR_NAME "IMX219"
/* #define DEBUG_IMX219_PLL */

static const struct v4l2_subdev_ops subdev_ops;

static const u32 **sensor_imx219_setfiles;
static const u32 *sensor_imx219_setfile_sizes;
static const struct sensor_pll_info **sensor_imx219_pllinfos;
static u32 sensor_imx219_max_setfile_num;

static void sensor_imx219_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_IMX219_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_IMX219_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_IMX219_COARSE_INTEGRATION_TIME_MIN;
...
int sensor_imx219_cis_init(struct v4l2_subdev *subdev)
{
	...
	cis->cis_data->cur_width = SENSOR_IMX219_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_IMX219_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_imx219_cis_data_calculation(sensor_imx219_pllinfos[setfile_index], cis->cis_data);
...
int sensor_imx219_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	sensor_cis_dump_registers(subdev, sensor_imx219_setfiles[0], sensor_imx219_setfile_sizes[0]);
...
static int sensor_imx219_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_imx219_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_imx219_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_imx219_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_imx219_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	if (mode > sensor_imx219_max_setfile_num) {
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_imx219_check_rev is fail");
			goto p_err;
		}
	}
	...
	if (ret < 0) {
		err("sensor_imx219_set_registers fail!!");
		goto p_err;
	}
...
int sensor_imx219_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	...
	ret = sensor_imx219_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_IMX219_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_IMX219_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_IMX219_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_IMX219_MAX_HEIGHT)) {
...
int sensor_imx219_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	ret = sensor_imx219_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
		err("[%s] sensor_imx219_cis_group_param_hold_func fail\n", __func__);

#ifdef DEBUG_IMX219_PLL
	{
	u16 pll, pll_h, pll_l;
	...
	ret = is_sensor_read16(client, 0x0306, &pll_h);
	ret = is_sensor_read16(client, 0x0307, &pll_l);
	dbg_sensor(1, "______ pll_multiplier(%x%x)\n", pll_h, pll_l);
	...
	ret = is_sensor_read16(client, 0x030c, &pll_h);
	ret = is_sensor_read16(client, 0x030d, &pll_l);
	dbg_sensor(1, "______ secnd_pll_multiplier(%x%x)\n", pll_h, pll_l);
	ret = is_sensor_read16(client, 0x0160, &pll_h);
	ret = is_sensor_read16(client, 0x0161, &pll_l);
	dbg_sensor(1, "______ frame_length_lines(%x%x)\n", pll_h, pll_l);
	ret = is_sensor_read16(client, 0x0162, &pll_h);
	ret = is_sensor_read16(client, 0x0163, &pll_l);
	dbg_sensor(1, "______ line_length_pck(%x%x)\n", pll_h, pll_l);
	}
#endif
...
int sensor_imx219_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_imx219_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	hold = sensor_imx219_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_imx219_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_imx219_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_imx219_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_imx219_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_imx219_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx219_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_imx219_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_imx219_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx219_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx219_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_imx219_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_imx219_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx219_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx219_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_imx219_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_imx219_cis_init,
	.cis_log_status = sensor_imx219_cis_log_status,
	.cis_group_param_hold = sensor_imx219_cis_group_param_hold,
	.cis_mode_change = sensor_imx219_cis_mode_change,
	.cis_set_size = sensor_imx219_cis_set_size,
	.cis_stream_on = sensor_imx219_cis_stream_on,
	.cis_stream_off = sensor_imx219_cis_stream_off,
	.cis_set_exposure_time = sensor_imx219_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_imx219_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_imx219_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_imx219_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_imx219_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_imx219_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_imx219_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_imx219_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_imx219_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_imx219_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_imx219_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_imx219_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_imx219_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_imx219_cis_get_min_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_IMX219);
	if (!sensor_peri) {
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_imx219_setfiles = sensor_imx219_setfiles_A;
		sensor_imx219_setfile_sizes = sensor_imx219_setfile_A_sizes;
		sensor_imx219_pllinfos = sensor_imx219_pllinfos_A;
		sensor_imx219_max_setfile_num = sizeof(sensor_imx219_setfiles_A) / sizeof(sensor_imx219_setfiles_A[0]);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_imx219_setfiles = sensor_imx219_setfiles_B;
		sensor_imx219_setfile_sizes = sensor_imx219_setfile_B_sizes;
		sensor_imx219_pllinfos = sensor_imx219_pllinfos_B;
		sensor_imx219_max_setfile_num = sizeof(sensor_imx219_setfiles_B) / sizeof(sensor_imx219_setfiles_B[0]);
	} else {
...
static const struct of_device_id sensor_cis_imx219_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-imx219",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_imx219_match);

static const struct i2c_device_id sensor_cis_imx219_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_imx219_driver = {
	.probe	= cis_imx219_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_imx219_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_imx219_idt
};

static int __init sensor_cis_imx219_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_imx219_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_imx219_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_imx219_init);
