...
#include "is-dt.h"
#include "is-cis-2l1.h"
#include "is-cis-2l1-setA.h"
#include "is-cis-2l1-setB.h"
...
int sensor_2l1_cis_check_rev(struct is_cis *cis)
{
	int ret = 0;
	u8 rev = 0;
	...
	switch (rev) {
	case 0xC1:
		dbg_sensor(1, "2l1 sensor Rev. 2.1\n");
		break;
	case 0xA0:
	case 0xB1:
	case 0xC0:
		dbg_sensor(1, "Old version 2l1 sensor. Rev. 0x%X\n", rev);
		break;
	case 0x00:
		warn("Set 2L1 W/A!!. Forcely set revision(0x%X\n) to Rev 2.1\n", rev);
		/* Sensor Initialize and write revision (0xC1) */
		is_sensor_write16(client, 0x6018, 0x0001);
...
int sensor_2l1_cis_set_global_setting_internal(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_2l1_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ext_info->use_retention_mode == SENSOR_RETENTION_USE) {
		sensor_2l1_cis_retention_crc_check(subdev);
	}
	...
	if (ext_info->use_retention_mode != SENSOR_RETENTION_DISABLE) {
		...
		case SENSOR_2L1_8064X3024_30FPS:
			...
			is_sensor_write16(cis->client, 0x0100, 0x0003);
			is_sensor_write16(cis->client, 0x021E, 0x0100);
			is_sensor_write16(cis->client, 0x6028, 0x2000);
			is_sensor_write16(cis->client, 0x602A, 0x0A78);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0x8800);
		...
		case SENSOR_2L1_8064X2268_30FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0A78);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0x8A00);
			is_sensor_write16(cis->client, 0x0100, 0x0103);
			break;
		case SENSOR_2L1_4032X2268_60FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0A78);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0x8E00);
			is_sensor_write16(cis->client, 0x0100, 0x0103);
			break;
		case SENSOR_2L1_1008X756_120FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0A78);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0x8C00);
			is_sensor_write16(cis->client, 0x0100, 0x0103);
			break;
		default:
...
int sensor_2l1_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	/* setfile global setting is at camera entrance */
	if (ext_info->use_retention_mode != SENSOR_RETENTION_USE) {
		sensor_2l1_cis_set_global_setting_internal(subdev);
		if (ext_info->use_retention_mode == SENSOR_RETENTION_READY) {
			sensor_2l1_cis_retention_prepare(subdev);
		}
	}
...
int sensor_2l1_cis_retention_crc_check(struct v4l2_subdev *subdev)
{
	...
	if(crc_check == 0x01) {
		...
		ret = sensor_2l1_cis_set_global_setting_internal(subdev);
		if (ret < 0) {
...
int sensor_2l1_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = sensor_2l1_cis_group_param_hold_func(subdev, 0x01);
	if (ret < 0)
	...
	ret = sensor_2l1_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
		err("group_param_hold_func failed at stream on");
...
int sensor_2l1_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	is_sensor_write8(client, 0x070A, 0x01);
#endif
...
int sensor_2l1_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	dbg_sensor(1, "[MOD:D:%d] %s, vsync_cnt(%d), vt_pic_clk_freq_mhz (%d),"
		...
		cis->id, __func__, cis_data->sen_vsync_count, vt_pic_clk_freq_mhz,
...
int sensor_2l1_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (ret < 0)
		goto p_err;
	ret = is_sensor_write16_array(client, 0x0216, dgains, 4);
	if (ret < 0)
		goto p_err;
	...
	if (is_vender_wdr_mode_on(cis_data)) {
		...
		ret = is_sensor_write16_array(client, 0x3072, dgains, 4);
		if (ret < 0)
			goto p_err;
		ret = is_sensor_write16_array(client, 0x307A, dgains, 4);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l1_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	if (lte_mode->sen_strm_off_on_enable) {
		...
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0B52);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1064);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0BFE);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1064);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0C0E);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1064);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0C16);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1064);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x858C);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x03C0);

		ret |= is_sensor_write16(cis->client, 0x0340, 0x7FFF);
		ret |= is_sensor_write16(cis->client, 0x0342, 0xBF00);

		/* Current save to line_length_pck for recover line_length_pck */
		lte_mode->pre_line_length_pck = cis->cis_data->line_length_pck;
		cis->cis_data->line_length_pck = 0xBF00;
	} else {
		ret |= is_sensor_write16(cis->client, 0x6028, 0x2000);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0B52);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0295);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0BFE);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0295);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0C0E);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0295);
		...
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0295);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x858C);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0220);

		ret |= is_sensor_write16(cis->client, 0x0340, 0x0C57);
		ret |= is_sensor_write16(cis->client, 0x0342, 0x2100);
...
static struct is_cis_ops cis_ops_2l1 = {
...
static const struct of_device_id sensor_cis_2l1_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-2l1",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_2l1_match);

static const struct i2c_device_id sensor_cis_2l1_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_2l1_driver = {
	.probe	= cis_2l1_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_2l1_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_2l1_idt
};

static int __init sensor_cis_2l1_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_2l1_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_2l1_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_2l1_init);
