...
#include "is-dt.h"
#include "is-cis-sr259.h"
#include "is-cis-sr259-setA.h"
#include "is-cis-sr259-setB.h"
...
static int sensor_sr259_cis_dump_registers(struct v4l2_subdev *subdev, const u32 *regs, const u32 size)
{
	...
	for (i = 0; i < size; i += I2C_WRITE) {
		if (regs[i + I2C_BYTE] == I2C_WRITE_ADDR8_DATA8) {
			...
			ret = is_sensor_addr8_read8(client, regs[i + I2C_ADDR], &data8);
			if (ret < 0) {
				err("is_sensor_addr8_read8 fail, ret(%d), addr(%#x)",
						ret, regs[i + I2C_ADDR]);
			} else {
...
int sensor_sr259_cis_init(struct v4l2_subdev *subdev)
{
	...
	is_sensor_addr8_read8(client, 0x04, &data8);
...
*/
	...
	ret = is_sensor_addr8_write8(client, 0x03, 0x00); // page 0 set
	if (ret < 0)
		goto p_err;

	is_sensor_addr8_read8(client, 0x05, &data8);
...
static int sensor_sr259_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	ret = is_sensor_addr8_write8(client, 0x1f, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_sr259_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	is_sensor_addr8_write8(client, 0x01, 0x00);
...
int sensor_sr259_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	is_sensor_addr8_write8(client, 0x01, 0x20);
...
int sensor_sr259_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* page select */
	ret = is_sensor_addr8_write8(client, 0x03, 0x20); // page 20 set
	if (ret < 0)
		goto p_err;

	/* Short exposure */
	ret = is_sensor_addr8_write8(client, 0x22, (short_coarse_int>>8)&0xff);
	if (ret < 0)
		goto p_err;

	ret = is_sensor_addr8_write8(client, 0x23, short_coarse_int&0xff);
	if (ret < 0)
		goto p_err;
...
int sensor_sr259_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = is_sensor_addr8_write8(client, 0x60, analog_gain_msb);
	if (ret < 0)
		goto p_err;

	ret = is_sensor_addr8_write8(client, 0x61, analog_gain_lsb);
	if (ret < 0)
		goto p_err;
...
int sensor_sr259_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = is_sensor_read16(client, 0x60, &analog_gain_msb);
	if (ret < 0)
		goto p_err;

	ret = is_sensor_read16(client, 0x61, &analog_gain_lsb);
	if (ret < 0)
		goto p_err;
