...
#include "is-dt.h"
#include "is-cis-4h5yc.h"
#include "is-cis-4h5yc-setA.h"
#include "is-cis-4h5yc-setB.h"
...
int sensor_4h5yc_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev, u32 expo, u32 *again, u32 *dgain)
{
	...
	coarse_int = (((expo * vt_pic_clk_freq_khz) / 1000) - (cis_data->line_length_pck - 0xf0)) / line_length_pck;
	...
	integration_time = ((cis_data->line_length_pck * coarse_int + (cis_data->line_length_pck - 0xf0)) / (vt_pic_clk_freq_khz / 1000));
...
static const struct of_device_id sensor_cis_4h5yc_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-4h5yc",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_4h5yc_match);

static const struct i2c_device_id sensor_cis_4h5yc_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_4h5yc_driver = {
	.probe	= cis_4h5yc_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_4h5yc_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_4h5yc_idt
};

static int __init sensor_cis_4h5yc_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_4h5yc_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_4h5yc_driver.driver.name, ret);
