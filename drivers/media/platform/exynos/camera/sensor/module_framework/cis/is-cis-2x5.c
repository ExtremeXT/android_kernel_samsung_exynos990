...
int sensor_2x5_cis_write_cal(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write16(client, SENSOR_2X5_W_DIR_PAGE, cal_page_lsc);
	...
	for (i = 0; i < SENSOR_2X5_CAL_LSC_SIZE / 2; i++) {
		ret = is_sensor_write16(client, cal_offset_lsc + (i * 2), cpu_to_be16(lsc_buf[i]));
	...
	for (i = 0; i < SENSOR_2X5_CAL_XTALK_SIZE / 2; i++) {
		ret = is_sensor_write16(client, cal_offset_xtalk + (i * 2), cpu_to_be16(xtalk_buf[i]));
...
int sensor_2x5_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cal_done == true) {
		ret = is_sensor_write16(client, SENSOR_2X5_W_DIR_PAGE, 0x4000);
		if (ret < 0) {
...
int sensor_2x5_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	if (false /* is_vender_wdr_mode_on(cis_data)*/) {
...
int sensor_2x5_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	ret = is_sensor_write16(client, SENSOR_2X5_W_DIR_PAGE, addr_page);
	if (ret < 0) {
		err("i2c fail addr(%x, %x) ,ret = %d\n", SENSOR_2X5_W_DIR_PAGE, addr_page, ret);
		goto p_err;
	}
	for (i = 0; i < ARRAY_SIZE(abs_gains); i++) {
		ret = is_sensor_write16(client, addr_offset + (i * 2), abs_gains[i]);
