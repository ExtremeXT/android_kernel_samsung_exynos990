...
#include "is-dt.h"
#include "is-cis-3p8sp.h"
#include "is-cis-3p8sp-setA.h"
#include "is-cis-3p8sp-setB.h"

#include "is-helper-i2c.h"

#define SENSOR_NAME "S5K3P8SP"
/* #define DEBUG_3P8SP_PLL */

static const struct v4l2_subdev_ops subdev_ops;

static const u32 *sensor_3p8sp_global;
static u32 sensor_3p8sp_global_size;
...
/* CIS OPS */
int sensor_3p8sp_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	u32 setfile_index = 0;
	...
	cis->cis_data->cur_width = SENSOR_3P8SP_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_3P8SP_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_3p8sp_cis_data_calculation(sensor_3p8sp_pllinfos[setfile_index], cis->cis_data);
...
int sensor_3p8sp_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
...
static int sensor_3p8sp_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_3p8sp_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_3p8sp_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_3p8sp_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = is_sensor_write16(cis->client, 0x6010, 0x0001);
	/* 3ms delay to operate sensor FW */
	usleep_range(2900, 3000);

	ret = sensor_cis_set_registers(subdev, sensor_3p8sp_global, sensor_3p8sp_global_size);
...
int sensor_3p8sp_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_3p8sp_check_rev is fail");
			goto p_err;
		}
	}

	sensor_3p8sp_cis_data_calculation(sensor_3p8sp_pllinfos[mode], cis->cis_data);

	ret = sensor_cis_set_registers(subdev, sensor_3p8sp_setfiles[mode], sensor_3p8sp_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_3p8sp_set_registers fail!!");
		goto p_err;
	}
...
int sensor_3p8sp_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_3p8sp_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_3P8SP_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_3P8SP_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_3P8SP_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_3P8SP_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_3P8SP_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_3P8SP_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_3p8sp_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	if (ret < 0)
		err("[%s] sensor_3p8sp_cis_group_param_hold_func fail\n", __func__);

#ifdef DEBUG_3P8SP_PLL
	{
	u16 pll;
...
int sensor_3p8sp_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x6028, 0x4000, ret);
	ret = is_sensor_write16(client, 0x0100, 0x0000);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0100, 0x00, ret);
...
int sensor_3p8sp_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
p_err:
	if (hold > 0) {
		hold = sensor_3p8sp_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_3p8sp_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_3p8sp_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_3p8sp_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_3p8sp_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_3p8sp_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_3p8sp_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_3p8sp_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_3p8sp_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_3p8sp_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_3p8sp_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_3p8sp_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_3p8sp_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_3p8sp_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	/* 3P8SP cannot read min/max digital gain */
...
int sensor_3p8sp_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_3p8sp_cis_init,
	.cis_log_status = sensor_3p8sp_cis_log_status,
	.cis_group_param_hold = sensor_3p8sp_cis_group_param_hold,
	.cis_set_global_setting = sensor_3p8sp_cis_set_global_setting,
	.cis_mode_change = sensor_3p8sp_cis_mode_change,
	.cis_set_size = sensor_3p8sp_cis_set_size,
	.cis_stream_on = sensor_3p8sp_cis_stream_on,
	.cis_stream_off = sensor_3p8sp_cis_stream_off,
	.cis_set_exposure_time = sensor_3p8sp_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_3p8sp_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_3p8sp_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_3p8sp_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_3p8sp_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_3p8sp_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_3p8sp_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_3p8sp_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_3p8sp_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_3p8sp_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_3p8sp_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_3p8sp_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_3p8sp_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_3p8sp_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_3p8sp_cis_get_max_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_3p8sp_setfile_sizes = sensor_3p8sp_setfile_A_sizes;
		...
		sensor_3p8sp_setfile_sizes = sensor_3p8sp_setfile_B_sizes;
		sensor_3p8sp_max_setfile_num = ARRAY_SIZE(sensor_3p8sp_setfiles_B);
		sensor_3p8sp_pllinfos = sensor_3p8sp_pllinfos_B;
	} else {
