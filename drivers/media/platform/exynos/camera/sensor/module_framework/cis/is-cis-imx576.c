...
int sensor_imx576_cis_check_rev(struct is_cis *cis)
{
	 ...
	 */
	ret = is_sensor_read8(client, 0x0018, &rev);
	if (ret < 0) {
...
int sensor_imx576_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	ret = is_sensor_write8_array(client, SENSOR_IMX576_COARSE_INTEG_TIME_ADDR, arrayBuf, 2);
		if (ret < 0)
			goto p_err;
...
int sensor_imx576_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = is_sensor_write8_array(client, SENSOR_IMX576_ANALOG_GAIN_ADDR, arrayBuf, 2);
	if (ret < 0)
		goto p_err;
...
int sensor_imx576_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = is_sensor_read16(client, SENSOR_IMX576_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_imx576_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (is_vender_wdr_mode_on(cis_data)) {
		...
		ret = is_sensor_write8_array(client, SENSOR_IMX576_SOHT_DIG_GAIN_ADDR, dgains, 2);
		if (ret < 0)
			goto p_err;
	}
	dgains[0] = (long_gain & 0xFF00) >> 8;
	dgains[1] = long_gain & 0xFF;
	ret = is_sensor_write8_array(client, SENSOR_IMX576_DIG_GAIN_ADDR, dgains, 2);
	if (ret < 0)
		goto p_err;
...
int sensor_imx576_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	ret = is_sensor_read16(client, SENSOR_IMX576_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_imx576_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	...
	if (lte_mode->sen_strm_off_on_enable) {
		if (lte_mode->expo[0] > 500000) {
			...
			ret |= is_sensor_write8(cis->client, 0x3100, cit_lshift_val);
		}
	} else {
		cit_lshift_val = 0;
		ret |= is_sensor_write8(cis->client, 0x3100, cit_lshift_val);
...
int sensor_imx576_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	ret = is_sensor_write16_array(client, SENSOR_IMX576_ABS_GAIN_GR_SET_ADDR, abs_gains, 4);
	if (ret < 0)
		goto p_err;
