...
static const struct cam_mipi_sensor_mode *sensor_gh1_mipi_sensor_mode;
static u32 sensor_gh1_mipi_sensor_mode_size;
static const int *sensor_gh1_verify_sensor_mode;
static int sensor_gh1_verify_sensor_mode_size;

static int sensor_gh1_cis_set_mipi_clock(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	if (mode >= sensor_gh1_mipi_sensor_mode_size) {
	...
	if (cis->mipi_clock_index_cur != cis->mipi_clock_index_new
		...
		cur_mipi_sensor_mode = &sensor_gh1_mipi_sensor_mode[mode];
...
#ifdef CAMERA_GH1_CAL_MODULE_VERSION
	if (finfo->header_ver[10] < CAMERA_GH1_CAL_MODULE_VERSION
#ifdef CAMERA_GH1_CAL_MODULE_VERSION_FOR_Z3
		|| (finfo->header_ver[1] == '1')
		|| (finfo->header_ver[10] == 'B' && finfo->header_ver[8] == '0'))
#endif
	{
		info("[%s] skip calibration, cal value not valid(cur_header : %s)", __func__, finfo->header_ver);
		xtc_skip = true;
...
int sensor_gh1_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
#ifdef USE_CAMERA_MIPI_CLOCK_VARIATION
	sensor_gh1_cis_set_mipi_clock(subdev);
#endif
...
static int sensor_gh1_cis_update_mipi_info(struct v4l2_subdev *subdev)
{
	...
	if (device->cfg->mode >= sensor_gh1_mipi_sensor_mode_size) {
		err("sensor mode is out of bound");
		return -1;
	}

	cur_mipi_sensor_mode = &sensor_gh1_mipi_sensor_mode[device->cfg->mode];
...
static int sensor_gh1_cis_get_mipi_clock_string(struct v4l2_subdev *subdev, char *cur_mipi_str)
{
	struct is_cis *cis = NULL;
	struct is_device_sensor *device;
	const struct cam_mipi_sensor_mode *cur_mipi_sensor_mode;
...
#ifdef USE_CAMERA_MIPI_CLOCK_VARIATION
		sensor_gh1_mipi_sensor_mode = sensor_gh1_setfile_A_mipi_sensor_mode;
		sensor_gh1_mipi_sensor_mode_size = ARRAY_SIZE(sensor_gh1_setfile_A_mipi_sensor_mode);
		sensor_gh1_verify_sensor_mode = sensor_gh1_setfile_A_verify_sensor_mode;
		sensor_gh1_verify_sensor_mode_size = ARRAY_SIZE(sensor_gh1_setfile_A_verify_sensor_mode);
#endif
	}
#if 0
...
#ifdef USE_CAMERA_MIPI_CLOCK_VARIATION
	for (i = 0; i < sensor_gh1_verify_sensor_mode_size; i++) {
		index = sensor_gh1_verify_sensor_mode[i];

		if (is_vendor_verify_mipi_channel(sensor_gh1_mipi_sensor_mode[index].mipi_channel,
					sensor_gh1_mipi_sensor_mode[index].mipi_channel_size)) {
