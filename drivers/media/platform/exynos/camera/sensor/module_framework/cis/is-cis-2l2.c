...
#include "is-dt.h"
#include "is-cis-2l2.h"
#include "is-cis-2l2-setA.h"
#include "is-cis-2l2-setB.h"
...
int sensor_2l2_cis_check_rev(struct is_cis *cis)
{
	int ret = 0;
	u8 rev = 0;
...
int sensor_2l2_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ext_info->use_retention_mode != SENSOR_RETENTION_DISABLE) {
		...
		case SENSOR_2L2_8064X3024_30FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0AA0);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0xA100);
			break;
		case SENSOR_2L2_8064X2268_30FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0AA0);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0xA400);
			break;
		case SENSOR_2L2_4032X2268_60FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0AA0);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0xAA00);
			break;
		case SENSOR_2L2_1008X756_120FPS:
			...
			is_sensor_write16(cis->client, 0x602A, 0x0AA0);
			is_sensor_write16(cis->client, 0x6F12, 0x2000);
			is_sensor_write16(cis->client, 0x6F12, 0xA700);
			break;
		default:
...
int sensor_2l2_cis_retention_crc_check(struct v4l2_subdev *subdev)
{
	...
	is_sensor_read8(cis->client, 0x6F12, &crc_check);
...
int sensor_2l2_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	is_sensor_write16(client, 0x6028, 0x2000);
	is_sensor_write16(client, 0x602A, 0x0ACD);
	is_sensor_write8(client, 0x6F12, 0x01);
#endif
...
int sensor_2l2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = is_sensor_write16(client, 0x3072, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l2_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	...
	if (lte_mode->sen_strm_off_on_enable) {
		...
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0C10);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1133);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0D68);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1133);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0D88);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1133);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0D98);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x1133);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0B62);
        ret |= is_sensor_write16(cis->client, 0x6F12, 0x03C0);

		ret |= is_sensor_write16(cis->client, 0x0340, 0x7FFF);
		ret |= is_sensor_write16(cis->client, 0x0342, 0xE500);
		...
		ret |= is_sensor_write16(cis->client, 0x6028, 0x2000);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0C10);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0361);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0D68);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0361);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0D88);
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0361);
		...
		ret |= is_sensor_write16(cis->client, 0x6F12, 0x0361);
		ret |= is_sensor_write16(cis->client, 0x602A, 0x0B62);
        ret |= is_sensor_write16(cis->client, 0x6F12, 0x0102);

		ret |= is_sensor_write16(cis->client, 0x0340, 0x24F0);
		ret |= is_sensor_write16(cis->client, 0x0342, 0x27B0);
...
static struct is_cis_ops cis_ops_2l2 = {
