...
#include "is-dt.h"
#include "is-cis-imx320.h"
#include "is-cis-imx320-setA.h"
#include "is-cis-imx320-setB.h"
...
int sensor_imx320_cis_check_rev(struct is_cis *cis)
{
	int ret = 0;
	u8 rev = 0;
	...
	ret = is_sensor_read8(client, 0x0A21, &rev);
	if (ret < 0) {
...
int sensor_imx320_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ext_info->use_retention_mode != SENSOR_RETENTION_DISABLE) {
		...
		default:
			...
			switch (index) {
			case 0:
				is_sensor_write8(cis->client, 0x9007, 0x20);
				break;
			case 1:
				is_sensor_write8(cis->client, 0x9007, 0x21);
				break;
			case 2:
				is_sensor_write8(cis->client, 0x9007, 0x22);
				break;
			case 3:
				is_sensor_write8(cis->client, 0x9007, 0x23);
				break;
			default:
				break;
			}
			...
			is_sensor_write8(cis->client, 0x40AD, 0x01); // DP/DN SWAP enabled
			is_sensor_write8(cis->client, 0x40B8, 0x02); // D0/D2 SWAP disabled
			is_sensor_write8(cis->client, 0x40B9, 0x01);
			is_sensor_write8(cis->client, 0x40BA, 0x03);
			is_sensor_write8(cis->client, 0x40BB, 0x00);
	        } else {
			/* Retention mode sensor mode select */
			switch (index) {
			case 0:
				is_sensor_write8(cis->client, 0x9007, 0x00);
				break;
			case 1:
				is_sensor_write8(cis->client, 0x9007, 0x01);
				break;
			case 2:
				is_sensor_write8(cis->client, 0x9007, 0x02);
				break;
			case 3:
				is_sensor_write8(cis->client, 0x9007, 0x03);
				break;
			default: /* Not Retention Mode */
				is_sensor_write8(cis->client, 0x9007, 0x00);
				break;
...
int sensor_imx320_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (is_vender_wdr_mode_on(cis_data)) {
                is_sensor_write8(cis->client, 0x0220, 0x61);
        } else {
                is_sensor_write8(cis->client, 0x0220, 0x00);
        }

        /* Long exposure */
        arrayBuf[0] = (cis_data->cur_long_exposure_coarse & 0xFF00) >> 8;
        arrayBuf[1] = cis_data->cur_long_exposure_coarse & 0xFF;
	ret = is_sensor_write8_array(client, 0x0202, arrayBuf, 2);
	if (ret < 0)
		goto p_err;
	...
	if (is_vender_wdr_mode_on(cis_data)) {
                ...
                ret = is_sensor_write8_array(client, 0x0224, arrayBuf, 2);
                if (ret < 0)
                        goto p_err;
        }
...
int sensor_imx320_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = is_sensor_write8_array(client, 0x0204, arrayBuf, 2);
	if (ret < 0)
		goto p_err;
...
int sensor_imx320_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (is_vender_wdr_mode_on(cis_data)) {
                ...
                ret = is_sensor_write8_array(client, 0x0226, dgains, 2);
                if (ret < 0) {
                        goto p_err;
                }
        }
        ...
        ret = is_sensor_write8_array(client, 0x020E, dgains, 2);
        if (ret < 0) {
                goto p_err;
        }
...
static struct is_cis_ops cis_ops_imx320 = {
