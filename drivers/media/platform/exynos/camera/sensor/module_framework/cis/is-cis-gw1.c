...
static const u32 *sensor_gw1_binning_ratio;
...
static int sensor_gw1_cis_check_rev(struct v4l2_subdev *subdev)
{
	...
	case 0xA2:
		probe_info("%s EVT0.2\n", __func__);
		sensor_gw1_global = sensor_gw1_setfile_EVT0P2_Global;
		sensor_gw1_global_size = ARRAY_SIZE(sensor_gw1_setfile_EVT0P2_Global);
		...
		sensor_gw1_crop_xy = sensor_gw1_crop_xy_A;
		break;
	case 0xA1:
		probe_info("%s EVT0.1\n", __func__);
	default:
...
static int sensor_gw1_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	/* NOTE: if cis is working as master, the frame duration need to be larger than slave cis.
	   Added by vivo Zhang Rui 2019/09/07 */
...
int sensor_gw1_cis_set_dual_setting(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis;
...
static int sensor_gw1_cis_get_binning_ratio(struct v4l2_subdev *subdev, u32 mode, int *binning_ratio)
{
	struct is_cis *cis = NULL;
	...
	if (!sensor_gw1_binning_ratio) {
		err("there is no binning ratio table");
		return -EINVAL;
	}

	*binning_ratio = sensor_gw1_binning_ratio[mode];
	dbg_sensor(1, "[%s] binning ratio is %d", __func__, *binning_ratio);
...
static struct is_cis_ops cis_ops = {
	...
	.cis_get_binning_ratio			= sensor_gw1_cis_get_binning_ratio,
};

static int cis_gw1_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_gw1_max_setfile_num = ARRAY_SIZE(sensor_gw1_setfiles_A);
		sensor_gw1_crop_xy = sensor_gw1_crop_xy_A;
		sensor_gw1_binning_ratio = sensor_gw1_binning_ratio_A;
