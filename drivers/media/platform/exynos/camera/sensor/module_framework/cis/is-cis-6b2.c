...
#include "is-cis-6b2-setA.h"
#include "is-cis-6b2-setB.h"

#include "is-helper-i2c.h"

#define SENSOR_NAME "S5K6B2"
/* #define DEBUG_6B2_PLL */

static const struct v4l2_subdev_ops subdev_ops;

static const u32 **sensor_6b2_setfiles;
static const u32 *sensor_6b2_setfile_sizes;
static const struct sensor_pll_info **sensor_6b2_pllinfos;
static u32 sensor_6b2_max_setfile_num;

/* If use FPGA for 6b2 sensor, necessary this function */
#if 0
static int FPGA_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	unsigned int cac_margin_x = 16;
	unsigned int cac_margin_y = 10;
...
#if defined CONFIG_CAMERA_FPGA_DPHY
	fpga_dphy_init(subdev, 0);
#endif
	is_sensor_read16(client, 0x0000, &rev);
	printk("______ cis id %d\n", rev);
	...
	frame_length_lines = 1598;
	line_length_pck = 1602;

	// Color ratio
	is_sensor_write8(client, 0x31D3, 0x01);    // efuse read en
	is_sensor_write8(client, 0x3426, 0x3A);    // [4]corr_en, [3:2]gain_b_sel, [1:0]gain_r_sel
	is_sensor_write8(client, 0x340D, 0x30);    // efuse clock off

	// Analog setting for Normal mode
	is_sensor_write8(client, 0x3067, 0x25);    // adc_sat[mV]=617mV
	is_sensor_write8(client, 0x307D, 0x08);    // dbr_tune_tgsl
	is_sensor_write8(client, 0x307E, 0x08);    // dbr_tune_rg
	is_sensor_write8(client, 0x307F, 0x08);    // dbr_tune_fdb
	is_sensor_write8(client, 0x3080, 0x04);    // dbr_tune_ntg
	is_sensor_write8(client, 0x3073, 0x73);    // comp1_bias, comp2_bias
	is_sensor_write8(client, 0x3074, 0x45);    // pix_bias, pix_bias_boost
	is_sensor_write8(client, 0x3075, 0xD4);    // clp_lvl
	is_sensor_write8(client, 0x3085, 0xF0);    // rdv_option; LOB_PLA enable
	is_sensor_write8(client, 0x3068, 0x55);    // ms[15:8]; x4~
	is_sensor_write8(client, 0x3069, 0x00);    // ms[7:0]; x1~x4
	is_sensor_write8(client, 0x3063, 0x08);    // cds_option[15:8]; [11]ldb nmos sw enable=1
	is_sensor_write8(client, 0x3064, 0x00);    // cds_option[7:0];
	is_sensor_write8(client, 0x3010, 0x04);

	// Row-diff improvement
	is_sensor_write8(client, 0x3247, 0x11); // [4] fadlc_blst_en
	is_sensor_write8(client, 0x3083, 0x00); // blst_en_cintr = 16
	is_sensor_write8(client, 0x3084, 0x10);

	// MIPI Clk
	is_sensor_write8(client, 0x6030, 0x18); // EXTCLK_MHz
	//is_sensor_write8(client, 0x6030, 0x0C); // EXTCLK_MHz

	//PLL
	is_sensor_write8(client, 0x0305, 0x06); //pre_pll_clk_div
	is_sensor_write8(client, 0x0306, 0x00); //pll_multiplier  //200d
	is_sensor_write8(client, 0x0307, /*140*/0xC8); //pll_multiplier
	is_sensor_write8(client, 0x3301, 0x03); // PLL_S      //*1/8
	is_sensor_write8(client, 0x3308, 0x04); //[6]dbr_gclk_auto, [5]always_pllcal_en
	is_sensor_write8(client, 0x3307, 0x11); //[3:0]div_dbr

	//BIT_RATE_MBPS (100Mbps)
	is_sensor_write8(client, 0x3351, 0x00);
	is_sensor_write8(client, 0x3352, 0x64);
	is_sensor_write8(client, 0x3353, 0x00);
	is_sensor_write8(client, 0x3354, 0x00);
	is_sensor_write8(client, 0x7339, 0x03);    // [2]dphy_en1, [1]dphy_en0, [0] dhpy_en_clk

	//Binninf mode Setting
	is_sensor_write8(client, 0x3190, 0x01);    // hv_avg_mode

	// Output size
	is_sensor_write8(client, 0x034c, 0x02);    // x_output_size MSB
	is_sensor_write8(client, 0x034d, 0x80 + cac_margin_x);    // x_output_size LSB
	is_sensor_write8(client, 0x034e, 0x01);    // y_output_size MSB
	is_sensor_write8(client, 0x034f, 0xE0 + cac_margin_y);    // y_output_size LSB
	is_sensor_write8(client, 0x306C, 0x28); // [5]aac_en
	is_sensor_write8(client, 0x0344, 0x01);    // x_addr_start MSB
	is_sensor_write8(client, 0x0345, 0x48 - cac_margin_x);    // x_addr_start LSB
	is_sensor_write8(client, 0x0348, 0x06);    // x_addr_end MSB
	is_sensor_write8(client, 0x0349, 0x47 + cac_margin_x);    // x_addr_end LSB
	is_sensor_write8(client, 0x0346, 0x00);    // y_addr_start MSB
	is_sensor_write8(client, 0x0347, 0x44 - cac_margin_y);    // y_addr_start LSB
	is_sensor_write8(client, 0x034a, 0x04);    // y_addr_end MSB
	is_sensor_write8(client, 0x034b, 0x03 + cac_margin_y);    // y_addr_end LSB
	is_sensor_write8(client, 0x0340, 0x11);    // 1fps
	is_sensor_write8(client, 0x0341, 0x94);

	/* Pattern selection signal
	 * 0 = No pattern (default) 1 = Solid color 2 = 100 % color bars 3 = Fade to grey' color bars 4 = PN9
	 */
	is_sensor_write16(client, 0x0600, 0x2);
...
static void sensor_6b2_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_6B2_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_6B2_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_6B2_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_6B2_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

static int sensor_6b2_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
/* CIS OPS */
int sensor_6b2_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	u32 setfile_index = 0;
	...
	cis->cis_data->cur_height = SENSOR_6B2_MAX_HEIGHT;
...
int sensor_6b2_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	sensor_cis_dump_registers(subdev, sensor_6b2_setfiles[0], sensor_6b2_setfile_sizes[0]);
...
static int sensor_6b2_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_6b2_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_6b2_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_6b2_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_6b2_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	if (mode > sensor_6b2_max_setfile_num) {
	...
	ret = sensor_cis_set_registers(subdev, sensor_6b2_setfiles[mode], sensor_6b2_setfile_sizes[mode]);
	if (ret < 0) {
...
int sensor_6b2_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_6b2_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_6B2_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_6B2_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_6B2_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_6B2_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_6B2_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_6B2_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_6b2_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	if (is_vender_wdr_mode_on(cis_data))
		is_sensor_write8(client, 0x0216, 0x01);
	else
		is_sensor_write8(client, 0x0216, 0x00);
...
int sensor_6b2_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_6b2_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	hold = sensor_6b2_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
int sensor_6b2_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_6b2_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_6b2_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_6b2_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_6b2_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_6b2_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_6b2_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_6b2_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_6b2_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_6b2_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_6b2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_6b2_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_6b2_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_6b2_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_6b2_cis_init,
	.cis_log_status = sensor_6b2_cis_log_status,
	.cis_group_param_hold = sensor_6b2_cis_group_param_hold,
	.cis_mode_change = sensor_6b2_cis_mode_change,
	.cis_set_size = sensor_6b2_cis_set_size,
	.cis_stream_on = sensor_6b2_cis_stream_on,
	.cis_stream_off = sensor_6b2_cis_stream_off,
	.cis_set_exposure_time = sensor_6b2_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_6b2_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_6b2_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_6b2_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_6b2_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_6b2_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_6b2_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_6b2_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_6b2_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_6b2_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_6b2_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_6b2_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_6b2_cis_get_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_6b2_setfiles = sensor_6b2_setfiles_A;
		sensor_6b2_setfile_sizes = sensor_6b2_setfile_A_sizes;
		sensor_6b2_pllinfos = sensor_6b2_pllinfos_A;
		sensor_6b2_max_setfile_num = sizeof(sensor_6b2_setfiles_A) / sizeof(sensor_6b2_setfiles_A[0]);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_6b2_setfiles = sensor_6b2_setfiles_B;
		sensor_6b2_setfile_sizes = sensor_6b2_setfile_B_sizes;
		sensor_6b2_pllinfos = sensor_6b2_pllinfos_B;
		sensor_6b2_max_setfile_num = sizeof(sensor_6b2_setfiles_B) / sizeof(sensor_6b2_setfiles_B[0]);
	} else {
...
static const struct of_device_id sensor_cis_6b2_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-6b2",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_6b2_match);

static const struct i2c_device_id sensor_cis_6b2_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_6b2_driver = {
	.probe	= cis_6b2_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_6b2_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_6b2_idt
};

static int __init sensor_cis_6b2_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_6b2_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_6b2_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_6b2_init);
