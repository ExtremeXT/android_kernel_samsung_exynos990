...
int sensor_imx316_cis_check_rev(struct is_cis *cis)
{
	...
	is_sensor_write8(client, 0x0A02,  0x7F);

	/* Turn ON OTP Read MODE */
	is_sensor_write8(client, 0x0A00,  0x01);

	/* Check status */
	is_sensor_read8(client, 0x0A01,  &rev);
#endif
...
int sensor_imx316_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 duration)
{
	...
	switch (duration) {
		case SENSOR_FPS_5:
			is_sensor_write8(client, 0x2154, 0x36);
			is_sensor_write8(client, 0x2155, 0x3C);
			break;
		case SENSOR_FPS_10:
			is_sensor_write8(client, 0x2154, 0x17);
			is_sensor_write8(client, 0x2155, 0xBC);
			break;
		case SENSOR_FPS_15:
			is_sensor_write8(client, 0x2154, 0x0D);
			is_sensor_write8(client, 0x2155, 0x92);
			break;
		case SENSOR_FPS_20:
			is_sensor_write8(client, 0x2154, 0x08);
			is_sensor_write8(client, 0x2155, 0x7E);
			break;
		case SENSOR_FPS_30:
			is_sensor_write8(client, 0x2154, 0x03);
			is_sensor_write8(client, 0x2155, 0x68);
			break;
		default:
...
int sensor_imx316_cis_set_vcsel_current(struct v4l2_subdev *subdev, u32 value)
{
	...
	is_sensor_write8(client, 0x0436, 0x00);
	is_sensor_write8(client, 0x0437, 0x00);
	is_sensor_write8(client, 0x043A, 0x02);
	is_sensor_write8(client, 0x0500, 0x02);
	is_sensor_write8(client, 0x0501, 0x08);
	is_sensor_write8(client, 0x0502, value8);

	is_sensor_write8(client, 0x0431, 0x01);
	is_sensor_write8(client, 0x0430, 0x01);
	is_sensor_write8(client, 0x0431, 0x00);
	...
	if (module->position == SENSOR_POSITION_REAR_TOF) {  /* for setting IPD_OFFSET */
		...
		is_sensor_write8(client, 0x043A, 0x02);
		is_sensor_write8(client, 0x0500, 0x02);
		is_sensor_write8(client, 0x0501, 0x0D);
		is_sensor_write8(client, 0x0502, 0xC3);
		is_sensor_write8(client, 0x0431, 0x01);
		is_sensor_write8(client, 0x0430, 0x01);
		is_sensor_write8(client, 0x0431, 0x00);
	}
...
int sensor_imx316_cis_get_vcsel_photo_diode(struct v4l2_subdev *subdev, u16 *value)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	is_sensor_write8(client, 0x0421, 0x00);
	is_sensor_write8(client, 0x0436, 0x00);
	is_sensor_write8(client, 0x0437, 0x00);
	is_sensor_write8(client, 0x043A, 0x01);
	is_sensor_write8(client, 0x0500, 0x11);
	is_sensor_write8(client, 0x0501, 0x90);
	is_sensor_write8(client, 0x0431, 0x01);
	is_sensor_write8(client, 0x0430, 0x01);
	is_sensor_write8(client, 0x0431, 0x00);

	is_sensor_read8(client, 0x058B, &value8_1); /*0x058B for APC2_CHECK_DATA[9:8]*/

	is_sensor_write8(client, 0x0423, 0xA0);
	is_sensor_write8(client, 0x0436, 0x00);
	is_sensor_write8(client, 0x0437, 0x00);
	is_sensor_write8(client, 0x043A, 0x01);
	is_sensor_write8(client, 0x0500, 0x07);
	is_sensor_write8(client, 0x0501, 0xA0);
	is_sensor_write8(client, 0x0431, 0x01);
	is_sensor_write8(client, 0x0430, 0x01);
	is_sensor_write8(client, 0x0431, 0x00);
	is_sensor_read8(client, 0x0582, &value8_0); /* 0x0582 for APC2_CHECK_DATA[7:0] */
	...
	const struct i2c_device_id *id)
{
...
#ifdef USE_CAMERA_FRONT_TOF_TX_FREQ_VARIATION
	{
		...
		for (i = 0; i < sensor_imx316_verify_sensor_mode_size; i++) {
			...
			if (is_vendor_verify_mipi_channel(sensor_imx316_tx_freq_sensor_mode[index].mipi_channel,
