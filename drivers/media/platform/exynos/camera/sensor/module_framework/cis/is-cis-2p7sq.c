...
#include "is-cis-2p7sq-setA.h"
#include "is-cis-2p7sq-setB.h"

#include "is-helper-i2c.h"

#define SENSOR_NAME "S5K2P7SQ"
/* #define DEBUG_2P7SQ_PLL */

static const struct v4l2_subdev_ops subdev_ops;

static const u32 *sensor_2p7sq_global;
static u32 sensor_2p7sq_global_size;
static const u32 **sensor_2p7sq_setfiles;
static const u32 *sensor_2p7sq_setfile_sizes;
static const struct sensor_pll_info_compact **sensor_2p7sq_pllinfos;
static u32 sensor_2p7sq_max_setfile_num;

static void sensor_2p7sq_cis_data_calculation(const struct sensor_pll_info_compact *pll_info_compact, cis_shared_data *cis_data)
{
	u32 vt_pix_clk_hz = 0;
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_2P7SQ_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_2P7SQ_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_2P7SQ_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_2P7SQ_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

static int sensor_2p7sq_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
int sensor_2p7sq_cis_init(struct v4l2_subdev *subdev)
{
	...
	if (ret < 0) {
		warn("sensor_2p7sq_check_rev is fail when cis init");
		cis->rev_flag = true;
		ret = 0;
	}
	...
	sensor_2p7sq_cis_data_calculation(sensor_2p7sq_pllinfos[setfile_index], cis->cis_data);
...
int sensor_2p7sq_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	sensor_cis_dump_registers(subdev, sensor_2p7sq_setfiles[0], sensor_2p7sq_setfile_sizes[0]);
...
static int sensor_2p7sq_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_2p7sq_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_2p7sq_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_2p7sq_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_2p7sq_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_cis_set_registers(subdev, sensor_2p7sq_global, sensor_2p7sq_global_size);
...
int sensor_2p7sq_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	if (mode > sensor_2p7sq_max_setfile_num) {
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_2p7sq_check_rev is fail");
			goto p_err;
		}
	}

	sensor_2p7sq_cis_data_calculation(sensor_2p7sq_pllinfos[mode], cis->cis_data);

	ret = sensor_cis_set_registers(subdev, sensor_2p7sq_setfiles[mode], sensor_2p7sq_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_2p7sq_set_registers fail!!");
		goto p_err;
	}
...
int sensor_2p7sq_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_2p7sq_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_2P7SQ_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_2P7SQ_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_2P7SQ_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_2P7SQ_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_2P7SQ_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_2P7SQ_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_2p7sq_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	ret = sensor_2p7sq_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
		err("group_param_hold_func failed at stream on");

#ifdef DEBUG_2P7SQ_PLL
	{
	u16 pll;
...
int sensor_2p7sq_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_2p7sq_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	hold = sensor_2p7sq_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_2p7sq_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_2p7sq_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_2p7sq_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_2p7sq_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_2p7sq_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p7sq_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_2p7sq_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_2p7sq_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_2p7sq_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p7sq_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p7sq_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_2p7sq_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_2p7sq_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p7sq_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p7sq_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_2p7sq_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_2p7sq_cis_init,
	.cis_log_status = sensor_2p7sq_cis_log_status,
	.cis_group_param_hold = sensor_2p7sq_cis_group_param_hold,
	.cis_set_global_setting = sensor_2p7sq_cis_set_global_setting,
	.cis_mode_change = sensor_2p7sq_cis_mode_change,
	.cis_set_size = sensor_2p7sq_cis_set_size,
	.cis_stream_on = sensor_2p7sq_cis_stream_on,
	.cis_stream_off = sensor_2p7sq_cis_stream_off,
	.cis_set_exposure_time = sensor_2p7sq_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_2p7sq_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_2p7sq_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_2p7sq_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_2p7sq_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_2p7sq_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_2p7sq_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_2p7sq_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_2p7sq_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_2p7sq_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_2p7sq_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_2p7sq_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_2p7sq_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_2p7sq_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_2p7sq_cis_get_max_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_S5K2P7SQ);
	if (!sensor_peri) {
...
static const struct of_device_id sensor_cis_2p7sq_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-2p7sq",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_2p7sq_match);

static const struct i2c_device_id sensor_cis_2p7sq_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_2p7sq_driver = {
	.probe	= cis_2p7sq_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_2p7sq_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_2p7sq_idt
};

static int __init sensor_cis_2p7sq_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_2p7sq_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_2p7sq_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_2p7sq_init);
