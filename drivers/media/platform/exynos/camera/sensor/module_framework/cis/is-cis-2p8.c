...
#include "is-dt.h"
#include "is-cis-2p8.h"
#include "is-cis-2p8-setA.h"
#include "is-cis-2p8-setB.h"

#include "is-helper-i2c.h"

#define SENSOR_NAME "S5K2P8"
/* #define DEBUG_2P8_PLL */

static const struct v4l2_subdev_ops subdev_ops;

static const u32 **sensor_2p8_setfiles;
static const u32 *sensor_2p8_setfile_sizes;
static const struct sensor_pll_info **sensor_2p8_pllinfos;
static u32 sensor_2p8_max_setfile_num;

static void sensor_2p8_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_2P8_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_2P8_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_2P8_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_2P8_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

static int sensor_2p8_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
/* CIS OPS */
int sensor_2p8_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	u32 setfile_index = 0;
	...
	cis->cis_data->cur_width = SENSOR_2P8_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_2P8_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_2p8_cis_data_calculation(sensor_2p8_pllinfos[setfile_index], cis->cis_data);
...
int sensor_2p8_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	sensor_cis_dump_registers(subdev, sensor_2p8_setfiles[0], sensor_2p8_setfile_sizes[0]);
...
#else
static inline int sensor_2p8_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_2p8_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_2p8_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_2p8_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_2p8_check_rev is fail");
			goto p_err;
		}
	}
...
int sensor_2p8_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_2p8_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
...
int sensor_2p8_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p8_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_2p8_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p8_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p8_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p8_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_2p8_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_2p8_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_2p8_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_2p8_cis_init,
	.cis_log_status = sensor_2p8_cis_log_status,
	.cis_group_param_hold = sensor_2p8_cis_group_param_hold,
	.cis_mode_change = sensor_2p8_cis_mode_change,
	.cis_set_size = sensor_2p8_cis_set_size,
	.cis_stream_on = sensor_2p8_cis_stream_on,
	.cis_stream_off = sensor_2p8_cis_stream_off,
	.cis_set_exposure_time = sensor_2p8_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_2p8_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_2p8_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_2p8_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_2p8_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_2p8_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_2p8_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_2p8_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_2p8_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_2p8_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_2p8_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_2p8_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_2p8_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_2p8_cis_get_min_digital_gain,
...
static const struct of_device_id sensor_cis_2p8_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-2p8",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_2p8_match);

static const struct i2c_device_id sensor_cis_2p8_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_2p8_driver = {
	.probe	= cis_2p8_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_2p8_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_2p8_idt
};

static int __init sensor_cis_2p8_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_2p8_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_2p8_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_2p8_init);
