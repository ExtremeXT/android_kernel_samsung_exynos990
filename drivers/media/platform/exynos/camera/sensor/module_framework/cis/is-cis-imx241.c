...
#include "is-cis-imx241-setA.h"
#include "is-cis-imx241-setB.h"

#include "is-helper-i2c.h"

#define SENSOR_NAME "IMX241"
#define DEBUG_IMX241_PLL

static const struct v4l2_subdev_ops subdev_ops;

static const u32 *sensor_imx241_init_setfile;
static u32 sensor_imx241_init_setfile_size;
static const u32 *sensor_imx241_init_setfile_global;
static u32 sensor_imx241_init_setfile_global_size;
static const u32 *sensor_imx241_init_setfile_Image;
static u32 sensor_imx241_init_setfile_Image_size;


static const u32 **sensor_imx241_setfiles;
static const u32 *sensor_imx241_setfile_sizes;
static const struct sensor_pll_info **sensor_imx241_pllinfos;
static u32 sensor_imx241_max_setfile_num;

static void sensor_imx241_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_IMX241_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_IMX241_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_IMX241_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_IMX241_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

u32 sensor_imx241_cis_calc_again_code(u32 permile)
{
	if (permile > 0)
...
u32 sensor_imx241_cis_calc_again_permile(u32 code)
{
	if (code < 256)
...
/* CIS OPS */
int sensor_imx241_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	u32 setfile_index = 0;
	...
	cis->cis_data->cur_height = SENSOR_IMX241_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	/* Read Lot Chip ID from OTP in IMX241 */
	client = cis->client;
	if (unlikely(!client)) {
		err("client is NULL");
		ret = -ENODEV;
	...
	if (ret < 0) {
		err("sensor_imx241_set_registers - init fail!!");
		goto p_err;
	}
...
int sensor_imx241_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client = NULL;
	...
	sensor_cis_dump_registers(subdev, sensor_imx241_setfiles[0], sensor_imx241_setfile_sizes[0]);
...
static int sensor_imx241_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_imx241_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_imx241_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_imx241_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	struct i2c_client *client = NULL;
	...
	sensor_imx241_cis_data_calculation(sensor_imx241_pllinfos[mode], cis->cis_data);

	ret = sensor_cis_set_registers(subdev, sensor_imx241_init_setfile_global,
			sensor_imx241_init_setfile_global_size);
	if (ret < 0) {
		err("sensor_imx241_set_registers - global fail!!");
		goto p_err;
	}

	ret = sensor_cis_set_registers(subdev, sensor_imx241_init_setfile_Image,
			sensor_imx241_init_setfile_Image_size);
	if (ret < 0) {
		err("sensor_imx241_set_registers - Image fail!!");
		goto p_err;
	}

	ret = sensor_cis_set_registers(subdev, sensor_imx241_setfiles[mode], sensor_imx241_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_imx241_set_registers fail!!");
		goto p_err;
	}
...
int sensor_imx241_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	...
	ret = sensor_imx241_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
		err("[%s] sensor_imx241_cis_group_param_hold_func fail\n", __func__);

#ifdef DEBUG_IMX241_PLL
	{
	u16 pll;
	ret = is_sensor_read16(client, 0x0301, &pll);
	dbg_sensor(1, "______ vt_pix_clk_div(%x)\n", pll);
	ret = is_sensor_read16(client, 0x0303, &pll);
	dbg_sensor(1, "______ vt_sys_clk_div(%x)\n", pll);
	ret = is_sensor_read16(client, 0x0305, &pll);
	dbg_sensor(1, "______ pre_pll_clk_div(%x)\n", pll);
	ret = is_sensor_read16(client, 0x0307, &pll);
	dbg_sensor(1, "______ pll_multiplier(%x)\n", pll);
	ret = is_sensor_read16(client, 0x0309, &pll);
	dbg_sensor(1, "______ op_pix_clk_div(%x)\n", pll);
	ret = is_sensor_read16(client, 0x030b, &pll);
	dbg_sensor(1, "______ op_sys_clk_div(%x)\n", pll);

	ret = is_sensor_read16(client, 0x030d, &pll);
	dbg_sensor(1, "______ secnd_pre_pll_clk_div(%x)\n", pll);
	ret = is_sensor_read16(client, 0x030f, &pll);
...
int sensor_imx241_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_imx241_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	hold = sensor_imx241_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_imx241_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_imx241_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_imx241_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_imx241_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_imx241_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx241_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_imx241_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_imx241_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	again_code = sensor_imx241_cis_calc_again_code(input_again);
	...
	again_permile = sensor_imx241_cis_calc_again_permile(again_code);
...
int sensor_imx241_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	analog_gain = (u16)sensor_imx241_cis_calc_again_code(again->val);
...
int sensor_imx241_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	*again = sensor_imx241_cis_calc_again_permile(analog_gain);
...
int sensor_imx241_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = 0; /* imx241 again range 0(x1) ~ 480(x16) */

	cis_data->min_analog_gain[0] = read_value;

	cis_data->min_analog_gain[1] = sensor_imx241_cis_calc_again_permile(read_value);
...
int sensor_imx241_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = 224; /* imx241 again range 0(x1) ~ 480(x16) */

	cis_data->max_analog_gain[0] = read_value;

	cis_data->max_analog_gain[1] = sensor_imx241_cis_calc_again_permile(read_value);
...
int sensor_imx241_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx241_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_imx241_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	/* IMX241 cannot read min/max digital gain */
...
int sensor_imx241_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_imx241_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev, u32 expo, u32 *again, u32 *dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_imx241_cis_init,
	.cis_log_status = sensor_imx241_cis_log_status,
	.cis_group_param_hold = sensor_imx241_cis_group_param_hold,
	.cis_mode_change = sensor_imx241_cis_mode_change,
	.cis_stream_on = sensor_imx241_cis_stream_on,
	.cis_stream_off = sensor_imx241_cis_stream_off,
	.cis_set_exposure_time = sensor_imx241_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_imx241_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_imx241_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_imx241_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_imx241_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_imx241_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_imx241_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_imx241_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_imx241_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_imx241_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_imx241_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_imx241_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_imx241_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_imx241_cis_get_min_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_imx241_init_setfile = sensor_imx241_setfile_A_initial;
		sensor_imx241_init_setfile_size = sensor_imx241_setfile_A_initial_size;
		sensor_imx241_init_setfile_global = sensor_imx241_setfile_A_global;
		sensor_imx241_init_setfile_global_size = sensor_imx241_setfile_A_global_size;
		sensor_imx241_init_setfile_Image = sensor_imx241_setfile_A_Image;
		sensor_imx241_init_setfile_Image_size = sensor_imx241_setfile_A_Image_size;
		sensor_imx241_setfiles = sensor_imx241_setfiles_A;
		sensor_imx241_setfile_sizes = sensor_imx241_setfile_A_sizes;
		sensor_imx241_pllinfos = sensor_imx241_pllinfos_A;
		sensor_imx241_max_setfile_num = sizeof(sensor_imx241_setfiles_A) / sizeof(sensor_imx241_setfiles_A[0]);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_imx241_init_setfile = sensor_imx241_setfile_B_initial;
		sensor_imx241_init_setfile_size = sensor_imx241_setfile_B_initial_size;
		sensor_imx241_init_setfile_global = sensor_imx241_setfile_B_global;
		sensor_imx241_init_setfile_global_size = sensor_imx241_setfile_B_global_size;
		sensor_imx241_init_setfile_Image = sensor_imx241_setfile_B_Image;
		sensor_imx241_init_setfile_Image_size = sensor_imx241_setfile_B_Image_size;
		sensor_imx241_setfiles = sensor_imx241_setfiles_B;
		sensor_imx241_setfile_sizes = sensor_imx241_setfile_B_sizes;
		sensor_imx241_pllinfos = sensor_imx241_pllinfos_B;
		sensor_imx241_max_setfile_num = sizeof(sensor_imx241_setfiles_B) / sizeof(sensor_imx241_setfiles_B[0]);
	} else {
