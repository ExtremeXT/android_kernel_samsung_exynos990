...
#include "is-dt.h"
#include "is-cis-2l4.h"
#ifndef USE_FLIP_WIDE_SENSOR
#include "is-cis-2l4-setA.h"
#else
#include "is-cis-2l4-setB.h"
#endif
	...
	if (embedded_buf[frame_id_idx-1] == SENSOR_2L4_VALID_TAG) {
		*frame_id = embedded_buf[frame_id_idx];
		...
		if (embedded_buf[frame_count_idx] == 254) {
			...
			case 0:
				is_sensor_write8(cis->client, 0x0A52, 0x01);
				record_status++;
				break;
			case 1:
				is_sensor_write8(cis->client, 0x0A54, 0x01);
				record_status++;
				break;
			case 5:
				is_sensor_write8(cis->client, 0x0A53, 0x01);
				record_status++;
				break;
			case 10:
				record_status = 0;
...
int sensor_2l4_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_read16(client, 0xEA2A, &data16);
	if (unlikely(!ret)) pr_info("0xEA2A(0x%x)\n", data16);
	else goto i2c_err;
	ret = is_sensor_read16(client, 0xEA2C, &data16);
	...
	ret = is_sensor_read16(client, 0x0C80, &data16);
...
int sensor_2l4_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	is_sensor_write16(cis->client, 0x602A, 0x1090);
...
int sensor_2l4_cis_set_lownoise_mode_change(struct v4l2_subdev *subdev)
{
	...
	is_sensor_read8(cis->client, 0x0005, &sensor_2l4_ln_mode_frame_count);
...
int sensor_2l4_cis_retention_crc_check(struct v4l2_subdev *subdev)
{
	...
	is_sensor_read8(cis->client, 0x100E, &crc_check);
...
int sensor_2l4_cis_stream_on(struct v4l2_subdev *subdev)
{
...
#ifdef DEBUG_2L4_PLL
	{
		...
		is_sensor_read16(client, 0x0314, &pll);
		dbg_sensor(1, "______ DRAM_pre_pll_clk_div(0x%x)\n", pll);
		is_sensor_read16(client, 0x0316, &pll);
		dbg_sensor(1, "______ DRAM_pll_multiplier(0x%x)\n", pll);
		is_sensor_read16(client, 0x0318, &pll);
...
int sensor_2l4_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* CIT shifter */
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = is_sensor_write16(client, 0x0702, coarse_integration_time_shifter);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l4_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = is_sensor_write8(client, 0x0701, frame_length_lines_shifter);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l4_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	...
	if (lte_mode->sen_strm_off_on_enable) {
		...
		cit_lshift_val = 0;
		ret |= is_sensor_write16(cis->client, 0xFCFC, 0x4000);
		ret |= is_sensor_write8(cis->client, 0x0701, cit_lshift_val);
...
#ifdef USE_FLIP_WIDE_SENSOR
			info("%s 180 degrees rotated\n", __func__);
			is_sensor_write16(cis->client, 0x0100, 0x0100);
#else
...
int sensor_2l4_cis_set_super_slow_motion_roi(struct v4l2_subdev *subdev, struct v4l2_rect *ssm_roi)
{
	...
	ret |= is_sensor_write16(cis->client, 0x0A64, ssm_roi->left);
	ret |= is_sensor_write16(cis->client, 0x0A66, ssm_roi->width);
	ret |= is_sensor_write16(cis->client, 0x0A68, ssm_roi->top);
	ret |= is_sensor_write16(cis->client, 0x0A6A, ssm_roi->height);
	if (ret < 0) {
...
int sensor_2l4_cis_get_super_slow_motion_gmc(struct v4l2_subdev *subdev, u32 *gmc)
{
	...
	ret |= is_sensor_write16(cis->client, 0x602C, 0x2001);
	ret |= is_sensor_write16(cis->client, 0x602E, 0x0E34);
...
int sensor_2l4_cis_get_super_slow_motion_frame_id(struct v4l2_subdev *subdev, u32 *frameid)
{
	...
	ret |= is_sensor_read8(cis->client, 0x1A0C, &frame_id);
	if (ret < 0) {
...
static struct is_cis_ops cis_ops_2l4 = {
	...
	const struct i2c_device_id *id)
{
	...
	for (i = 0; i < sensor_2l4_verify_sensor_mode_size; i++) {
		...
		if (is_vendor_verify_mipi_channel(sensor_2l4_mipi_sensor_mode[index].mipi_channel,
