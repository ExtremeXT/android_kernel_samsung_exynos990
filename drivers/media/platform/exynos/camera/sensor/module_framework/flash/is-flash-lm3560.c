...
static int __init flash_lm3560_probe(struct device *dev, struct i2c_client *client)
{
	int ret = 0;
	struct is_core *core;
...
static int flash_lm3560_i2c_probe(struct i2c_client *client,
...
static int __init flash_lm3560_platform_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct device *dev;
...
static const struct of_device_id exynos_is_sensor_flash_lm3560_match[] = {
	{
		.compatible = "samsung,sensor-flash-lm3560",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_flash_lm3560_match);
...
static struct i2c_driver sensor_flash_lm3560_i2c_driver = {
	...
	.driver = {
		...
		.owner	= THIS_MODULE,
		.of_match_table = exynos_is_sensor_flash_lm3560_match
		.suppress_bind_attrs = true,
	}
	.id_table = flash_lm3560_i2c_idt,
};

static int __init sensor_flash_lm3560_i2c_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_flash_lm3560_i2c_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_flash_lm3560_i2c_driver.driver.name, ret);

	return ret;
};
late_initcall_sync(sensor_flash_lm3560_i2c_init);
...
static int __init sensor_flash_lm3560_platform_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_flash_lm3560_platform_driver,
				flash_lm3560_platform_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_flash_lm3560_platform_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_flash_lm3560_platform_init);
