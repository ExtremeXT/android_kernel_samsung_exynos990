...
static int flash_torch_set_init(struct i2c_client *client)
{
	...
	/* Auto BST EN */
	ret = is_flash_updata_reg(client, REG_FLED_CTRL0, AUTO_BST_EN_ON, AUTO_BST_EN_MASK);
	if (ret < 0)
		warn("%s: Failed to auto BST EN setting\n", __func__);

	/* Time out Max Setting */
	ret = is_flash_updata_reg(client, REG_FLED_CTRL4, TIMEOUT_MAX, TIMEOUT_MASK);
	if (ret < 0)
		warn("%s: Failed to time out max setting\n", __func__);

	/* Torch Timer Max mode setting */
	ret = is_flash_updata_reg(client, REG_FLED_CTRL4, TORCH_TMR_MODE_ON, TORCH_TMR_MODE_MASK);
	if (ret < 0)
		warn("%s: Failed to torch time out max setting\n", __func__);

	/* Torch off */
	ret = is_flash_updata_reg(client, REG_FLED_CTRL0, TORCH_ENABLE_OFF, TORCH_ENABLE_MASK);
	if (ret < 0)
	...
	unsigned long action, void *data)
{
	...
	flash = container_of(nb, struct is_flash, flash_noti_ta);
	FIMC_BUG(!flash);
	...
	case MUIC_NOTIFY_CMD_LOGICALLY_DETACH:
		...
		ret = is_flash_updata_reg(flash->client, REG_FLED_CTRL0,
		...
		is_flash_data_read(flash->client, REG_FLED_CTRL0, &reg);
		if (reg < 0) {
...
static int sensor_i2c_flash_control(struct v4l2_subdev *subdev, enum flash_mode mode, u32 intensity)
{
	...
	if (mode == CAM2_FLASH_MODE_OFF) {
		...
		ret = is_flash_updata_reg(flash->client, REG_FLED_CTRL0, TORCH_ENABLE_OFF, TORCH_ENABLE_MASK);
		if (ret)
			err("torch flash off fail");
		...
		/* Brightness set */
		ret = is_flash_updata_reg(flash->client, REG_FLED_CTRL1, (u8)(intensity & 0xff), CURRENT_MASK);
		/* Torch on */
		ret = is_flash_updata_reg(flash->client, REG_FLED_CTRL0, TORCH_ENABLE_ON, TORCH_ENABLE_MASK);
		if (ret)
...
static struct attribute *is_flash_entries[] = {
	&dev_attr_flash_torch_control.attr,
	NULL,
};

static struct attribute_group is_flash_attr_group = {
	.name	= "torch",
	.attrs	= is_flash_entries,
};
...
static int __init flash_probe(struct device *dev, struct i2c_client *client)
{
	...
	ret = sysfs_create_group(&core->pdev->dev.kobj, &is_flash_attr_group);
	if (ret < 0)
...
static const struct of_device_id exynos_is_sensor_flash_i2c_match[] = {
	{
		.compatible = "samsung,sensor-flash-i2c",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_flash_i2c_match);
...
static struct i2c_driver sensor_flash_i2c_driver = {
	...
	.driver = {
		...
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_flash_i2c_match
