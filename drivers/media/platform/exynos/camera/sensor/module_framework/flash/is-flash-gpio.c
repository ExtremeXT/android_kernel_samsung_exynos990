...
static const struct of_device_id exynos_is_sensor_flash_gpio_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, exynos_is_sensor_flash_gpio_match);
...
static struct platform_driver sensor_flash_gpio_platform_driver = {
	.driver = {
		...
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_flash_gpio_match,
	}
};

static int __init is_sensor_flash_gpio_init(void)
{
	int ret;
...
late_initcall_sync(is_sensor_flash_gpio_init);
