...
static int sensor_dw9714_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = is_sensor_data_write16(client, val_high, val_low);
...
int sensor_dw9714_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	/* STEP 1 : Protect Off */
	ret = is_sensor_data_write16(client, 0xEC, 0xA3);
	if (ret < 0)
		goto p_err;

	/* SETP 2 : T_SRC setting (default is 0x00) */
	ret = is_sensor_data_write16(client, 0xF2, (0x9 << 3));
	if (ret <0)
		goto p_err;

	/* STEP 3 : Protection On */
	ret = is_sensor_data_write16(client, 0xDC, 0x51);
	if (ret <0)
		goto p_err;
...
int sensor_dw9714_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	 ...
	 */
	ret = is_sensor_data_read16(client, &val);
