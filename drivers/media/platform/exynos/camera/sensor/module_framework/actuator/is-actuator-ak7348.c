...
static int sensor_ak7348_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = is_sensor_addr8_write8(client, AK7348_REG_POS_HIGH, val_high);
	if (ret < 0)
		goto p_err;
	ret = is_sensor_addr8_write8(client, AK7348_REG_POS_LOW, val_low);
...
int sensor_ak7348_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	// CheckProductID
	ret = is_sensor_addr8_read8(client, 0x03, &product_id);
	if (ret < 0)
		goto p_err;
