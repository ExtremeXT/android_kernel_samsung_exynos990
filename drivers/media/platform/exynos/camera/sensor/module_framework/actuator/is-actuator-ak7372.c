...
static int sensor_ak7372_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = is_sensor_addr8_write8(client, AK7372_REG_POS_HIGH, val_high);

	if (ret < 0)
		goto p_err;
	ret = is_sensor_addr8_write8(client, AK7372_REG_POS_LOW, val_low);
	if (ret < 0)
		goto p_err;
