...
int sensor_zc533_init(struct i2c_client *client, struct is_caldata_list_zc533 *cal_data)
{
	int ret = 0;
	u8 i2c_data[2];
...
static int sensor_zc533_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = is_sensor_addr_data_write16(client, REG_POSITION_MSB, val_high, val_low);
	if (ret < 0)
		goto p_err;
...
int sensor_zc533_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	int ret = 0;
	struct is_actuator *actuator;
	struct is_caldata_list_zc533 *cal_data = NULL;
	...
	cal_data = (struct is_caldata_list_zc533 *)(cal_addr);
...
int sensor_zc533_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	...
	ret = is_sensor_addr8_read8(client, REG_POSITION_MSB, &val);
	data = (val & 0x0300);
	ret = is_sensor_addr8_read8(client, REG_POSITION_LSB, &val);
	data |= (val & 0x00ff);
#endif
