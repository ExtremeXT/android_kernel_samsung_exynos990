...
static int sensor_zc569_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = is_sensor_addr8_write8(client, ZC569_REG_POS_HIGH, val_high);
	if (ret < 0)
		goto p_err;
	ret = is_sensor_addr8_write8(client, ZC569_REG_POS_LOW, val_low);
	if (ret < 0)
		goto p_err;
...
int sensor_zc569_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	if (actuator->vendor_use_sleep_mode) {
		/* Go sleep mode */
		ret = is_sensor_addr8_write8(client, 0x02, 32);
	} else {
		...
		/* Go active mode */
		ret = is_sensor_addr8_write8(client, 0x02, 0);
		if (ret < 0)
			goto p_err;
	}
...
static const struct of_device_id exynos_is_zc569_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-actuator-zc569",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_zc569_match);
...
static struct i2c_driver actuator_zc569_driver = {
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_is_zc569_match
	},
