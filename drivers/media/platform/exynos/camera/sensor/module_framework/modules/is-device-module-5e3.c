...
static struct is_sensor_cfg config_module_5e3[] = {
	/* 2560x1920@30fps */
	IS_SENSOR_CFG(2576, 1932, 30, 19, 0, CSI_DATA_LANES_2),
	/* 1280x960@30fps */
	IS_SENSOR_CFG(1280, 960, 30, 19, 1, CSI_DATA_LANES_2),
	/* 1280x960@15fps */
	IS_SENSOR_CFG(1280, 960, 15, 19, 2, CSI_DATA_LANES_2),
	/* 640x480@116fps */
	IS_SENSOR_CFG(640, 480, 116, 19, 3, CSI_DATA_LANES_2),
};

static struct is_vci vci_module_5e3[] = {
	{
...
static int __init sensor_module_5e3_probe(struct platform_device *pdev)
{
	...
	is_module_parse_dt(dev, sensor_module_5e3_power_setpin);
	...
	module->bitwidth = 10;
	module->vcis = ARRAY_SIZE(vci_module_5e3);
	module->vci = vci_module_5e3;
	module->sensor_maker = "SLSI";
	module->sensor_name = "S5K5E3";
	module->setfile_name = "setfile_5e3.bin";
	module->cfgs = ARRAY_SIZE(config_module_5e3);
	module->cfg = config_module_5e3;
	module->ops = NULL;
	/* Sensor peri */
...
static const struct of_device_id exynos_is_sensor_module_5e3_match[] = {
	{
		.compatible = "samsung,sensor-module-5e3",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_5e3_match);

static struct platform_driver sensor_module_5e3_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-5E3",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_5e3_match,
	}
};

static int __init is_sensor_module_5e3_init(void)
{
	int ret;
...
late_initcall(is_sensor_module_5e3_init);
