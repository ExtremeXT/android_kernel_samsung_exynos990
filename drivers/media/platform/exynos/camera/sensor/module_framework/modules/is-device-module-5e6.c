...
#include "is-device-module-5e6.h"
...
static struct is_sensor_cfg config_5e6[] = {
			/* width, height, fps, settle, mode, lane, speed, interleave, pd_mode */
	IS_SENSOR_CFG(1920, 1920, 30, 0, 0, CSI_DATA_LANES_2, 1400, CSI_MODE_VC_DT, PD_NONE,
...
static void sensor_5e6_vsync_work(struct work_struct *data)
{
	struct is_module_enum *module
		= container_of(data, struct is_module_enum, vsync_work);
...
{
	struct is_module_enum *module
		= container_of(timer, struct is_module_enum, vsync_timer);
...
static int sensor_5e6_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	ulong i;
	struct is_module_enum *module;
	struct is_module_5e6 *module_5e6;
	...
	for (i = 0; i < ARRAY_SIZE(setfile_vision_5e6); i++) {
		ret = is_sensor_write8(client, setfile_vision_5e6[i][0],
				(u8)setfile_vision_5e6[i][1]);
		if (ret) {
#ifdef USE_CAMERA_HW_BIG_DATA
			is_sec_get_hw_param(&iris_hw_param, SENSOR_POSITION_SECURE);
			if (iris_hw_param)
	...
	if (!is_iris_ver_read) {
		/* 1. read page_select */
		/* make initial state */
		ret = is_sensor_write8(client, 0x0A00, 0x04);
		if (ret) {
		...
		/* set the PAGE of OTP */
		ret = is_sensor_write8(client, 0x0A02, 0x04);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		ret = is_sensor_read8(client, 0x0A12, &page_select);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		/* set the PAGE of OTP */
		ret = is_sensor_write8(client, 0x0A02, page_select);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		ret = is_sensor_read8(client, 0x0A04, &sensor_otp);
		if (ret == 0) {
			if (sensor_otp == 0x01)
		...
		/* read year */
		ret = is_sensor_read8(client, 0x0A05, &is_iris_mtf_read_data[0]);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		/* read month */
		ret = is_sensor_read8(client, 0x0A06, &is_iris_mtf_read_data[1]);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		/* read company */
		ret = is_sensor_read8(client, 0x0A07, &is_iris_mtf_read_data[2]);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		ret = is_sensor_read8(client, SENSOR_REG_VERSION, &sensor_ver);
		if (ret == 0) {
...
int sensor_5e6_stream_on(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write8(client, SENSOR_REG_MIPI_CLK_CTRL, 0x01);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}

	ret = is_sensor_write8(client, SENSOR_REG_STREAM_ON_0, 0x04);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}

	ret = is_sensor_write8(client, SENSOR_REG_STREAM_ON_1, 0x01);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}

	ret = is_sensor_write8(client, SENSOR_REG_STREAM_ON_2, 0x00);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}
	...
	ret = is_sensor_write8(client, SENSOR_REG_STREAM_ON_0, 0x00);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}
	...
	mdelay(1);
	ret = is_sensor_write8(client, SENSOR_REG_MIPI_CLK_CTRL, 0x07);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}

	ret = is_sensor_write8(client, SENSOR_REG_MIPI_CLK_CTRL, 0x06);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}
	...
	ret = is_sensor_write8(client, SENSOR_REG_MIPI_CLK_CTRL, 0x0F);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}
...
int sensor_5e6_stream_off(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write8(client, SENSOR_REG_STREAM_OFF, 0x00);
	if (ret < 0) {
		err("is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}
...
int sensor_5e6_s_duration(struct v4l2_subdev *subdev, u64 duration)
{
	...
	if (duration == SENSOR_FPS_30) {
		for (i = 0; i < ARRAY_SIZE(setfile_vision_5e6_fps_30); i++) {
			ret = is_sensor_write8(client, setfile_vision_5e6_fps_30[i][0],
		...
		for (i = 0; i < ARRAY_SIZE(setfile_vision_5e6_fps_15); i++) {
			ret = is_sensor_write8(client, setfile_vision_5e6_fps_15[i][0],
...
int sensor_5e6_s_exposure(struct v4l2_subdev *subdev, u64 exposure)
{
	...
	is_sensor_write8(client, SENSOR_REG_EXPOSURE_TIME, value);
...
int sensor_5e6_s_again(struct v4l2_subdev *subdev, u64 gain)
{
	...
	value = calc_value >> 8;
	is_sensor_write8(client, SENSOR_REG_GAIN_ADJUST_0, value);
	value = calc_value & 0xFF;
	is_sensor_write8(client, SENSOR_REG_GAIN_ADJUST_1, value);
...
int sensor_5e6_s_shutterspeed(struct v4l2_subdev *subdev, u64 shutterspeed)
{
	...
	value = calc_value >> 8;
	is_sensor_write8(client, SENSOR_REG_SHUTTERSPEED_ADJUST_0, value);
	value = calc_value & 0xFF;
	is_sensor_write8(client, SENSOR_REG_SHUTTERSPEED_ADJUST_1, value);
...
struct is_sensor_ops module_5e6_ops = {
	...
	const struct i2c_device_id *id)
{
	...
	if (is_dev == NULL) {
		warn("is_dev is not yet probed(5e6)");
...
#ifdef CONFIG_OF
	is_module_parse_dt(dev, sensor_5e6_power_setpin);
#endif
...
#if 0
	module->private_data = kzalloc(sizeof(struct is_module_5e6), GFP_KERNEL);
...
/* Sensor peri */
	...
	(((struct is_device_sensor_peri*)module->private_data)->cis).cis_data = kzalloc(sizeof(cis_shared_data), GFP_KERNEL);
...
#ifdef CONFIG_OF
static const struct of_device_id exynos_is_sensor_module_5e6_match[] = {
	{
...
static struct i2c_driver sensor_module_5e6_driver = {
	...
	.driver = {
		...
		.owner  = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = exynos_is_sensor_module_5e6_match,
#endif
...
static int __init is_sensor_module_5e6_init(void)
{
	int ret;
...
late_initcall_sync(is_sensor_module_5e6_init);
