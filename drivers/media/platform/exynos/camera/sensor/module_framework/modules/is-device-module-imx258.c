...
static struct is_sensor_cfg config_module_imx258[] = {
	/* 4144x3106@30fps */
	IS_SENSOR_CFG(4144, 3106, 30, 24, 0, CSI_DATA_LANES_4),
	/* 4144x2332@30fps */
	IS_SENSOR_CFG(4144, 2332, 30, 24, 1, CSI_DATA_LANES_4),
	/* 2072x1552@15fps */
	IS_SENSOR_CFG(2072, 1552, 15, 24, 2, CSI_DATA_LANES_4),
	/* 2072x1552@30fps */
	IS_SENSOR_CFG(2072, 1552, 30, 24, 3, CSI_DATA_LANES_4),
	/* 2072x1162@30fps */
	IS_SENSOR_CFG(2072, 1162, 30, 24, 4, CSI_DATA_LANES_4),
	/* 1008x754@114fps */
	IS_SENSOR_CFG(1008, 754, 114, 24, 5, CSI_DATA_LANES_4),
	/* 1008x568@120fps */
	IS_SENSOR_CFG(1008, 568, 120, 24, 6, CSI_DATA_LANES_4),
};

static struct is_vci vci_module_imx258[] = {
	{
	...
	struct exynos_platform_is_module *pdata)
{
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "VDD28_CAM_AF", PIN_REGULATOR, 1, 9000);
...
static int __init sensor_module_imx258_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
	...
	is_module_parse_dt(dev, sensor_module_imx258_power_setpin);
...
static const struct of_device_id exynos_is_sensor_module_imx258_match[] = {
	{
		.compatible = "samsung,sensor-module-imx258",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_imx258_match);

static struct platform_driver sensor_module_imx258_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-IMX258",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_imx258_match,
	}
};
static int __init is_sensor_module_imx258_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_imx258_driver,
				sensor_module_imx258_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_imx258_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_imx258_init);
