...
static struct is_sensor_cfg config_virtual[] = {
	/* width, height, fps, settle, mode, lane, speed, interleave, pd_mode */
	/* 5344x3008@30fps */
	IS_SENSOR_CFG(4032, 3024, 30, 0, 0, CSI_DATA_LANES_4, 1500, 0, PD_NONE,
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0)),
	/* 3264x2448@30fps */
	IS_SENSOR_CFG(3264, 2448, 30, 0, 0, CSI_DATA_LANES_2, 1500, 0, PD_NONE,
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0)),
	IS_SENSOR_CFG(960, 800, 30, 0, 0, CSI_DATA_LANES_4, 1500, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW8, 960, 800), VC_OUT(HW_FORMAT_RAW8, VC_NOTHING, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0),
		VC_IN(0, 0, 0, 0), VC_OUT(0, 0, 0, 0)),
	IS_SENSOR_CFG(1920, 800, 30, 0, 0, CSI_DATA_LANES_4, 1500, CSI_MODE_VC_DT, PD_NONE,
...
struct is_sensor_ops module_virtual_ops = {
...
#ifdef CONFIG_OF
	is_module_parse_dt(dev, sensor_virtual_power_setpin);
#endif
...
static const struct of_device_id exynos_is_sensor_module_virtual_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-sensor-virtual",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_virtual_match);

static struct platform_driver sensor_module_virtual_driver = {
	.driver = {
		.name   = SENSOR_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_virtual_match,
	}
};

static int __init is_sensor_module_virtual_init(void)
{
	int ret;
...
late_initcall(is_sensor_module_virtual_init);
