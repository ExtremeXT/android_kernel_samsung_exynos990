...
static struct is_sensor_cfg config_module_2p7sx[] = {
	/* 4608x3456@30fps */
	IS_SENSOR_CFG_EXT(4608, 3456, 30, 0, 0, CSI_DATA_LANES_4, 1495,
			SET_VC(VC_TAIL_MODE_PDAF, 144, 864), 0, 0),
	/* 2320X1728@30fps */
	IS_SENSOR_CFG_EXT(2304, 1728, 30, 0, 1, CSI_DATA_LANES_4, 1300,
			SET_VC(VC_TAIL_MODE_PDAF, 144, 864), 0, 0),
	/* 2320X1296@30fps */
	IS_SENSOR_CFG_EXT(2304, 1296, 120, 0, 2, CSI_DATA_LANES_4, 1300, 0, 0, 0),
};

static struct is_vci vci_module_2p7sx[] = {
	{
	...
	struct exynos_platform_is_module *pdata)
{
	...
	gpio_vdd28_drv_en = of_get_named_gpio(dnode, "gpio_vdd28_drv_en", 0);
	if (gpio_is_valid(gpio_vdd28_drv_en)) {
		gpio_request_one(gpio_vdd28_drv_en, GPIOF_OUT_INIT_LOW, "CAM_GPIO_VDD28_EN");
		gpio_free(gpio_vdd28_drv_en);
	} else {
		dev_err(dev, "failed to get PIN_POWER_EN\n");
		return -EINVAL;
	}

	gpio_avdd28_en = of_get_named_gpio(dnode, "gpio_avdd28_mcam_en", 0);
	if (gpio_is_valid(gpio_avdd28_en)) {
		gpio_request_one(gpio_avdd28_en, GPIOF_OUT_INIT_LOW, "CAM_GPIO_AVDD28_MCAM_EN");
		gpio_free(gpio_avdd28_en);
	} else {
		dev_err(dev, "failed to get PIN_POWER_EN\n");
		return -EINVAL;
	}
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_vdd28_drv_en, "vdd28_drv_en", PIN_OUTPUT, 1, 0);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_avdd28_en, "avdd28_mcam_en", PIN_OUTPUT, 1, 2000);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "VLDO34_PMIC", PIN_REGULATOR, 1, 0);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_vdd28_drv_en, "vdd28_drv_en", PIN_INPUT, 0, 0);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_avdd28_en, "avdd28_mcam_en", PIN_OUTPUT, 0, 0);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_none, "VLDO34_PMIC", PIN_REGULATOR, 0, 0);
...
static int __init sensor_module_2p7sx_probe(struct platform_device *pdev)
{
	...
	is_module_parse_dt(dev, sensor_module_2p7sx_power_setpin);
	...
	module->pdata = pdata;
	module->dev = dev;
	module->sensor_id = SENSOR_NAME_S5K2P7SX;
	module->subdev = subdev_module;
	module->device = pdata->id;
	module->client = NULL;
	module->active_width = 4624 + 16;
	module->active_height = 3476 + 12;
	...
	module->vci = vci_module_2p7sx;
	module->sensor_maker = "SLSI";
	module->sensor_name = "S5K2P7SX";
	module->setfile_name = "setfile_2p7sx.bin";
	module->cfgs = ARRAY_SIZE(config_module_2p7sx);
	module->cfg = config_module_2p7sx;
	module->ops = NULL;
	/* Sensor peri */
...
static const struct of_device_id exynos_is_sensor_module_2p7sx_match[] = {
	{
		.compatible = "samsung,sensor-module-2p7sx",
	},
	{
		.compatible = "samsung,sensor-module-2p7sx-front",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_2p7sx_match);

static struct platform_driver sensor_module_2p7sx_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-2P7SX",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_2p7sx_match,
	}
};

static int __init is_sensor_module_2p7sx_init(void)
{
	int ret;
...
late_initcall(is_sensor_module_2p7sx_init);
