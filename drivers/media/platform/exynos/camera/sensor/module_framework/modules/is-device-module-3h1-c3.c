...
static struct is_sensor_cfg config_module_3h1_c3[] = {
	/* 3264X2448@30fps */
	IS_SENSOR_CFG_EXT(3264, 2448, 30, 15, 0, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 3264, 6), 0),
	/* 2448X2448@30fps */
	IS_SENSOR_CFG_EXT(2448, 2448, 30, 15, 1, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 2448, 8), 0),
	/* 3264X1836@30fps */
	IS_SENSOR_CFG_EXT(3264, 1836, 30, 15, 2, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 3264, 6), 0),
	/* 1632X918@60fps */
	...
	/* 800X600@30fps */
	IS_SENSOR_CFG(800, 600, 120, 136, 7, CSI_DATA_LANES_2),
	/* 752X1328@15fps */
	IS_SENSOR_CFG(752, 1328, 15, 15, 8, CSI_DATA_LANES_2),
	/* 376X664@110fps */
	IS_SENSOR_CFG(376, 664, 110, 15, 9, CSI_DATA_LANES_2),
};

static struct is_vci vci_module_3h1_c3[] = {
	{
...
static int sensor_module_3h1_c3_power_setpin(struct device *dev,
...
static int __init sensor_module_3h1_c3_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
	...
	is_module_parse_dt(dev, sensor_module_3h1_c3_power_setpin);
	...
	for (vc_idx = 0; vc_idx < 2; vc_idx++) {
		...
		case VC_BUF_DATA_TYPE_SENSOR_STAT1:
			...
			module->vc_extra_info[vc_idx].max_width = S5K3H1_PDAF_MAXWIDTH;
			module->vc_extra_info[vc_idx].max_height = S5K3H1_PDAF_MAXHEIGHT;
			module->vc_extra_info[vc_idx].max_element = S5K3H1_PDAF_ELEMENT;
			break;
		case VC_BUF_DATA_TYPE_GENERAL_STAT1:
			module->vc_extra_info[vc_idx].stat_type = VC_STAT_TYPE_INVALID;
			module->vc_extra_info[vc_idx].sensor_mode = VC_SENSOR_MODE_INVALID;
			module->vc_extra_info[vc_idx].max_width = S5K3H1_MIPI_MAXWIDTH;
			module->vc_extra_info[vc_idx].max_height = S5K3H1_MIPI_MAXHEIGHT;
			module->vc_extra_info[vc_idx].max_element = S5K3H1_MIPI_ELEMENT;
			break;
		}
	}
...
static const struct of_device_id exynos_is_sensor_module_3h1_c3_match[] = {
	{
		.compatible = "samsung,sensor-module-3h1-c3",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_3h1_c3_match);

static struct platform_driver sensor_module_3h1_c3_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-3H1-C3",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_3h1_c3_match,
	}
};

static int __init is_sensor_module_3h1_c3_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_3h1_c3_driver,
				sensor_module_3h1_c3_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_3h1_c3_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_3h1_c3_init);
