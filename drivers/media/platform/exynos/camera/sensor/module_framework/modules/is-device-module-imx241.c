...
static struct is_sensor_cfg config_module_imx241[] = {
	/* 2592x1942@30fps */
	IS_SENSOR_CFG(2592, 1942, 30, 20, 0, CSI_DATA_LANES_2),
	/* 2592x1450@30fps */
	IS_SENSOR_CFG(2592, 1450, 30, 20, 1, CSI_DATA_LANES_2),
	/* 648x484@60fps */
	IS_SENSOR_CFG(648, 484, 114, 24, 2, CSI_DATA_LANES_2),
	/* 648x484@120fps */
	IS_SENSOR_CFG(648, 484, 120, 24, 3, CSI_DATA_LANES_2),
};

static struct is_vci vci_module_imx241[] = {
	{
...
static int sensor_module_imx241_power_setpin(struct device *dev,
	struct exynos_platform_is_module *pdata)
{
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "VDD18_CAM_SENSOR_IO", PIN_REGULATOR, 1, 1000);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "pin", PIN_FUNCTION, 2, 0);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_stby, "sen_stby high", PIN_OUTPUT, 1, 0);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_stby, "sen_stby low", PIN_OUTPUT, 0, 0);
...
static int __init sensor_module_imx241_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
	...
	is_module_parse_dt(dev, sensor_module_imx241_power_setpin);
	...
	module->bitwidth = 10;
	module->vcis = ARRAY_SIZE(vci_module_imx241);
	module->vci = vci_module_imx241;
	module->sensor_maker = "SONY";
	module->sensor_name = "IMX241";
	module->setfile_name = "setfile_imx241.bin";
	module->cfgs = ARRAY_SIZE(config_module_imx241);
	module->cfg = config_module_imx241;
	module->ops = NULL;
	/* Sensor peri */
...
static const struct of_device_id exynos_is_sensor_module_imx241_match[] = {
	{
		.compatible = "samsung,sensor-module-imx241",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_imx241_match);

static struct platform_driver sensor_module_imx241_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-IMX241",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_imx241_match,
	}
};

static int __init is_sensor_module_imx241_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_imx241_driver,
				sensor_module_imx241_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_imx241_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_imx241_init);
