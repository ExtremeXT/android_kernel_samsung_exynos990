...
#define MAX_2P7SQ_SETPIN_CNT 2

static struct is_sensor_cfg config_module_2p7sq[] = {
			/* width, height, fps, settle, mode, lane, speed, interleave, pd_mode */
	IS_SENSOR_CFG(4640, 3488, 30, 0, 0, CSI_DATA_LANES_4, 1495, CSI_MODE_VC_DT, PD_NONE,
	...
	IS_SENSOR_CFG(4608, 3456, 30, 0, 1, CSI_DATA_LANES_4, 1495, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 4608, 3456), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 4608, 3456),
		VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
		VC_IN(2, HW_FORMAT_USER, 0, 0), VC_OUT(HW_FORMAT_USER, VC_NOTHING, 0, 0),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(4608, 2624, 30, 0, 2, CSI_DATA_LANES_4, 1495, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 4608, 2624), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 4608, 2624),
		VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
		VC_IN(2, HW_FORMAT_USER, 0, 0), VC_OUT(HW_FORMAT_USER, VC_NOTHING, 0, 0),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(4608, 2240, 30, 0, 3, CSI_DATA_LANES_4, 1495, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 4608, 2240), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 4608, 2240),
		VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
		VC_IN(2, HW_FORMAT_USER, 0, 0), VC_OUT(HW_FORMAT_USER, VC_NOTHING, 0, 0),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(2304, 1312, 30, 0, 4, CSI_DATA_LANES_4, 1495, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 2304, 1312), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 2304, 1312),
		VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
		VC_IN(2, HW_FORMAT_USER, 0, 0), VC_OUT(HW_FORMAT_USER, VC_NOTHING, 0, 0),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(2304, 1728, 30, 0, 5, CSI_DATA_LANES_4, 1495, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 2304, 1728), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 2304, 1728),
		VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
		VC_IN(2, HW_FORMAT_USER, 0, 0), VC_OUT(HW_FORMAT_USER, VC_NOTHING, 0, 0),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(1152, 864, 120, 0, 6, CSI_DATA_LANES_4, 663, CSI_MODE_VC_DT, PD_NONE,
...
static int sensor_module_2p7sq_power_setpin(struct device *dev,
...
static int __init sensor_module_2p7sq_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
...
#ifdef CONFIG_OF
	is_module_parse_dt(dev, sensor_module_2p7sq_power_setpin);
#endif
	...
	module->pdata = pdata;
	module->dev = dev;
	module->sensor_id = SENSOR_NAME_S5K2P7SQ;
	...
	module->sensor_name = "S5K2P7SQ";
	module->setfile_name = "setfile_2p7sq.bin";
	module->cfgs = ARRAY_SIZE(config_module_2p7sq);
	module->cfg = config_module_2p7sq;
	module->ops = NULL;
...
static const struct of_device_id exynos_is_sensor_module_2p7sq_match[] = {
	{
		.compatible = "samsung,sensor-module-2p7sq",
	},
	{
		.compatible = "samsung,sensor-module-2p7sq-front",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_2p7sq_match);

static struct platform_driver sensor_module_2p7sq_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-2P7SQ",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_2p7sq_match,
	}
};

static int __init is_sensor_module_2p7sq_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_2p7sq_driver,
				sensor_module_2p7sq_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_2p7sq_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_2p7sq_init);
