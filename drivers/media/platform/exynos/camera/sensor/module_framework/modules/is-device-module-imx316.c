...
static struct is_sensor_cfg config_module_imx316[] = {
IS_SENSOR_CFG(480, 1440, 30, 0, 0, CSI_DATA_LANES_2, 598, CSI_MODE_DT_ONLY, PD_NONE,
...
struct is_sensor_ops module_imx316_ops = {
...
static int __init sensor_module_imx316_probe(struct platform_device *pdev)
{
	...
	is_module_parse_dt(dev, sensor_module_imx316_power_setpin);
...
static const struct of_device_id exynos_is_sensor_module_imx316_match[] = {
	{
		.compatible = "samsung,sensor-module-imx316",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_imx316_match);

static struct platform_driver sensor_module_imx316_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-IMX316",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_imx316_match,
	}
};

static int __init is_sensor_module_imx316_init(void)
{
	int ret;
...
late_initcall(is_sensor_module_imx316_init);
