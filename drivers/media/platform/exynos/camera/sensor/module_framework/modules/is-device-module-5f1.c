...
#include "is-device-module-5f1.h"
...
static struct is_sensor_cfg config_5f1[] = {
	/* 2400x2400@30fps */
	IS_SENSOR_CFG(2400, 2400, 30, 0, 0, CSI_DATA_LANES_2, 2041, CSI_MODE_CH0_ONLY, PD_NONE,
...
static int sensor_5f1_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	ulong i;
	struct is_module_enum *module;
	struct is_module_5f1 *module_5f1;
	...
	for (i = 0; i < (int)ARRAY_SIZE(setfile_vision_5f1_reset); i++) {
		ret = is_sensor_write16(client, setfile_vision_5f1_reset[i][0],
	...
	for (i = 0; i < (int)ARRAY_SIZE(setfile_vision_5f1_open_clock); i++) {
		ret = is_sensor_write16(client, setfile_vision_5f1_open_clock[i][0],
	...
	if (!is_iris_ver_read) {
		/* Streaming ON */
		ret = is_sensor_write8(client, SENSOR_REG_STREAM_ON, 0x01);
		if (ret) {
		...
		ret = is_sensor_write16(client, 0x0A02, 0x3E00);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
		...
		/* read start */
		ret = is_sensor_write16(client, 0x0A00, 0x0100);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
		...
		ret = is_sensor_read8(client, 0x0A12, &page_info);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
		...
		/* set the PAGE of OTP */
		ret = is_sensor_write16(client, 0x0A02, page_select);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
		...
		/* read test result */
		ret = is_sensor_read8(client, 0x0A04, &sensor_test[0]); /* focusing */
		ret |= is_sensor_read8(client, 0x0A05, &sensor_test[1]); /* macro */
		ret |= is_sensor_read8(client, 0x0A06, &sensor_test[2]); /* pan */
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
		...
		ret = is_sensor_read8(client, 0x0A07, &is_iris_mtf_read_data[0]); /* read year */
		ret |= is_sensor_read8(client, 0x0A08, &is_iris_mtf_read_data[1]); /* read month */
		ret |= is_sensor_read8(client, 0x0A09, &is_iris_mtf_read_data[2]); /* read company */
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
		...
		/* make initial state */
		ret = is_sensor_write16(client, 0x0A00, 0x0000);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto streamoff_exit;
...
streamoff_exit:
		/* Streaming OFF */
		ret = is_sensor_write8(client, SENSOR_REG_STREAM_ON, 0x00);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
...
int sensor_5f1_s_duration(struct v4l2_subdev *subdev, u64 duration)
{
	...
	if (is_need_init_setting == true) {
		...
		for (i = 0; i < (int)ARRAY_SIZE(setfile_vision_5f1_fps_30_26MHz); i++) {
			ret = is_sensor_write16(client, setfile_vision_5f1_fps_30_26MHz[i][0],
	...
	if (duration == SENSOR_FPS_30) {
		fll = 0x13D7;
		ret = is_sensor_write16(client, SENSOR_REG_FRAME_LENGTH_LINE, fll);
...
int sensor_5f1_s_exposure(struct v4l2_subdev *subdev, u64 exposure)
{
	...
	is_sensor_write16(client, SENSOR_REG_EXPOSURE_TIME, value);
...
int sensor_5f1_s_again(struct v4l2_subdev *subdev, u64 gain)
{
	...
	is_sensor_write16(client, SENSOR_REG_GAIN_ADJUST, calc_value);
...
int sensor_5f1_s_shutterspeed(struct v4l2_subdev *subdev, u64 shutterspeed)
{
	...
	is_sensor_write16(client, SENSOR_REG_SHUTTERSPEED_ADJUST, calc_value);
...
struct is_sensor_ops module_5f1_ops = {
	...
	const struct i2c_device_id *id)
{
	...
	if (is_dev == NULL) {
		warn("is_dev is not yet probed(5f1)");
...
#ifdef CONFIG_OF
	is_module_parse_dt(dev, sensor_5f1_power_setpin);
#endif
...
#ifdef CONFIG_OF
static const struct of_device_id exynos_is_sensor_5f1_match[] = {
	{
...
static struct i2c_driver sensor_5f1_driver = {
	.driver = {
		...
		.owner  = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = exynos_is_sensor_5f1_match
#endif
	},
