...
#define IMX320_PDAF_MAXWIDTH	0 /* MAX witdh size */
#define IMX320_PDAF_MAXHEIGHT	0 /* MAX height size */
#define IMX320_PDAF_ELEMENT	0 /* V4L2_PIX_FMT_SBGGR16 */

#define IMX320_MIPI_MAXWIDTH	3264 /* MAX width size */
#define IMX320_MIPI_MAXHEIGHT	6 /* MAX height size */
#define IMX320_MIPI_ELEMENT	1 /* V4L2_PIX_FMT_SBGGR16 */

static struct is_sensor_cfg config_module_imx320_c3[] = {
	/* 3264X2448@30fps */
	IS_SENSOR_CFG_EXT(3264, 2448, 30, 14, 0, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 3264, 6), 0),
	/* 2448X2448@30fps */
	IS_SENSOR_CFG_EXT(2448, 2448, 30, 14, 1, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 2448, 8), 0),
	/* 3264X1836@30fps */
	IS_SENSOR_CFG_EXT(3264, 1836, 30, 14, 2, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 3264, 6), 0),
	/* 1632X918@60fps */
	IS_SENSOR_CFG_EXT(1632, 918, 60, 7, 3, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 1632, 4), 0),
	/* 1632X918@30fps */
	IS_SENSOR_CFG_EXT(1632, 918, 30, 7, 4, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 1632, 4), 0),
	/* 1632X1224@30fps */
	IS_SENSOR_CFG_EXT(1632, 1224, 30, 7, 5, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 1632, 4), 0),
	/* 1224X1224@30fps */
	IS_SENSOR_CFG_EXT(1224, 1224, 30, 7, 6, CSI_DATA_LANES_2, 2145, 0, SET_VC(VC_MIPI_STAT, 1224, 4), 0),
	/* 800X600@30fps */
	IS_SENSOR_CFG(800, 600, 120, 7, 7, CSI_DATA_LANES_2),
	/* 752X1328@15fps */
	IS_SENSOR_CFG(752, 1328, 15, 14, 8, CSI_DATA_LANES_2),
	/* 376X664@110fps */
	IS_SENSOR_CFG(376, 664, 110, 14, 9, CSI_DATA_LANES_2),
};

static struct is_vci vci_module_imx320_c3[] = {
	{
...
static int sensor_module_imx320_c3_power_setpin(struct device *dev,
...
static int __init sensor_module_imx320_c3_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
	...
	is_module_parse_dt(dev, sensor_module_imx320_c3_power_setpin);
	...
	module->bitwidth = 10;
	module->vcis = ARRAY_SIZE(vci_module_imx320_c3);
	module->vci = vci_module_imx320_c3;
	...
	module->cfgs = ARRAY_SIZE(config_module_imx320_c3);
	module->cfg = config_module_imx320_c3;
	module->ops = NULL;
	...
	for (vc_idx = 0; vc_idx < 2; vc_idx++) {
		...
		case VC_BUF_DATA_TYPE_SENSOR_STAT1:
			...
			module->vc_extra_info[vc_idx].max_width = IMX320_PDAF_MAXWIDTH;
			module->vc_extra_info[vc_idx].max_height = IMX320_PDAF_MAXHEIGHT;
			module->vc_extra_info[vc_idx].max_element = IMX320_PDAF_ELEMENT;
			break;
		case VC_BUF_DATA_TYPE_GENERAL_STAT1:
			module->vc_extra_info[vc_idx].stat_type = VC_STAT_TYPE_INVALID;
			module->vc_extra_info[vc_idx].sensor_mode = VC_SENSOR_MODE_INVALID;
			module->vc_extra_info[vc_idx].max_width = IMX320_MIPI_MAXWIDTH;
			module->vc_extra_info[vc_idx].max_height = IMX320_MIPI_MAXHEIGHT;
			module->vc_extra_info[vc_idx].max_element = IMX320_MIPI_ELEMENT;
			break;
		}
	}
...
static const struct of_device_id exynos_is_sensor_module_imx320_c3_match[] = {
	{
		.compatible = "samsung,sensor-module-imx320-c3",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_imx320_c3_match);

static struct platform_driver sensor_module_imx320_c3_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-IMX320-C3",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_imx320_c3_match,
	}
};

static int __init is_sensor_module_imx320_c3_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_imx320_c3_driver,
				sensor_module_imx320_c3_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_imx320_c3_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_imx320_c3_init);
