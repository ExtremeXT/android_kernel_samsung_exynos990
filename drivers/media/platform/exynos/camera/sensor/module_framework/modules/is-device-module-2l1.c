...
static struct is_sensor_cfg config_module_2l1[] = {
			/* width, height, fps, settle, mode, lane, speed, interleave, pd_mode */
	IS_SENSOR_CFG(4032, 3024, 30, 0, 0, CSI_DATA_LANES_4, 1368, CSI_MODE_VC_DT, PD_NONE,
	...
	IS_SENSOR_CFG(4032, 2268, 30, 0, 1, CSI_DATA_LANES_4, 1368, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 4032, 2268), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 0, 0),
		VC_IN(1, HW_FORMAT_RAW10, 0, 0), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 0, 0),
		VC_IN(0, HW_FORMAT_USER, 4032, 14), VC_OUT(HW_FORMAT_USER, VC_MIPISTAT, 4032, 14),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(3024, 3024, 30, 0, 2, CSI_DATA_LANES_4, 1368, CSI_MODE_VC_DT, PD_NONE,
	...
	IS_SENSOR_CFG(2016, 1134, 120, 0, 4, CSI_DATA_LANES_4, 1092, CSI_MODE_VC_DT, PD_NONE,
		...
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(2016, 1134, 240, 0, 5, CSI_DATA_LANES_4, 1800, CSI_MODE_VC_DT, PD_NONE,
		VC_IN(0, HW_FORMAT_RAW10, 2016, 1134), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 0, 0),
		VC_IN(1, HW_FORMAT_RAW10, 0, 0), VC_OUT(HW_FORMAT_RAW10, VC_NOTHING, 0, 0),
		VC_IN(0, HW_FORMAT_USER, 2016, 8), VC_OUT(HW_FORMAT_USER, VC_MIPISTAT, 2016, 8),
		VC_IN(3, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0)),
	IS_SENSOR_CFG(1008, 756, 120, 0, 6, CSI_DATA_LANES_4, 1800, CSI_MODE_VC_DT, PD_NONE,
...
#ifdef CONFIG_OF
static int sensor_2l1_power_setpin(struct device *dev,
...
static int __init sensor_module_2l1_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
...
#ifdef CONFIG_OF
	is_module_parse_dt(dev, sensor_2l1_power_setpin);
#endif
...
static const struct of_device_id exynos_is_sensor_module_2l1_match[] = {
	{
		.compatible = "samsung,sensor-module-2l1",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_2l1_match);

static struct platform_driver sensor_module_2l1_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-2L1",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_2l1_match,
	}
};

static int __init is_sensor_module_2l1_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_2l1_driver,
				sensor_module_2l1_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_2l1_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_2l1_init);
