...
static struct is_sensor_cfg config_module_imx516[] = {
/* 640 * 480 dual mode / dual freq 8 frame */
IS_SENSOR_CFG(1280, 3840, 30, 0, 0, CSI_DATA_LANES_2, 1440, CSI_MODE_DT_ONLY, PD_NONE,
  VC_IN(0, HW_FORMAT_RAW12, 1280, 3840), VC_OUT(HW_FORMAT_RAW12, VC_NOTHING, 1280, 3840),
  ...
  VC_IN(0, HW_FORMAT_EMBEDDED_8BIT, 192, 3*8), VC_OUT(HW_FORMAT_EMBEDDED_8BIT, VC_EMBEDDED, 192, 3*8)),
/* 320 * 240 dual mode / dual freq 8 frame */
IS_SENSOR_CFG(640, 1920, 30, 0, 1, CSI_DATA_LANES_2, 1440, CSI_MODE_DT_ONLY, PD_NONE,
  VC_IN(0, HW_FORMAT_RAW12, 640, 1920), VC_OUT(HW_FORMAT_RAW12, VC_NOTHING, 640, 1920),
  VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
  VC_IN(2, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
  VC_IN(0, HW_FORMAT_EMBEDDED_8BIT, 192, 3*8), VC_OUT(HW_FORMAT_EMBEDDED_8BIT, VC_EMBEDDED, 192, 3*8)),
 /* 160 * 120 dual mode / dual freq 8 frame */
IS_SENSOR_CFG(320, 960, 30, 0, 2, CSI_DATA_LANES_2, 1440, CSI_MODE_DT_ONLY, PD_NONE,
  VC_IN(0, HW_FORMAT_RAW12, 320, 960), VC_OUT(HW_FORMAT_RAW12, VC_NOTHING, 320, 960),
  VC_IN(1, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
  VC_IN(2, HW_FORMAT_UNKNOWN, 0, 0), VC_OUT(HW_FORMAT_UNKNOWN, VC_NOTHING, 0, 0),
  VC_IN(0, HW_FORMAT_EMBEDDED_8BIT, 192, 3*8), VC_OUT(HW_FORMAT_EMBEDDED_8BIT, VC_EMBEDDED, 192, 3*8)),
};

static int sensor_module_imx516_s_routing(struct v4l2_subdev *sd,
	u32 input, u32 output, u32 config)
{
	info("sensor_module_imx516_s_routing ");
	return 0;
}

int sensor_imx516_s_shutterspeed(struct v4l2_subdev *subdev, u64 shutterspeed)
{
	int ret = 0;
	...
	sensor_peri = (struct is_device_sensor_peri*)sensor->private_data;
...
int sensor_imx516_s_duration(struct v4l2_subdev *subdev, u64 duration)
{
	int ret = 0;
...
struct is_sensor_ops module_imx516_ops = {
	.s_shutterspeed = sensor_imx516_s_shutterspeed,
	.s_duration = sensor_imx516_s_duration,
};
...
static const struct v4l2_subdev_video_ops video_ops = {
	.s_routing = sensor_module_imx516_s_routing,
...
static int sensor_module_imx516_power_setpin(struct device *dev,
...
static int __init sensor_module_imx516_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
	...
	is_module_parse_dt(dev, sensor_module_imx516_power_setpin);
	...
	module->pdata = pdata;
	module->dev = dev;
	module->sensor_id = SENSOR_NAME_IMX516;
	...
	module->sensor_name = "IMX516";
	module->setfile_name = "setfile_imx516.bin";
	module->cfgs = ARRAY_SIZE(config_module_imx516);
	module->cfg = config_module_imx516;
	module->ops = &module_imx516_ops;
...
static const struct of_device_id exynos_is_sensor_module_imx516_match[] = {
	{
		.compatible = "samsung,sensor-module-imx516",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_sensor_module_imx516_match);

static struct platform_driver sensor_module_imx516_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-IMX516",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_is_sensor_module_imx516_match,
	}
};

static int __init is_sensor_module_imx516_init(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_module_imx516_driver,
				sensor_module_imx516_probe);
	if (ret)
		err("failed to probe %s driver: %d\n",
			sensor_module_imx516_driver.driver.name, ret);

	return ret;
}
late_initcall(is_sensor_module_imx516_init);
