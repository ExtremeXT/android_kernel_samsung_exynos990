...
#include "is-config.h"
#include "is-pafstat.h"
#include "is-hw-pafstat.h"
#include "is-interface-library.h"

static struct is_pafstat pafstat_devices[MAX_NUM_OF_PAFSTAT];
atomic_t	g_pafstat_rsccount;

static void prepare_pafstat_sfr_dump(struct is_pafstat *pafstat)
{
	int reg_size = 0;
...
void pafstat_sfr_dump(struct is_pafstat *pafstat)
{
	int reg_size = 0;
...
static irqreturn_t is_isr_pafstat(int irq, void *data)
{
...
static void pafstat_tasklet_fwin_stat(unsigned long data)
{
	...
	pafstat = (struct is_pafstat *)data;
	if (!pafstat) {
		err("failed to get PAFSTAT");
		return;
	}

	module = (struct is_module_enum *)v4l2_get_subdev_hostdata(pafstat->subdev);
	if (!module) {
...
static void pafstat_worker_fwin_stat(struct work_struct *work)
{
	...
	pafstat = container_of(work, struct is_pafstat, work_fwin_stat);
	module = (struct is_module_enum *)v4l2_get_subdev_hostdata(pafstat->subdev);
	if (!module) {
		err("failed to get module");
		return;
...
int pafstat_set_num_buffers(struct v4l2_subdev *subdev, u32 num_buffers, struct is_device_sensor *sensor)
{
	struct is_pafstat *pafstat;
	struct is_device_csi *csi;
		...
		struct paf_setting_t *regs, u32 regs_cnt)
{
	...
	module = (struct is_module_enum *)v4l2_get_subdev_hostdata(subdev);
	if (!module) {
		err("failed to get module");
		return -ENODEV;
	}
		...
		paf_notifier_t notifier, void *data)
{
	...
	pafstat = (struct is_pafstat *)v4l2_get_subdevdata(subdev);
	if (!pafstat) {
...
int pafstat_register(struct is_module_enum *module, int pafstat_ch)
{
	int ret = 0;
	struct is_pafstat *pafstat;
	struct is_device_sensor_peri *sensor_peri = module->private_data;
	u32 version = 0;
	...
	set_bit(IS_SENSOR_PAFSTAT_AVAILABLE, &sensor_peri->peri_state);
...
int pafstat_unregister(struct is_module_enum *module)
{
	...
	clear_bit(IS_SENSOR_PAFSTAT_AVAILABLE, &sensor_peri->peri_state);
...
static void _pafstat_wait_streamoff(struct is_pafstat *pafstat)
{
	uint8_t retry = 0;
	...
	struct v4l2_subdev_format *fmt)
{
	...
	u32 lic_mode;
	int pd_mode = PD_NONE;
	int sensor_mode = VC_SENSOR_MODE_INVALID;
	...
	sensor = (struct is_device_sensor *)v4l2_get_subdev_hostdata(module->subdev);
	if (!sensor) {
...
static int __init pafstat_probe(struct platform_device *pdev)
{
	...
	snprintf(pafstat->name, IS_STR_LEN, "PAFSTAT%d", pafstat->id);
