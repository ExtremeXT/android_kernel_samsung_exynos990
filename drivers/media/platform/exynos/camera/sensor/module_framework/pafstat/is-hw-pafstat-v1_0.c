...
#include "is-hw-pafstat.h"
#include "is-hw-pafstat-v1_0.h"
...
{
	return is_hw_get_field(base_reg, &pafstat_regs[PAFSTAT_R_COM_VERSION],
			&pafstat_fields[PAFSTAT_F_COM_VERSION]);
}

u32 pafstat_hw_g_irq_src(void __iomem *base_reg)
{
	return is_hw_get_reg(base_reg, &pafstat_regs[PAFSTAT_R_CTX_INTERRUPT_SRC]);
}

void pafstat_hw_s_irq_src(void __iomem *base_reg, u32 status)
{
	is_hw_set_reg(base_reg, &pafstat_regs[PAFSTAT_R_CTX_INTERRUPT_SRC], status);
}

u32 pafstat_hw_g_irq_mask(void __iomem *base_reg)
{
	return is_hw_get_reg(base_reg, &pafstat_regs[PAFSTAT_R_CTX_INTERRUPT_MASK]);
}

void pafstat_hw_s_irq_mask(void __iomem *base_reg, u32 mask)
{
	is_hw_set_reg(base_reg, &pafstat_regs[PAFSTAT_R_CTX_INTERRUPT_SRC], 0);
	is_hw_set_reg(base_reg, &pafstat_regs[PAFSTAT_R_CTX_INTERRUPT_MASK], mask);
}

void pafstat_hw_com_s_output_mask(void __iomem *base_reg, u32 val)
{
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_COM_DISABLE_OUTPUTMASK],
...
int pafstat_hw_s_sensor_mode(void __iomem *base_reg, u32 pd_mode)
{
	...
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_CTX_SENSOR_MODE],
...
void pafstat_hw_s_input_path(void __iomem *base_reg, enum pafstat_input_path input)
{
	...
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_CTX_SEL_OTF],
			&pafstat_fields[PAFSTAT_F_CTX_SEL_OTF], input);

	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_CTX_LIC_IS_DMA],
	...
	enum pafstat_lic_mode lic_mode, enum pafstat_input_path input)
{
	...
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_COM_LIC_SEL_SINGLE_INPUT],
			&pafstat_fields[PAFSTAT_F_COM_LIC_SEL_SINGLE_INPUT], id + offset);
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_COM_LIC_MODE],
			&pafstat_fields[PAFSTAT_F_COM_LIC_MODE], lic_mode);
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_COM_LIC_BYPASS],
...
int pafstat_hw_s_4ppc(void __iomem *base_reg, u32 csi_pixel_mode)
{
	...
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_CTX_LIC_4PPC],
...
{
	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_CTX_TIMEOUT_CNT_INIT],
...
#define IS_PAFSTAT_RESET_DONE(addr, reg, field)	\
	(is_hw_get_field(addr, &pafstat_regs[reg], &pafstat_fields[field]) != 0)

int pafstat_hw_sw_reset(void __iomem *base_reg)
{
	u32 reset_count = 0;

	is_hw_set_field(base_reg, &pafstat_regs[PAFSTAT_R_COM_RESET_REQ],
...
void pafstat_hw_s_lbctrl(void __iomem *base_reg, u32 width, u32 height)
{
	...
	pd_mode = is_hw_get_field(base_reg, &pafstat_regs[PAFSTAT_R_CTX_SENSOR_MODE],
...
int pafstat_hw_g_fwin_stat(void __iomem *base_reg, void *buf, size_t len)
{
	...
	for (f_idx = 0; f_idx < 6; f_idx++) {
		for (t = 0; t < 112; t++) {
			...
			fwin_stat[f_idx][t] = is_hw_get_reg(base_reg, &pafstat_regs[reg_idx]);
		}
	}
...
/* PAF RDMA */
void is_hw_paf_oneshot_enable(void __iomem *base_reg)
{
...
void is_hw_paf_common_config(void __iomem *base_reg_com, void __iomem *base_reg, u32 paf_ch, u32 width, u32 height)
{
	int enable = 0;
...
void is_hw_paf_rdma_reset(void __iomem *base_reg)
{
	int ret = 0;
	u32 timeout = 1000;

	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_COM_RESET],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_COM_SW_RESET], 1);

	do {
		ret = is_hw_get_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_COM_RESET_STATUS],
...
void is_hw_paf_rdma_config(void __iomem *base_reg, u32 hw_format, u32 bitwidth,
		u32 width, u32 height, u32 pix_stride)
{
	...
	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_I_DATA_FORMAT],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_RDMA_I_DATA_FORMAT], dma_format);

	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_I_IMG_WIDTH],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_RDMA_I_IMG_WIDTH], width);
	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_I_IMG_HEIGHT],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_RDMA_I_IMG_HEIGHT], height);

	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_I_IMG_STRIDE],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_RDMA_I_IMG_STRIDE], stride_size);
}

void is_hw_paf_rdma_set_addr(void __iomem *base_reg, u32 addr)
{
	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_I_BASE_ADDR],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_RDMA_I_BASE_ADDR], addr);
}

void is_hw_paf_rdma_enable(void __iomem *base_reg_com, void __iomem *base_reg, u32 enable)
{
	is_hw_set_field(base_reg_com, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_COM_QACTIVE],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_COM_RDMA_QACTIVE], enable);
	is_hw_set_field(base_reg, &pafstat_rdma_regs[PAFSTAT_RDMA_R_RDMA_I_EN],
			&pafstat_rdma_fields[PAFSTAT_RDMA_F_RDMA_I_EN], enable);
}

void is_hw_paf_sfr_dump(void __iomem *base_reg_com, void __iomem *base_reg)
{
	info("pafstat sfr dump\n");

	info("PAFSTAT SFR DUMP : %p\n", base_reg_com);
	is_hw_dump_regs(base_reg_com, pafstat_regs, PAFSTAT_R_PAFSTAT_YEXTR_HDRMODE);

	info("PAFSTAT RDMA SFR DUMP : %p\n", base_reg);
	is_hw_dump_regs(base_reg, pafstat_rdma_regs, PAFSTAT_RDMA_REG_CNT);
}
