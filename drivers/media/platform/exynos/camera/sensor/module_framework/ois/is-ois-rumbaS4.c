...
int is_ois_fw_version_read(struct is_ois *ois)
{
	int ret = 0;
	char version[7] = {0, };
...
int is_ois_fw_ready(struct is_ois *ois)
{
	...
	if (ois_status32 == OIS_FW_UPDATE_ERROR_STATUS) {
		...
		ret = is_ois_fw_version_read(ois);
		if (ret < 0) {
			err("Failed to ois read cal");
			return ret;
		}
...
int is_ois_firmware_update(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
#ifdef CONFIG_OIS_DIRECT_FW_CONTROL
	.ois_fw_update = is_ois_firmware_update,
#endif
};
...
static const struct of_device_id sensor_ois_rumbaS4_match[] = {
	{
		.compatible = "samsung,exynos-is-ois-rumbaS4",
	},
	{},
};
