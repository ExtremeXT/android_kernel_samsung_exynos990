...
#include "is-ois-idg2030.h"
#include "is-ois.h"

#define OIS_NAME "OIS_IDG2030"

static u8 fw_data[OIS_BOOT_FW_SIZE + OIS_FW_SIZE] = {0,};
static int ois_shift_x[POSITION_NUM];
static int ois_shift_y[POSITION_NUM];

static char ois_fw_ver[IS_OIS_FW_VER_SIZE];
static char p_ois_fw_ver[IS_OIS_FW_VER_SIZE];

static const struct v4l2_subdev_ops subdev_ops;

int is_ois_read_cal(struct is_ois *ois)
{
	int ret = 0;
	u8 version[20] = {0, };
	u8 read_ver[3] = {0, };
	...
	ret = is_ois_write_multi(ois->client, 0x0100, version, 0x16);
	if (ret < 0) {
		err("ois read version fail");
		return ret;
	}

	ret |= is_ois_read(ois->client, 0x0118, &read_ver[0]);
	ret |= is_ois_read(ois->client, 0x0119, &read_ver[1]);
	ret |= is_ois_read(ois->client, 0x011A, &read_ver[2]);
	if (ret < 0) {
		err("i2c cmd fail\n");
		ret = -EINVAL;
		return ret;
...
int is_ois_read_user_data(struct is_ois *ois)
{
	int ret = 0;
	int i = 0;
	u8 send_data[2];
	...
	/* OIS servo OFF */
	ret = is_ois_write(ois->client ,0x0000, 0x00);
	if (ret < 0) {
	...
	send_data[0] = 0x00;
	send_data[1] = 0x00;
	ret |= is_ois_write(ois->client ,0x000F, 0x12);
	ret |= is_ois_write_multi(ois->client, 0x0010, send_data, 4);
	ret |= is_ois_write(ois->client ,0x000E, 0x04);
	if (ret < 0) {
		err("ois step1 setting fail");
		return ret;
	}

	ret = is_ois_read_multi(ois->client, 0x0100, read_data, 73);
	if (ret < 0) {
		err("ois read fail");
		return ret;
	}
	...
	send_data[0] = 0x00;
	send_data[1] = 0x04;
	ret |= is_ois_write(ois->client ,0x000F, 0x0E);
	ret |= is_ois_write_multi(ois->client, 0x0010, send_data, 4);
	ret |= is_ois_write(ois->client ,0x000E, 0x04);
	if (ret < 0) {
...
int is_ois_fw_version(struct is_ois *ois)
{
	int ret = 0;
	char version[7] = {0, };
	...
	ret = is_ois_read(ois->client, 0x00FB, &version[0]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}

	ret = is_ois_read(ois->client, 0x00F9, &version[1]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}

	ret = is_ois_read(ois->client, 0x00F8, &version[2]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}

	ret = is_ois_read(ois->client, 0x007C, &version[3]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}

	ret = is_ois_read(ois->client, 0x007D, &version[4]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}

	ret = is_ois_read(ois->client, 0x007E, &version[5]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}

	ret = is_ois_read(ois->client, 0x007F, &version[6]);
	if (ret < 0) {
		err("i2c read fail\n");
		return ret;
	}
...
int is_ois_fw_ready(struct is_ois *ois)
{
	int ret = 0;
	...
	ret = is_ois_read_multi(ois->client, 0x00FC, ois_status, 4);
	if (ret < 0) {
	...
	if (ois_status32 == OIS_FW_UPDATE_ERROR_STATUS) {
		ret = is_ois_read_cal(ois);
		if (ret < 0) {
			err("Failed to ois read cal");
			return ret;
		}
	} else {
		ret = is_ois_fw_version(ois);
		if (ret < 0) {
			err("Failed to ois read cal");
			return ret;
		}

		ret = is_ois_read_user_data(ois);
		if (ret < 0) {
	...
	case 'A':
	case 'C':
		ret = is_ois_fw_open(ois, IS_OIS_FW_NAME_DOM);
		if (ret < 0) {
	...
	case 'B':
	case 'D':
		ret = is_ois_fw_open(ois, IS_OIS_FW_NAME_SEC);
		if (ret < 0) {
			err("OIS %c load is fail\n", ois_fw_ver[FW_OIS_VERSION]);
			return ret;
		}
...
int is_ois_fw_download(struct is_ois *ois)
{
	...
	send_data[0] = 0x75;
	ret = is_ois_write(ois->client, 0x000C, send_data[0]);
	if (ret < 0) {
	...
	for (block = 0; block < 112; block++) {
		...
		ret = is_ois_write_multi(ois->client, 0x0100, send_data, FW_TRANS_SIZE + 2);
		if (ret < 0) {
...
int is_ois_shift_compensation(struct v4l2_subdev *subdev, int position)
{
	int ret = 0;
	struct is_ois *ois;
	u32 af_value;
	...
	ret = is_ois_write_multi(ois->client, 0x004C, write_data, 4);
	if (ret < 0)
...
int is_ois_mode_change(struct v4l2_subdev *subdev, int mode)
{
	int ret = 0;
	struct is_ois *ois;
	...
	case OPTICAL_STABILIZATION_MODE_STILL:
		is_ois_write(client, 0x0002, 0x00);
		is_ois_write(client, 0x0000, 0x01);
		break;
	case OPTICAL_STABILIZATION_MODE_STILL_ZOOM:
		is_ois_write(client, 0x0002, 0x13);
		is_ois_write(client, 0x0000, 0x01);
		break;
	case OPTICAL_STABILIZATION_MODE_VIDEO:
		is_ois_write(client, 0x0002, 0x01);
		is_ois_write(client, 0x0000, 0x01);
		break;
	case OPTICAL_STABILIZATION_MODE_CENTERING:
		is_ois_write(client, 0x0002, 0x05);
		is_ois_write(client, 0x0000, 0x01);
		break;
	case OPTICAL_STABILIZATION_MODE_VDIS:
		write_data[0] = 0xFF;
		write_data[1] = 0xFF;
		is_ois_write_multi(client, 0x005E, write_data, 2);
		is_ois_write(client, 0x0002, 0x14);
		is_ois_write(client, 0x0000, 0x01);
		break;
	default:
...
int is_ois_init(struct v4l2_subdev *subdev)
{
	...
	/* use user data */
	ret |= is_ois_write(ois->client, 0x000F, 0x0E);
	ret |= is_ois_write(ois->client, 0x0010, 0x0004);
	ret |= is_ois_write(ois->client, 0x000E, 0x04);
	if (ret < 0) {
	...
	for (i = 0; i < 9; i++) {
		cal_data[0] = 0;
		cal_data[1] = 0;
		is_ois_read_multi(ois->client, 0x0108 + (i * 2), cal_data, 2);
		ois_shift_x_cal = (cal_data[1] << 8) | (cal_data[0]);

		cal_data[0] = 0;
		cal_data[1] = 0;
		is_ois_read_multi(ois->client, 0x0120 + (i * 2), cal_data, 2);
	...
	/* CACTRL */
	ret = is_ois_write(ois->client, 0x0039, 0x01);
	if (ret < 0) {
...
static struct is_ois_ops ois_ops = {
	...
	.ois_mode_change = is_ois_mode_change,
	.ois_shift_compensation = is_ois_shift_compensation,
		...
		const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_ois_id(device, OIS_NAME_IDG2030);
	if (!sensor_peri) {
...
static const struct of_device_id sensor_ois_idg2030_match[] = {
	{
		.compatible = "samsung,exynos-is-ois-idg2030",
	},
	{},
};
