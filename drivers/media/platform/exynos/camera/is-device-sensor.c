...
int is_search_sensor_module(struct is_device_sensor *device,
...
#ifdef ENABLE_DTP
static void is_sensor_dtp(struct timer_list *t)
{
	struct is_video_ctx *vctx;
	struct is_device_sensor *device = from_timer(device, t, dtp_timer);
...
static int is_sensor_start(struct is_device_sensor *device)
{
	...
	if (test_bit(IS_SENSOR_DRIVING, &device->state)) {
...
int is_sensor_subdev_tag(struct is_device_sensor *device,
	struct is_frame *frame)
{
	int ret = 0;
	...
	unsigned int notification)
{
	...
	hashkey = device->fcount % IS_TIMESTAMP_HASH_KEY;
	device->timestamp[hashkey] = is_get_timestamp();
	device->timestampboot[hashkey] = is_get_timestamp_boot();
	...
	case CSIS_NOTIFY_FSTART:
		...
		for (i = CSI_VIRTUAL_CH_1; i <= CSI_VIRTUAL_CH_3; i++) {
			...
			if (test_bit(IS_SUBDEV_INTERNAL_USE, &dma_subdev->state)) {
				...
				if (frame) {
					frame->fcount = device->fcount;
					is_sensor_subdev_tag(device, frame);
				}
			}
	...
	unsigned int notification)
{
	...
	if (device->force_stop)
		is_sensor_dtp(&device->dtp_timer);
#endif
	...
	if (frame) {
		...
		/* get video context */
		vctx = (frame->subdev) ?
			((struct is_subdev *)frame->subdev)->vctx : device->vctx;
		FIMC_BUG(!vctx);
...
static void is_sensor_instanton(struct work_struct *data)
{
	...
	if (ret) {
		...
		merr("is_sensor_start is fail(%d)\n", device, ret);
...
static int __init is_sensor_probe(struct platform_device *pdev)
{
	...
	instance = v4l2_device_set_name(&device->v4l2_dev, IS_SENSOR_DEV_NAME, &device_id);
	...
	/* 3. state init*/
	clear_bit(IS_SENSOR_PROBE, &device->state);
	...
	clear_bit(IS_SENSOR_DRIVING, &device->state);
	...
	ret = is_csi_probe(device, device->pdata->csi_ch);
	if (ret) {
	...
	is_subdev_probe(&device->ssvc0, instance, ENTRY_SSVC0, "SSXVC0", NULL);
	...
	is_subdev_probe(&device->ssvc1, instance, ENTRY_SSVC1, "SSXVC1", NULL);
	...
	is_subdev_probe(&device->ssvc2, instance, ENTRY_SSVC2, "SSXVC2", NULL);
	...
	struct is_video_ctx *vctx)
{
	...
	ret = is_subdev_internal_open((void *)device, IS_DEVICE_SENSOR);
	if (ret) {
		merr("is_sensor_subdev_internal_open is fail(%d)", device, ret);
...
err_resource_get:
	ret_err = is_subdev_internal_close((void *)device, IS_DEVICE_SENSOR);
	if (ret_err)
		merr("is_sensor_subdev_internal_open is fail(%d)", device, ret_err);
...
int is_sensor_close(struct is_device_sensor *device)
{
	...
	ret = is_subdev_internal_close((void *)device, IS_DEVICE_SENSOR);
	if (ret)
		merr("is_sensor_subdev_internal_open is fail(%d)", device, ret);
	...
	u32 scenario)
{
	...
	if (scenario == SENSOR_SCENARIO_NORMAL || scenario == SENSOR_SCENARIO_STANDBY)
		...
		set_bit(IS_SENSOR_DRIVING, &device->state);
...
/* HACK :: smc code is not prepared		ret = exynos_smc(MC_SECURE_CAMERA_PREPARE, 0, 0, 0); */
	...
	ret = is_sensor_mclk_on(device, scenario, module->pdata->mclk_ch);
	if (ret) {
	...
	vender = &core->vender;
	is_vender_sensor_s_input(vender, (void *)module);
	...
	struct is_queue *queue)
{
	...
	if (!test_bit(IS_SENSOR_SUBDEV_MODULE_INIT, &device->state)) {
		ret = v4l2_subdev_call(subdev_module, core, init, 0);
		if (ret) {
			merr("v4l2_module_call(init) is fail(%d)", device, ret);
			goto p_err;
		...
		set_bit(IS_SENSOR_SUBDEV_MODULE_INIT, &device->state);
	}
	...
	ret = is_subdev_internal_s_format((void *)device, IS_DEVICE_SENSOR);
	if (ret) {
		merr("is_sensor_subdev_inernal_s_format is fail(%d)", device, ret);
		goto p_err;
	}
...
int is_sensor_s_fcount(struct is_device_sensor *device)
{
	...
	case CSI_ID_D:
		...
		break;
	default:
		merr("[SEN:D] CSI ch%d is invlid", device, csi->ch);
		ret = -EINVAL;
		break;
	}
	...
	u32 index)
{
	...
	if (unlikely(!device || !test_bit(IS_SENSOR_OPEN, &device->state))) {
	...
	struct is_queue *queue)
{
	...
	if (test_bit(IS_SENSOR_BACK_NOWAIT_STOP, &device->state)) {
		mwarn("is_flite_stop, no waiting...", device);
	...
	u32 nonblock)
{
	...
	ret = is_subdev_internal_start((void *)device, IS_DEVICE_SENSOR);
	if (ret) {
...
int is_sensor_front_stop(struct is_device_sensor *device)
{
	...
	ret = is_subdev_internal_stop((void *)device, IS_DEVICE_SENSOR);
	if (ret)
...
int is_sensor_runtime_suspend(struct device *dev)
{
	...
	ret = is_sensor_g_module(device, &module);
	if (ret)
		mwarn("is_sensor_g_module is fail(%d)", device, ret);
...
/* HACK :: smc code is not prepared	ret = exynos_smc(MC_SECURE_CAMERA_CFW_ENABLE, 0, 0, 0); */
	if(ret != 0) {
		...
		minfo("[SMC] Call MC_SECURE_CAMERA_CFW_ENABLE ret(%d) / smc_state(%d->%d)\n",
				device, ret, device->smc_state, IS_SENSOR_SMC_CFW_ENABLE);
		device->smc_state = IS_SENSOR_SMC_CFW_ENABLE;
	}
#endif
