...
/*
 * This function must be called in case of IS_GROUP_PIPE_INPUT.
 */
static int is_cancel_all_cap_nodes(struct is_device_ischain *device,
	struct is_pipe *pipe,
	struct is_video_ctx  *src_vctx,
	struct is_video_ctx  *junction_vctx,
	struct is_framemgr *src_framemgr,
	struct is_frame *src_frame)
{
	int ret = 0;
	int capture_id;
	unsigned long flags;
	...
	junction = (struct is_subdev *)junction_vctx->subdev;
	...
	for (capture_id = 0; capture_id < CAPTURE_NODE_MAX; ++capture_id) {
		...
		subdev = video2subdev(IS_ISCHAIN_SUBDEV, (void *)device, cap_node->vid);
		if (!subdev) {
 ...
 */
static int is_pipe_dqbuf_proxy(struct is_video_ctx *vctx,
	...
	bool blocking)
{
	...
	ret = is_video_dqbuf(vctx, &v4l2_buf, blocking);
	if (ret) {
		err("is_video_dqbuf is fail(%d)", ret);
 ...
 */
static int is_pipe_qbuf_proxy(struct is_video_ctx *vctx,
	struct v4l2_buffer *buf)
{
	int ret;
	struct is_video *video;
	...
	ret = is_video_qbuf(vctx, buf);
	if (ret)
		err("is_video_qbuf is fail(%d)", ret);
...
static int is_pipe_callback(struct is_device_ischain *device,
	...
	struct is_frame *frame)
{
	...
	struct is_pipe *pipe;
	struct is_framemgr *src_framemgr, *junction_framemgr, *dst_framemgr;
	struct is_frame *junction_frame = NULL;
	struct is_frame *dst_frame = NULL;
	struct is_video_ctx *src_vctx, *junction_vctx, *dst_vctx;
	struct is_subdev *junction;
	...
	if (!test_bit(IS_GROUP_PIPE_OUTPUT, &group->state))
		goto p_err;
	...
	if (!timeout) {
		...
		is_cancel_all_cap_nodes(device, pipe,
	 ...
	 * Also, without this setting, the problem that is_frame's device addr
	...
	if (ret) {
		mperr("[F%d] is_video_qbuf is fail(%d)",
...
static int is_pipe_qbuf(struct is_video_ctx *vctx,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	...
	if (test_bit(IS_GROUP_PIPE_INPUT, &pipe->dst->state)) {
	...
	if (qbuf_proxy)
		ret = is_pipe_qbuf_proxy(vctx, buf);
	else
		ret = is_video_qbuf(vctx, buf);

	if (ret)
		mperr("is_video_qbuf(%d) is fail(%d)", device, pipe, vctx->video, buf->index, ret);

p_err:
	return ret;
}

static int is_pipe_dqbuf(struct is_video_ctx *vctx,
	struct v4l2_buffer *buf,
	bool blocking)
{
	int ret = 0;
	...
	struct is_pipe *pipe;
	struct is_framemgr *src_framemgr, *junction_framemgr, *dst_framemgr;
	struct is_frame *src_frame, *junction_frame, *dst_frame;
	struct is_video_ctx  *src_vctx, *junction_vctx, *dst_vctx;
	...
	if (src_vctx->video->id == vctx->video->id) {
		/* Source */
		ret = is_video_dqbuf(src_vctx, buf, blocking);
		src_framemgr = GET_FRAMEMGR(src_vctx);
		dst_framemgr = GET_FRAMEMGR(dst_vctx);
		src_frame = &src_framemgr->frames[buf->index];

		if (ret) {
			mperr("is_video_dqbuf is fail(%d)", device, pipe, vctx->video, ret);
		...
		if (test_bit(IS_GROUP_PIPE_INPUT, &pipe->dst->state)) {
			...
			if (buf->flags & V4L2_BUF_FLAG_ERROR) {
				...
				/* cancel all capture node buffer */
				tmp_ret = is_cancel_all_cap_nodes(device, pipe,
			...
			if (tmp_ret) {
				mperr("is_video_dqbuf is fail(%d)", device, pipe, vctx->video, tmp_ret);
				goto pipe_pass;
			}
		}
		...
		/* Junction */
		if (test_bit(IS_GROUP_PIPE_INPUT, &pipe->dst->state)) {
			index = is_pipe_dqbuf_proxy(junction_vctx, buf, blocking);
			if (index < 0) {
				ret = index;
				mperr("is_video_dqbuf is fail(%d)", device, pipe, vctx->video, ret);
				goto pipe_pass;
			...
			ret = is_video_dqbuf(junction_vctx, buf, blocking);
			if (ret) {
				mperr("is_video_dqbuf is fail(%d)", device, pipe, vctx->video, ret);
				goto pipe_pass;
			}
		...
		if (ret)
			mperr("[F%d] is_video_qbuf is fail(%d)", device, pipe, vctx->video, index, ret);

	} else if (dst_vctx->video->id == vctx->video->id) {
		/* Destination */
		ret = is_pipe_dqbuf_proxy(dst_vctx, buf, blocking);
		if (ret < 0) {
			mperr("is_video_dqbuf is fail(%d)", device, pipe, vctx->video, ret);
			goto pipe_pass;
		}
...
static int is_pipe_done(struct is_video_ctx *vctx,
	u32 index, u32 state)
{
	int ret = 0;
	struct is_device_ischain *device;
	struct is_pipe *pipe;
	struct is_framemgr *dst_framemgr;
	struct is_frame *dst_frame;
	struct is_video_ctx *dst_vctx;
	...
	ret = is_video_buffer_done(vctx, index, state);
	if (ret)
		mperr("is_video_buffer_done is fail(%d)", device, pipe, vctx->video, ret);
	...
	struct is_group *dst)
{
	...
	pipe->src = src;
	pipe->dst = dst;
	src->pipe_shot_callback = is_pipe_callback;
	dst->pipe_shot_callback = is_pipe_callback;
	...
	for (i = 0; i < PIPE_SLOT_MAX; i++) {
		pipe->vctx[i]->vops.qbuf = is_pipe_qbuf;
		pipe->vctx[i]->vops.dqbuf = is_pipe_dqbuf;
		pipe->vctx[i]->vops.done = is_pipe_done;

		memset(pipe->buf[i], 0x0, (sizeof(struct v4l2_buffer) * IS_MAX_BUFS));
		memset(pipe->planes[i], 0x0, (sizeof(struct v4l2_plane) * IS_MAX_BUFS * VIDEO_MAX_PLANES));
	}
