...
const struct v4l2_file_operations is_mcs_video_fops;
...
static int is_mcs_video_open(struct file *file)
{
	int ret = 0;
	int ret_err = 0;
	struct is_video *video;
	...
	ret = is_ischain_mcs_open(device, vctx);
	if (ret) {
		merr("is_ischain_mcs_open is fail(%d)", device, ret);
...
static int is_mcs_video_close(struct file *file)
{
	int ret = 0;
	int refcount;
	struct is_video_ctx *vctx;
	...
	ret = is_ischain_mcs_close(device, vctx);
	if (ret)
		merr("is_ischain_mcs_close is fail(%d)", device, ret);
...
static unsigned int is_mcs_video_poll(struct file *file,
...
static int is_mcs_video_mmap(struct file *file,
...
const struct v4l2_file_operations is_mcs_video_fops = {
	.owner		= THIS_MODULE,
	.open		= is_mcs_video_open,
	.release	= is_mcs_video_close,
	.poll		= is_mcs_video_poll,
	.unlocked_ioctl	= video_ioctl2,
	.mmap		= is_mcs_video_mmap,
};
...
static int is_mcs_video_querycap(struct file *file, void *fh,
...
static int is_mcs_video_set_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mcs_video_querybuf(struct file *file, void *priv,
...
static int is_mcs_video_qbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	...
	mvdbgs(3, "%s(%02d:%d)\n", vctx,
			&vctx->queue, __func__, buf->type, buf->index);
...
static int is_mcs_video_dqbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mcs_video_prepare(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mcs_video_streamon(struct file *file, void *priv,
...
static int is_mcs_video_streamoff(struct file *file, void *priv,
	enum v4l2_buf_type type)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mcs_video_enum_input(struct file *file, void *priv,
...
static int is_mcs_video_g_input(struct file *file, void *priv,
	unsigned int *input)
{
...
static int is_mcs_video_s_input(struct file *file, void *priv,
	unsigned int input)
{
	int ret = 0;
	...
	ret = is_ischain_mcs_s_input(device, stream, position, vindex, intype, leader);
	if (ret) {
		merr("is_ischain_mcs_s_input is fail(%d)", device, ret);
		goto p_err;
	}
...
static int is_mcs_video_s_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_IS_FORCE_DONE:
		set_bit(IS_GROUP_REQUEST_FSTOP, &device->group_mcs.state);
		break;
	default:
...
static int is_mcs_video_g_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
...
static int is_mcs_video_g_ext_ctrl(struct file *file, void *priv,
...
const struct v4l2_ioctl_ops is_mcs_video_ioctl_ops = {
	...
	.vidioc_s_fmt_vid_out_mplane	= is_mcs_video_set_format_mplane,
	.vidioc_s_fmt_vid_cap_mplane	= is_mcs_video_set_format_mplane,

	.vidioc_querybuf		= is_mcs_video_querybuf,
	.vidioc_reqbufs			= is_mcs_video_reqbufs,

	.vidioc_qbuf			= is_mcs_video_qbuf,
	.vidioc_dqbuf			= is_mcs_video_dqbuf,
	.vidioc_prepare_buf		= is_mcs_video_prepare,

	.vidioc_streamon		= is_mcs_video_streamon,
	.vidioc_streamoff		= is_mcs_video_streamoff,

	.vidioc_enum_input		= is_mcs_video_enum_input,
	.vidioc_g_input			= is_mcs_video_g_input,
	.vidioc_s_input			= is_mcs_video_s_input,
...
static int is_mcs_start_streaming(struct vb2_queue *vbq,
	unsigned int count)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static void is_mcs_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	struct is_queue *queue;
...
static void is_mcs_buffer_queue(struct vb2_buffer *vb)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	struct is_queue *queue;
	...
	ret = is_ischain_mcs_buffer_queue(device, queue, vb->index);
	if (ret) {
		merr("is_ischain_mcs_buffer_queue is fail(%d)", device, ret);
		return;
	}
}
