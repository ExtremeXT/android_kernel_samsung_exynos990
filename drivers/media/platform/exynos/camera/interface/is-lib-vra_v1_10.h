...
#define VRA_DICO_API_VERSION			112
...
enum api_vra_type {
	...
	VRA_Err_NotAllowedForCh0Input		= 0x68, /*!< VRA_GetFrWorkFrameAllocation not allowed when Ch0 input is used */
	VRA_Err_NotAllowedForHwCfgId		= 0x69,	/*!< Operation not allowed for current VRA_HW_CFG_ID */
	VRA_Err_NoFreeSlot					= 0x6A,	/*!< no free pyramid slot */
	VRA_Err_SlotAlreadyAllocated		= 0x6B,	/*!< A pyramid slot was already allocated and not used */
	VRA_Err_SlotNotAllocated			= 0x6C,	/*!< No pyramid slot was allocated */
	VRA_Err_WrongSlotIndex				= 0x6D, /*!< The input slot index wasn't allocated for HOST */
	VRA_Err_WrongInputFormatForAlloc	= 0x6E, /*!< Current frame input format isn't allowed for allocated frame */
	VRA_Err_WrongAllocLineOfs			= 0x6F,	/*!< The allocated UV line offset or Y line offsert not match the sizes of current frame */
	VRA_Err_WrongAllocUVAdr				= 0x70, /*!< The allocated UV address don't match to the allocated slot */
	VRA_Err_WrongAllocYAdr				= 0x71, /*!< The allocated Y address don't match to the allocated slot */
	VRA_Err_AllocPadTopPyr				= 0x72, /*!< Y top pyramid padding is required - not allowed for allocated input */
	VRA_Err_TaskActive					= 0x73,	/*!< Can't be done while Framework task is active */
	VRA_Err_HwClocksInRequiredEnState	= 0x74,	/*!< VRA_FrameWorkEnableHwClocks with EnableClocks that is already set */
	VRA_Err_HwClocksDisabled			= 0x75,	/*!< Must enable clocks before new input */
