...
const struct v4l2_file_operations is_mxp_video_fops;
...
static int is_mxp_video_open(struct file *file)
{
	int ret = 0;
	int ret_err = 0;
	struct is_video *video;
...
static int is_mxp_video_close(struct file *file)
{
	int ret = 0;
	int refcount;
	struct is_video_ctx *vctx;
	...
	minfo("[M%dP:V] %s(%d,%d):%d\n", device,
		GET_MXP_ID(video), __func__,
		atomic_read(&device->open_cnt), refcount, ret);

	return ret;
}

static unsigned int is_mxp_video_poll(struct file *file,
...
static int is_mxp_video_mmap(struct file *file,
...
const struct v4l2_file_operations is_mxp_video_fops = {
	.owner		= THIS_MODULE,
	.open		= is_mxp_video_open,
	.release	= is_mxp_video_close,
	.poll		= is_mxp_video_poll,
	.unlocked_ioctl	= video_ioctl2,
	.mmap		= is_mxp_video_mmap,
};
...
static int is_mxp_video_querycap(struct file *file, void *fh,
...
static int is_mxp_video_set_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mxp_video_reqbufs(struct file *file, void *priv,
...
static int is_mxp_video_querybuf(struct file *file, void *priv,
...
static int is_mxp_video_qbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	...
	is_hw_s_ctrl(vctx, vctx->video->id, HW_S_CTRL_HWFC_IDX_RESET, (void *)1);

	return ret;
}

static int is_mxp_video_dqbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mxp_video_streamon(struct file *file, void *priv,
...
static int is_mxp_video_streamoff(struct file *file, void *priv,
	enum v4l2_buf_type type)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mxp_video_enum_input(struct file *file, void *priv,
...
static int is_mxp_video_g_input(struct file *file, void *priv,
	unsigned int *input)
{
	dbg("%s\n", __func__);
	return 0;
...
static int is_mxp_video_s_input(struct file *file, void *priv,
	unsigned int input)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static int is_mxp_video_g_ctrl(struct file *file, void *priv,
...
static int is_mxp_video_g_ext_ctrl(struct file *file, void *priv,
	struct v4l2_ext_controls *ctrls)
{
	return 0;
}

static int is_mxp_video_s_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
const struct v4l2_ioctl_ops is_mxp_video_ioctl_ops = {
	...
	.vidioc_dqbuf			= is_mxp_video_dqbuf,
	.vidioc_streamon		= is_mxp_video_streamon,
	.vidioc_streamoff		= is_mxp_video_streamoff,
	.vidioc_enum_input		= is_mxp_video_enum_input,
	.vidioc_g_input			= is_mxp_video_g_input,
	.vidioc_s_input			= is_mxp_video_s_input,
	.vidioc_g_ctrl			= is_mxp_video_g_ctrl,
	.vidioc_s_ctrl			= is_mxp_video_s_ctrl,
	.vidioc_g_ext_ctrls		= is_mxp_video_g_ext_ctrl,
};

static int is_mxp_queue_setup(struct vb2_queue *vbq,
...
static int is_mxp_start_streaming(struct vb2_queue *vbq,
	unsigned int count)
{
	int ret = 0;
	struct is_video_ctx *vctx;
...
static void is_mxp_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	struct is_queue *queue;
...
static void is_mxp_buffer_queue(struct vb2_buffer *vb)
{
	int ret = 0;
	struct is_video_ctx *vctx;
	struct is_queue *queue;
