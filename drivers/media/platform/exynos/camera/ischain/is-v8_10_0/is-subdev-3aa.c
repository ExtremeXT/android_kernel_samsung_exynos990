	...
	u32 *indexes)
{
	...
	if (group->head->id == GROUP_ID_PAF0 ||
		group->head->id == GROUP_ID_PAF1 ||
		group->head->id == GROUP_ID_PAF2)
		otf_input->source = OTF_INPUT_PAF_RDMA_PATH;

	otf_input->cmd = OTF_INPUT_COMMAND_ENABLE; /* in 9630, 3AA has only OTF input */
