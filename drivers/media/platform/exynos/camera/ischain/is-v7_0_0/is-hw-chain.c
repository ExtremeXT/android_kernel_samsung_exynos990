...
#include "../../hardware/is-hw-mcscaler-v2.h"

static struct is_reg sysreg_isppre_regs[SYSREG_ISPPRE_REG_CNT] = {
...
static struct is_reg sysreg_isplp_regs[SYSREG_ISPLP_REG_CNT] = {
	{0x0404, "ISPLP_USER_CON"},
	{0x0408, "ISPLP_USER_CON2"},
};

static struct is_reg sysreg_isphq_regs[SYSREG_ISPHQ_REG_CNT] = {
	{0x0404, "ISPHQ_USER_CON"},
};

static struct is_field sysreg_isppre_fields[SYSREG_ISPPRE_REG_FIELD_CNT] = {
...
static struct is_field sysreg_isplp_fields[SYSREG_ISPLP_REG_FIELD_CNT] = {
...
static struct is_field sysreg_isphq_fields[SYSREG_ISPHQ_REG_FIELD_CNT] = {
...
const struct is_subdev_ops is_subdev_scc_ops;
const struct is_subdev_ops is_subdev_scp_ops;
const struct is_subdev_ops is_subdev_dis_ops;
const struct is_subdev_ops is_subdev_dxc_ops;
const struct is_subdev_ops is_subdev_dcp_ops;
const struct is_subdev_ops is_subdev_dcxs_ops;
const struct is_subdev_ops is_subdev_dcxc_ops;
...
void is_hw_group_init(struct is_group *group)
{
	int i;
...
void is_hw_camif_init(void)
{
	...
	is_hw_set_reg(sysreg_isppre_base,
...
int is_hw_camif_fix_up(struct is_device_sensor *sensor)
{
	...
	actual = is_hw_get_reg(sysreg_isppre_base, &sysreg_isppre_regs[pdp_mux]);
	expected = csi_ch;
	if (csi_ch == CSI_ID_E)
		expected = 3;
	...
	if (csi_ch != CSI_ID_D) {
		if (actual != expected) {
			is_hw_set_reg(sysreg_isppre_base, &sysreg_isppre_regs[pdp_mux], expected);
...
int is_hw_camif_cfg(void *sensor_data)
{
	...
	is_hw_set_reg(isppre_reg, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_PDP_IN_EN], 0xff);
...
int is_hw_ischain_enable(struct is_device_ischain *device)
{
	...
	isppre_val = is_hw_get_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_USER_CON]);
	isplp_val = is_hw_get_reg(isplp_regs, &sysreg_isplp_regs[SYSREG_R_ISPLP_USER_CON2]);
	isphq_val = is_hw_get_reg(isphq_regs, &sysreg_isplp_regs[SYSREG_R_ISPHQ_USER_CON]);

	isppre_backup = isppre_val;
	isplp_backup = isplp_val;
	isphq_backup = isphq_val;

	isppre_val = is_hw_set_field_value(isppre_val, &sysreg_isppre_fields[SYSREG_F_ENABLE_OTF_IN_DSPMISPPRE], 0);
	isppre_val = is_hw_set_field_value(isppre_val, &sysreg_isppre_fields[SYSREG_F_ENABLE_OTF_OUT_ISPPREDSPM], 0);
#ifdef CONFIG_SOC_EXYNOS9820_EVT0
	isppre_val = is_hw_set_field_value(isppre_val, &sysreg_isppre_fields[SYSREG_F_ENABLE_OTF_OUT_ISPPREISPLP], 1);
	isppre_val = is_hw_set_field_value(isppre_val, &sysreg_isppre_fields[SYSREG_F_ENABLE_OTF_OUT_ISPPREISPHQ], 1);

	isplp_val = is_hw_set_field_value(isplp_val, &sysreg_isplp_fields[SYSREG_F_ENABLE_OTF_IN_ISPPREISPLP], 1);
	isplp_val = is_hw_set_field_value(isplp_val, &sysreg_isplp_fields[SYSREG_F_ENABLE_OTF_IN_ISPHQISPLP], 1);
	isplp_val = is_hw_set_field_value(isplp_val, &sysreg_isplp_fields[SYSREG_F_ENABLE_OTF_IN_CIPISPLP], 0);
#endif
	isplp_val = is_hw_set_field_value(isplp_val, &sysreg_isplp_fields[SYSREG_F_ENABLE_OTF_IN_DSPMISPLP], 0);
	isplp_val = is_hw_set_field_value(isplp_val, &sysreg_isplp_fields[SYSREG_F_ENABLE_OTF_OUT_ISPLPDSPM], 0);

#ifdef CONFIG_SOC_EXYNOS9820_EVT0
	isphq_val = is_hw_set_field_value(isphq_val, &sysreg_isphq_fields[SYSREG_F_ENABLE_OTF_IN_ISPPREISPHQ], 1);
	isphq_val = is_hw_set_field_value(isphq_val, &sysreg_isphq_fields[SYSREG_F_ENABLE_OTF_OUT_ISPHQISPLP], 1);
#endif
	isphq_val = is_hw_set_field_value(isphq_val, &sysreg_isphq_fields[SYSREG_F_ENABLE_OTF_OUT_ISPHQDSPM], 0);

	if (isppre_backup != isppre_val) {
		minfo("SYSREG_R_ISPPRE_USER_CON:(0x%08X)->(0x%08X)\n", device, isppre_backup, isppre_val);
		is_hw_set_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_USER_CON], isppre_val);
	}

	if (isplp_backup != isplp_val) {
		minfo("SYSREG_R_ISPLP_USER_CON2:(0x%08X)->(0x%08X)\n", device, isplp_backup, isplp_val);
		is_hw_set_reg(isplp_regs, &sysreg_isplp_regs[SYSREG_R_ISPLP_USER_CON2], isplp_val);
	}

	if (isphq_backup != isphq_val) {
		minfo("SYSREG_R_ISPHQ_USER_CON:(0x%08X)->(0x%08X)\n", device, isphq_backup, isphq_val);
		is_hw_set_reg(isphq_regs, &sysreg_isphq_regs[SYSREG_R_ISPHQ_USER_CON], isphq_val);
	}
...
int is_hw_ischain_cfg(void *ischain_data)
{
	 ...
	 *    SCORE : 1
	 */
	isppre_val0 = is_hw_get_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON14]);
	isppre_val1 = is_hw_get_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON13]);
	isppre_backup0 = isppre_val0;
	isppre_backup1 = isppre_val1;

	isppre_val0 = is_hw_set_field_value(isppre_val0, &sysreg_isppre_fields[SYSREG_F_GLUEMUX_3AA0_VAL], 0);
	isppre_val1 = is_hw_set_field_value(isppre_val1, &sysreg_isppre_fields[SYSREG_F_GLUEMUX_3AA1_VAL], 0);
	...
	if (isppre_backup0 != isppre_val0 || isppre_backup1 != isppre_val1) {
		...
		is_hw_set_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON14], isppre_val0);

		minfo("SYSREG_R_ISPPRE_SC_CON13:(0x%08X)->(0x%08X)\n", device, isppre_backup1, isppre_val1);
		is_hw_set_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON13], isppre_val1);
	}
	...
	isppre_val0 = is_hw_get_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON16]);
	isppre_val1 = is_hw_get_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON15]);
	isppre_backup0 = isppre_val0;
	isppre_backup1 = isppre_val1;

	isppre_val0 = is_hw_set_field_value(isppre_val0, &sysreg_isppre_fields[SYSREG_F_GLUEMUX_ISPLP_VAL], input_isp0);
	isppre_val1 = is_hw_set_field_value(isppre_val1, &sysreg_isppre_fields[SYSREG_F_GLUEMUX_ISPHQ_VAL], input_isp1);

	if (isppre_backup0 != isppre_val0 || isppre_backup1 != isppre_val1) {
		minfo("SYSREG_R_ISPPRE_SC_CON16:(0x%08X)->(0x%08X)\n", device, isppre_backup0, isppre_val0);
		is_hw_set_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON16], isppre_val0);

		minfo("SYSREG_R_ISPPRE_SC_CON15:(0x%08X)->(0x%08X)\n", device, isppre_backup1, isppre_val1);
		is_hw_set_reg(isppre_regs, &sysreg_isppre_regs[SYSREG_R_ISPPRE_SC_CON15], isppre_val1);
	}
	 ...
	 *    Score -> MCSC1 : 3
	 */
	isplp_val = is_hw_get_reg(isplp_regs, &sysreg_isplp_regs[SYSREG_R_ISPLP_USER_CON]);
	isplp_backup = isplp_val;

	input_mcsc_src0 = 0;
	input_mcsc_src1 = 0;
	/* It will be changed in is_hardware_shot */
	isplp_val = is_hw_set_field_value(isplp_val,
	...
	if (isplp_backup != isplp_val) {
		...
		is_hw_set_reg(isplp_regs, &sysreg_isplp_regs[SYSREG_R_ISPLP_USER_CON], isplp_val);	/* MCSC input */
	}

	/* 6) Write enable register: EVT0 */
	ret = is_hw_ischain_enable(device);
...
static inline void is_isr1_ddk(void *data)
{
...
static inline void is_isr2_ddk(void *data)
{
	struct is_interface_hwip *itf_hw = NULL;
	struct hwip_intr_handler *intr_hw = NULL;
...
static inline void is_isr1_host(void *data)
{
	struct is_interface_hwip *itf_hw = NULL;
	struct hwip_intr_handler *intr_hw = NULL;
...
static irqreturn_t is_isr1_3aa0(int irq, void *data)
{
	is_isr1_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr2_3aa0(int irq, void *data)
{
	is_isr2_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_3aa1(int irq, void *data)
{
	is_isr1_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr2_3aa1(int irq, void *data)
{
	is_isr2_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_vpp(int irq, void *data)
{
	is_isr1_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr2_vpp(int irq, void *data)
{
	is_isr2_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_isp0(int irq, void *data)
{
	is_isr1_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr2_isp0(int irq, void *data)
{
	is_isr2_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_isp1(int irq, void *data)
{
	is_isr1_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr2_isp1(int irq, void *data)
{
	is_isr2_ddk(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_mcs0(int irq, void *data)
{
	is_isr1_host(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_mcs1(int irq, void *data)
{
	is_isr1_host(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_vra(int irq, void *data)
{
	struct is_interface_hwip *itf_hw = NULL;
	struct hwip_intr_handler *intr_hw = NULL;
...
static irqreturn_t is_isr1_paf0(int irq, void *data)
{
	is_isr1_host(data);
	return IRQ_HANDLED;
}

static irqreturn_t is_isr1_paf1(int irq, void *data)
{
	is_isr1_host(data);
	return IRQ_HANDLED;
}
	...
	const char *name, int isr_num, irqreturn_t (*func)(int, void *))
{
	...
	snprintf(itf_hwip->irq_name[isr_num], name_len, "is%s-%d", name, isr_num);
...
int is_hw_request_irq(void *itfc_data, int hw_id)
{
	...
	switch (hw_id) {
	case DEV_HW_3AA0:
		ret = __is_hw_request_irq(itf_hwip, "3aa0", INTR_HWIP1, is_isr1_3aa0);
		ret = __is_hw_request_irq(itf_hwip, "3aa0", INTR_HWIP2, is_isr2_3aa0);
		break;
	case DEV_HW_3AA1:
		ret = __is_hw_request_irq(itf_hwip, "3aa1", INTR_HWIP1, is_isr1_3aa1);
		ret = __is_hw_request_irq(itf_hwip, "3aa1", INTR_HWIP2, is_isr2_3aa1);
		break;
	case DEV_HW_ISP0:
		ret = __is_hw_request_irq(itf_hwip, "isp0", INTR_HWIP1, is_isr1_isp0);
		ret = __is_hw_request_irq(itf_hwip, "isp0", INTR_HWIP2, is_isr2_isp0);
		break;
	case DEV_HW_ISP1:
		ret = __is_hw_request_irq(itf_hwip, "isp1", INTR_HWIP1, is_isr1_isp1);
		ret = __is_hw_request_irq(itf_hwip, "isp1", INTR_HWIP2, is_isr2_isp1);
		break;
	case DEV_HW_MCSC0:
		ret = __is_hw_request_irq(itf_hwip, "mcs0", INTR_HWIP1, is_isr1_mcs0);
		break;
	case DEV_HW_MCSC1:
		ret = __is_hw_request_irq(itf_hwip, "mcs1", INTR_HWIP1, is_isr1_mcs1);
		break;
	case DEV_HW_VRA:
		ret = __is_hw_request_irq(itf_hwip, "vra", INTR_HWIP2, is_isr1_vra);
		break;
	case DEV_HW_PAF0:
		ret = __is_hw_request_irq(itf_hwip, "paf0", INTR_HWIP1, is_isr1_paf0);
		break;
	case DEV_HW_PAF1:
		ret = __is_hw_request_irq(itf_hwip, "paf1", INTR_HWIP1, is_isr1_paf1);
		break;
	case DEV_HW_3AA2:
		ret = __is_hw_request_irq(itf_hwip, "vpp", INTR_HWIP1, is_isr1_vpp);
		ret = __is_hw_request_irq(itf_hwip, "vpp", INTR_HWIP2, is_isr2_vpp);
		break;
	default:
		probe_err("hw_id(%d) is invalid", hw_id);
		return -EINVAL;
...
u32 is_hw_find_settle(u32 mipi_speed, u32 use_cphy)
{
	...
	if (is_csi_settle_table[s] < align_mipi_speed)
		return is_csi_settle_table[s + 1];

	if (is_csi_settle_table[e] > align_mipi_speed)
		return is_csi_settle_table[e + 1];

	/* Binary search */
	while (s <= e) {
		m = ALIGN((s + e) / 2, 2);
		find_mipi_speed = is_csi_settle_table[m];
	...
	return is_csi_settle_table[m + 1];
}
...
void is_hw_djag_get_input(struct is_device_ischain *ischain, u32 *djag_in)
{
	...
	reprocessing = test_bit(IS_ISCHAIN_REPROCESSING, &ischain->state);
