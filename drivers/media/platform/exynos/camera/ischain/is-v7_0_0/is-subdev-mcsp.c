	...
	u32 *indexes)
{
	...
	if (queue->framecfg.quantization == V4L2_QUANTIZATION_FULL_RANGE) {
		...
		if (scenario_id != IS_SN_VIDEO_HIGH_SPEED_120FPS && scenario_id != IS_SN_VIDEO_HIGH_SPEED_240FPS) {
			msinfo("CRange:N\n", device, subdev);
		}
	}
	...
	u32 *indexes)
{
	...
	if ((index - PARAM_MCS_OUTPUT0) != MCSC_OUTPUT_DS)
		is_ischain_mxp_compare_size(device, mcs_param, incrop);
	...
	struct camera2_node *node)
{
	...
	if (node->request) {
		...
		if (!test_bit(IS_GROUP_OTF_INPUT, &head->state) && (head->asyn_shots == 1)) {
