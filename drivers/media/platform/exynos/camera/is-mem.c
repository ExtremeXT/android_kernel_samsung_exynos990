...
static long is_dbufcon_prepare(struct is_vb2_buf *vbuf,
		u32 num_planes, struct device *dev)
{
	...
	for (p = 0; p < num_planes; p++) {
		...
		/* IS_MAX_PLANES includes a meta-plane */
		if ((count * num_planes) >= IS_MAX_PLANES) {
			err("out of range plane. count: %d, planes: %d >= supported: %d",
					count, num_planes, IS_MAX_PLANES);
			ret = -EINVAL;
