...
void is_hw_mcsc_tdnr_init(struct is_hw_ip *hw_ip,
	struct mcs_param *mcs_param, u32 instance)
{
	...
	is_scaler_clear_tdnr_rdma_addr(hw_ip->regs[REG_SETA], TDNR_IMAGE);
	is_scaler_clear_tdnr_wdma_addr(hw_ip->regs[REG_SETA], TDNR_IMAGE);

	memcpy(&hw_mcsc->tdnr_cfgs, &init_tdnr_cfgs, sizeof(struct tdnr_configs));
#endif
}

void is_hw_mcsc_tdnr_deinit(struct is_hw_ip *hw_ip,
	struct mcs_param *mcs_param, u32 instance)
{
	struct is_hw_mcsc *hw_mcsc;
	struct is_hw_mcsc_cap *cap;
	...
	is_scaler_set_tdnr_mode_select(hw_ip->regs[REG_SETA], TDNR_MODE_BYPASS);

	is_scaler_clear_tdnr_rdma_addr(hw_ip->regs[REG_SETA], TDNR_IMAGE);
	is_scaler_clear_tdnr_rdma_addr(hw_ip->regs[REG_SETA], TDNR_WEIGHT);

	is_scaler_set_tdnr_wdma_enable(hw_ip->regs[REG_SETA], TDNR_WEIGHT, false);
	is_scaler_clear_tdnr_wdma_addr(hw_ip->regs[REG_SETA], TDNR_WEIGHT);

	hw_mcsc->cur_tdnr_mode = TDNR_MODE_BYPASS;
}

static int is_hw_mcsc_check_tdnr_mode_pre(struct is_hw_ip *hw_ip,
...
static void is_hw_mcsc_check_tdnr_mode_post(struct is_hw_ip *hw_ip,
	struct is_group *head,
...
static int is_hw_mcsc_cfg_tdnr_rdma(struct is_hw_ip *hw_ip,
	struct is_group *head,
	struct is_frame *frame,
	struct mcs_param *mcs_param,
	enum tdnr_mode tdnr_mode)
{
	...
	is_scaler_get_tdnr_wdma_addr(hw_ip->regs[REG_SETA], TDNR_IMAGE,
			&prev_wdma_addr, NULL, NULL, USE_DMA_BUFFER_INDEX);
	is_scaler_set_tdnr_rdma_addr(hw_ip->regs[REG_SETA], TDNR_IMAGE,
	...
	/* 2DNR -> 3DNR */
	is_scaler_get_tdnr_wdma_size(hw_ip->regs[REG_SETA],
			TDNR_IMAGE, &prev_width, &prev_height);
	is_scaler_get_tdnr_wdma_stride(hw_ip->regs[REG_SETA],
	...
	is_scaler_set_tdnr_rdma_size(hw_ip->regs[REG_SETA],
			TDNR_IMAGE, prev_width, prev_height);
	is_scaler_set_tdnr_rdma_stride(hw_ip->regs[REG_SETA],
			TDNR_IMAGE, prev_y_stride, 0);

	return ret;
}

static int is_hw_mcsc_cfg_tdnr_wdma(struct is_hw_ip *hw_ip,
	struct is_group *head,
	struct is_frame *frame,
	struct mcs_param *mcs_param,
	enum tdnr_mode tdnr_mode)
{
	...
	is_scaler_set_tdnr_wdma_addr(hw_ip->regs[REG_SETA], TDNR_IMAGE,
	...
	is_scaler_set_tdnr_wdma_size(hw_ip->regs[REG_SETA],
			TDNR_IMAGE, wdma_width, wdma_height);

	/* TDNR DMA stride = image size * 2 */
	img_y_stride = wdma_width * 2;
	is_scaler_set_tdnr_wdma_stride(hw_ip->regs[REG_SETA],
...
static int is_hw_mcsc_cfg_tdnr_tuning_param(struct is_hw_ip *hw_ip,
	struct is_group *head,
	struct is_frame *frame,
	enum tdnr_mode *tdnr_mode,
	bool start_flag)
...
config:
	...
	if (start_flag) {
		is_scaler_set_tdnr_tuneset_general(hw_ip->regs[REG_SETA],
			tdnr_cfgs.general_cfg);
		is_scaler_set_tdnr_tuneset_constant_lut_coeffs(hw_ip->regs[REG_SETA],
			tdnr_cfgs.constant_lut_coeffs);
		is_scaler_set_tdnr_tuneset_refine_control(hw_ip->regs[REG_SETA],
			tdnr_cfgs.refine_cfg);
	}

	/* update tuning SFR (include NI depended cfg) */
	is_scaler_set_tdnr_tuneset_yuvtable(hw_ip->regs[REG_SETA], tdnr_cfgs.yuv_tables);
	is_scaler_set_tdnr_tuneset_temporal(hw_ip->regs[REG_SETA],
		tdnr_cfgs.temporal_dep_cfg, tdnr_cfgs.temporal_indep_cfg);
	is_scaler_set_tdnr_tuneset_regional_feature(hw_ip->regs[REG_SETA],
		tdnr_cfgs.regional_dep_cfg, tdnr_cfgs.regional_indep_cfg);
	is_scaler_set_tdnr_tuneset_spatial(hw_ip->regs[REG_SETA],
...
int is_hw_mcsc_update_tdnr_register(struct is_hw_ip *hw_ip,
	struct is_frame *frame,
	...
	bool start_flag)
{
	...
	tdnr_mode = is_hw_mcsc_check_tdnr_mode_pre(hw_ip, head,
	...
	if (test_bit(IS_GROUP_OTF_INPUT, &head->state))
		...
		hw_mcsc->yic_en = TDNR_YIC_ENABLE;
	is_scaler_set_tdnr_yic_ctrl(hw_ip->regs[REG_SETA], hw_mcsc->yic_en);

	is_scaler_set_tdnr_image_size(hw_ip->regs[REG_SETA],
	...
	case TDNR_MODE_2DNR:
	case TDNR_MODE_3DNR:
		ret = is_hw_mcsc_cfg_tdnr_rdma(hw_ip, head, frame, mcs_param, tdnr_mode);
		if (ret) {
			err_hw("[ID:%d] tdnr rdma cfg failed", hw_ip->id);
			break;
		}

		ret = is_hw_mcsc_cfg_tdnr_wdma(hw_ip, head, frame, mcs_param, tdnr_mode);
		if (ret) {
			err_hw("[ID:%d] tdnr wdma cfg failed", hw_ip->id);
			break;
		}

		ret = is_hw_mcsc_cfg_tdnr_tuning_param(hw_ip, head, frame, &tdnr_mode, start_flag);
		if (ret) {
	...
	is_hw_mcsc_check_tdnr_mode_post(hw_ip, head,
			hw_mcsc->cur_tdnr_mode, &tdnr_mode);

tdnr_mode_select:
	is_scaler_set_tdnr_mode_select(hw_ip->regs[REG_SETA], tdnr_mode);
	hw_mcsc->cur_tdnr_mode = tdnr_mode;

	return 0;
}

int is_hw_mcsc_recovery_tdnr_register(struct is_hw_ip *hw_ip,
