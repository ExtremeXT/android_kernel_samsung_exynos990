 ...
 */
#include "is-hw-api-mcscaler-v2.h"
#include "sfr/is-sfr-mcsc-v5_0.h"
#include "is-hw.h"
...
#define WDMADS_SRAM_BASE_VALUE	(680)	/* max 768 */
...
void is_scaler_set_poly_scaler_enable(void __iomem *base_addr, u32 hw_id, u32 output_id, u32 enable)
{
	...
	case MCSC_OUTPUT4:
		if (enable)
			is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_CTRL],
				&mcsc_fields[MCSC_F_SC4_INPUT_SEL], input_source);
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_CTRL], &mcsc_fields[MCSC_F_SC4_ENABLE], enable);
		break;
	default:
		break;
	}
...
void is_scaler_set_poly_scaler_bypass(void __iomem *base_addr, u32 output_id, u32 bypass)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_CTRL], &mcsc_fields[MCSC_F_SC4_BYPASS], bypass);
		break;
	default:
		break;
	}
	...
	u32 pos_x, u32 pos_y, u32 width, u32 height)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_SC4_SRC_HPOS], pos_x);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_SC4_SRC_VPOS], pos_y);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_SRC_POS], reg_val);

		reg_val = 0;
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_SC4_SRC_HSIZE], width);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_SC4_SRC_VSIZE], height);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_SRC_SIZE], reg_val);
		break;
	default:
		break;
	}
...
void is_scaler_get_poly_src_size(void __iomem *base_addr, u32 output_id, u32 *width, u32 *height)
{
	...
	case MCSC_OUTPUT3:
		...
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_SC3_SRC_SIZE],
		...
		break;
	case MCSC_OUTPUT4:
		*width = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_SC4_SRC_SIZE],
			&mcsc_fields[MCSC_F_SC4_SRC_HSIZE]);
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_SC4_SRC_SIZE],
...
void is_scaler_set_poly_dst_size(void __iomem *base_addr, u32 output_id, u32 width, u32 height)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_SC4_DST_HSIZE], width);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_SC4_DST_VSIZE], height);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_DST_SIZE], reg_val);
		break;
	default:
		break;
	}
...
void is_scaler_get_poly_dst_size(void __iomem *base_addr, u32 output_id, u32 *width, u32 *height)
{
	...
	case MCSC_OUTPUT3:
		...
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_SC3_DST_SIZE],
		...
		break;
	case MCSC_OUTPUT4:
		*width = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_SC4_DST_SIZE],
			&mcsc_fields[MCSC_F_SC4_DST_HSIZE]);
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_SC4_DST_SIZE],
...
void is_scaler_set_poly_scaling_ratio(void __iomem *base_addr, u32 output_id, u32 hratio, u32 vratio)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_H_RATIO],
			&mcsc_fields[MCSC_F_SC4_H_RATIO], hratio);
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_V_RATIO],
...
void is_scaler_set_h_init_phase_offset(void __iomem *base_addr, u32 output_id, u32 h_offset)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_H_INIT_PHASE_OFFSET],
...
void is_scaler_set_v_init_phase_offset(void __iomem *base_addr, u32 output_id, u32 v_offset)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_V_INIT_PHASE_OFFSET],
...
void is_scaler_set_poly_scaler_h_coef(void __iomem *base_addr, u32 output_id, u32 h_sel)
{
	...
	case MCSC_OUTPUT4:
		for (index = 0; index <= 8; index++) {
			...
			is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_H_COEFF_0AB + (4 * index)], reg_val);
			reg_val = 0;
			reg_val = is_hw_set_field_value(reg_val,
			...
			is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_H_COEFF_0CD + (4 * index)], reg_val);
			reg_val = 0;
			reg_val = is_hw_set_field_value(reg_val,
			...
			is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_H_COEFF_0EF + (4 * index)], reg_val);
			reg_val = 0;
			reg_val = is_hw_set_field_value(reg_val,
			...
			is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_H_COEFF_0GH + (4 * index)], reg_val);
		}
		break;
	default:
		break;
...
void is_scaler_set_poly_scaler_v_coef(void __iomem *base_addr, u32 output_id, u32 v_sel)
{
	...
	case MCSC_OUTPUT4:
		for (index = 0; index <= 8; index++) {
			...
			is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_V_COEFF_0AB + (2 * index)], reg_val);
			reg_val = 0;
			reg_val = is_hw_set_field_value(reg_val,
			...
			is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_SC4_V_COEFF_0CD + (2 * index)], reg_val);
		}
		break;
	default:
		break;
...
void is_scaler_set_poly_round_mode(void __iomem *base_addr, u32 output_id, u32 mode)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_SC4_ROUND_MODE],
...
void is_scaler_set_420_conversion(void __iomem *base_addr, u32 output_id, u32 conv420_weight, u32 conv420_en)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_PC4_CONV420_WEIGHT],
			&mcsc_fields[MCSC_F_PC4_CONV420_WEIGHT], conv420_weight);
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_PC4_CONV420_CTRL],
...
void is_scaler_set_bchs_enable(void __iomem *base_addr, u32 output_id, bool bchs_en)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_PC4_BCHS_CTRL],
			&mcsc_fields[MCSC_F_PC4_BCHS_ENABLE], bchs_en);

		/* default BCHS clamp value */
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_PC4_BCHS_CLAMP_Y], 0x03FF0000);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_PC4_BCHS_CLAMP_C], 0x03FF0000);
		break;
	default:
		break;
	}
...
void is_scaler_set_b_c(void __iomem *base_addr, u32 output_id, u32 y_offset, u32 y_gain)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_PC4_BCHS_Y_OFFSET], y_offset);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_PC4_BCHS_Y_GAIN], y_gain);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_PC4_BCHS_BC], reg_val);
		break;
	default:
		break;
	}
	...
	u32 c_gain00, u32 c_gain01, u32 c_gain10, u32 c_gain11)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_PC4_BCHS_C_GAIN_00], c_gain00);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_PC4_BCHS_C_GAIN_01], c_gain01);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_PC4_BCHS_HS1], reg_val);
		reg_val = 0;
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_PC4_BCHS_C_GAIN_10], c_gain10);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_PC4_BCHS_C_GAIN_11], c_gain11);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_PC4_BCHS_HS2], reg_val);
		break;
	default:
		break;
	}
	...
	u32 y_max, u32 y_min, u32 c_max, u32 c_min)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val_y = is_hw_set_field_value(reg_val_y, &mcsc_fields[MCSC_F_PC4_BCHS_Y_CLAMP_MAX], y_max);
		reg_val_y = is_hw_set_field_value(reg_val_y, &mcsc_fields[MCSC_F_PC4_BCHS_Y_CLAMP_MIN], y_min);
		reg_val_c = is_hw_set_field_value(reg_val_c, &mcsc_fields[MCSC_F_PC4_BCHS_C_CLAMP_MAX], c_max);
		reg_val_c = is_hw_set_field_value(reg_val_c, &mcsc_fields[MCSC_F_PC4_BCHS_C_CLAMP_MIN], c_min);
...
void is_scaler_set_dma_out_enable(void __iomem *base_addr, u32 output_id, bool dma_out_en)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_PC4_DMA_OUT_CTRL],
			&mcsc_fields[MCSC_F_PC4_DMA_OUT_ENABLE], (u32)dma_out_en);
		break;
	case MCSC_OUTPUT_DS:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_DMA_OUT_CTRL],
...
void is_scaler_set_wdma_sram_base(void __iomem *base_addr, u32 output_id)
{
	switch (output_id) {
	case MCSC_OUTPUT0:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA0_SRAM_BASE],
			&mcsc_fields[MCSC_F_WDMA0_SRAM_BASE], WDMA0_SRAM_BASE_VALUE);
		break;
	case MCSC_OUTPUT1:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA1_SRAM_BASE],
			&mcsc_fields[MCSC_F_WDMA1_SRAM_BASE], WDMA1_SRAM_BASE_VALUE);
		break;
	case MCSC_OUTPUT2:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA2_SRAM_BASE],
			&mcsc_fields[MCSC_F_WDMA2_SRAM_BASE], WDMA2_SRAM_BASE_VALUE);
		break;
	case MCSC_OUTPUT3:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_SRAM_BASE],
			&mcsc_fields[MCSC_F_WDMA3_SRAM_BASE], WDMA3_SRAM_BASE_VALUE);
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_SRAM_BASE],
			&mcsc_fields[MCSC_F_WDMA4_SRAM_BASE], WDMA4_SRAM_BASE_VALUE);
		break;
	case MCSC_OUTPUT_DS:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_SRAM_BASE],
...
u32 is_scaler_get_dma_out_enable(void __iomem *base_addr, u32 output_id)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		ret = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_PC4_DMA_OUT_CTRL],
			&mcsc_fields[MCSC_F_PC4_DMA_OUT_ENABLE]);
		break;
	case MCSC_OUTPUT_DS:
		ret = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_DS_DMA_OUT_CTRL],
...
{
	/* 0 ~ 4 : otf out path */
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_CTRL],
		&mcsc_fields[MCSC_F_DS_CHAIN_SEL], output_id);
}
...
u32 is_scaler_get_rdma_format(void __iomem *base_addr, u32 hw_id)
{
	...
	if (mono)
		...
		fmt = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_DATA_FORMAT],
			&mcsc_fields[MCSC_F_RDMAOTF_DATA_FORMAT]);
...
void is_scaler_set_rdma_format(void __iomem *base_addr, u32 hw_id, u32 in_fmt)
{
	...
	if (in_fmt == MCSC_MONO_Y8) {
		in_fmt = MCSC_YUV422_2P_UFIRST;
		en = 1;
	}

	is_hw_set_field(base_addr, &mcsc_regs[reg_offset], &mcsc_fields[field_offset], en);
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_DATA_FORMAT],
		&mcsc_fields[MCSC_F_RDMAOTF_DATA_FORMAT], in_fmt);
}
...
void is_scaler_set_rdma_10bit_type(void __iomem *base_addr, u32 dma_in_10bit_type)
{
	...
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_10BIT_TYPE],
		&mcsc_fields[MCSC_F_RDMAOTF_10BIT_TYPE], dma_in_10bit_type);
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_DITHER],
...
void is_scaler_set_wdma_mono_ctrl(void __iomem *base_addr, u32 output_id, u32 fmt, bool en)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_MONO_EN], en);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA4_MONO_CTRL], reg_val);
		break;
	case MCSC_OUTPUT_DS:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_MONO_EN], en);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMADS_MONO_CTRL], reg_val);
		break;
	default:
		break;
	}
...
void is_scaler_set_wdma_rgb_ctrl(void __iomem *base_addr, u32 output_id, u32 img_fmt, bool en)
{
	u32 reg_val = 0;
	u32 hw_fmt = img_fmt - MCSC_RGB_ARGB8888;

	switch (output_id) {
	case MCSC_OUTPUT0:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA0_RGB_EN], en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA0_RGB_FORMAT], hw_fmt);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA0_RGB_CTRL], reg_val);
		break;
	case MCSC_OUTPUT1:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA1_RGB_EN], en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA1_RGB_FORMAT], hw_fmt);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA1_RGB_CTRL], reg_val);
		break;
	case MCSC_OUTPUT2:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA2_RGB_EN], en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA2_RGB_FORMAT], hw_fmt);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA2_RGB_CTRL], reg_val);
		break;
	case MCSC_OUTPUT3:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA3_RGB_EN], en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA3_RGB_FORMAT], hw_fmt);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA3_RGB_CTRL], reg_val);
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_RGB_EN], en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_RGB_FORMAT], hw_fmt);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA4_RGB_CTRL], reg_val);
		break;
	case MCSC_OUTPUT_DS:
		/* Not support */
		break;
...
void is_scaler_set_wdma_rgb_coefficient(void __iomem *base_addr, u32 alpha_value)
{
	u32 reg_val = 0;

	/* this value is valid only YUV422 only.  601 wide coefficient is used in JPEG, Pictures, preview. */
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_ALPHA], alpha_value);
	...
	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C00], 256);
	...
	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C20], 369);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C01], 256);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA_RGB_COEF_1], reg_val);
	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C11], 32727);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C21], 32625);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA_RGB_COEF_2], reg_val);
	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C02], 256);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RGB_COEF_C12], 471);
...
void is_scaler_set_wdma_format(void __iomem *base_addr, u32 hw_id, u32 output_id, u32 out_fmt)
{
	u32 in_fmt;

	in_fmt = is_scaler_get_rdma_format(base_addr, hw_id);
	...
	case MCSC_RGB_ABGR8888:
		is_scaler_set_wdma_rgb_ctrl(base_addr, output_id, out_fmt, true);
		is_scaler_set_wdma_rgb_coefficient(base_addr, 0);	/* alpha is zero */
		out_fmt = MCSC_YUV422_1P_YUYV;
		break;
	case MCSC_MONO_Y8:
		is_scaler_set_wdma_rgb_ctrl(base_addr, output_id, MCSC_RGB_ARGB8888, false);
		is_scaler_set_wdma_mono_ctrl(base_addr, output_id, out_fmt, true);
		out_fmt = MCSC_YUV422_2P_UFIRST;
		break;
	default:
		break;
	}
	...
	case MCSC_OUTPUT2:
		...
		break;
	case MCSC_OUTPUT3:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_DATA_FORMAT],
			&mcsc_fields[MCSC_F_WDMA3_DATA_FORMAT], out_fmt);
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_DATA_FORMAT],
			&mcsc_fields[MCSC_F_WDMA4_DATA_FORMAT], out_fmt);
		break;
	case MCSC_OUTPUT_DS:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_DATA_FORMAT],
			&mcsc_fields[MCSC_F_WDMADS_DATA_FORMAT], out_fmt);
		break;
	default:
		break;
	}
...
void is_scaler_set_wdma_10bit_type(void __iomem *base_addr, u32 output_id,
	u32 fmt, u32 bitwidth, enum exynos_sensor_position sensor_position)
{
	u32 dither_en = 0;
	u32 round_en = 0;
	u32 img_10bit_type;
	...
	case DMA_OUTPUT_BIT_WIDTH_10BIT:
		img_10bit_type = 1; /* 8+2 bit mode  */
		break;
	case DMA_OUTPUT_BIT_WIDTH_16BIT:
		img_10bit_type = 2; /* P201 or P010 mode */
		break;
	default:
		img_10bit_type = 0; /* 8-bit mode */
		break;
	}

	if ((fmt == DMA_OUTPUT_FORMAT_RGB) || (output_id == MCSC_OUTPUT_DS)) {
	...
	switch (output_id) {
	case MCSC_OUTPUT0:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA0_10BIT_TYPE],
		...
		break;
	case MCSC_OUTPUT1:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA1_10BIT_TYPE],
		...
		break;
	case MCSC_OUTPUT2:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA2_10BIT_TYPE],
		...
		break;
	case MCSC_OUTPUT3:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_10BIT_TYPE],
			&mcsc_fields[MCSC_F_WDMA3_10BIT_TYPE], img_10bit_type);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA3_DITHER_EN], dither_en);
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_10BIT_TYPE],
			&mcsc_fields[MCSC_F_WDMA4_10BIT_TYPE], img_10bit_type);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_DITHER_EN], dither_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_ROUND_EN], round_en);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA4_DITHER], reg_val);
		break;
	case MCSC_OUTPUT_DS:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_DITHER_EN], dither_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_ROUND_EN], round_en);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMADS_DITHER], reg_val);
		break;
	default:
		break;
	}
...
void is_scaler_get_wdma_format(void __iomem *base_addr, u32 output_id, u32 *out_fmt)
{
	switch (output_id) {
	case MCSC_OUTPUT0:
	...
	case MCSC_OUTPUT2:
		*out_fmt = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA2_DATA_FORMAT],
		...
		break;
	case MCSC_OUTPUT3:
		*out_fmt = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_DATA_FORMAT],
			&mcsc_fields[MCSC_F_WDMA3_DATA_FORMAT]);
		break;
	case MCSC_OUTPUT4:
		*out_fmt = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_DATA_FORMAT],
			&mcsc_fields[MCSC_F_WDMA4_DATA_FORMAT]);
		break;
	case MCSC_OUTPUT_DS:
		*out_fmt = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_DATA_FORMAT],
...
void is_scaler_set_swap_mode(void __iomem *base_addr, u32 output_id, u32 swap)
{
	...
	case MCSC_OUTPUT0:
		...
		break;
	case MCSC_OUTPUT1:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA1_SWAP_TABLE],
			&mcsc_fields[MCSC_F_WDMA1_SWAP_TABLE], swap);
		break;
	case MCSC_OUTPUT2:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA2_SWAP_TABLE],
			&mcsc_fields[MCSC_F_WDMA2_SWAP_TABLE], swap);
		break;
	case MCSC_OUTPUT3:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_SWAP_TABLE],
			&mcsc_fields[MCSC_F_WDMA3_SWAP_TABLE], swap);
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_SWAP_TABLE],
			&mcsc_fields[MCSC_F_WDMA4_SWAP_TABLE], swap);
		break;
	case MCSC_OUTPUT_DS:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_SWAP_TABLE],
...
void is_scaler_set_flip_mode(void __iomem *base_addr, u32 output_id, u32 flip)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_FLIP_CONTROL],
...
void is_scaler_set_rdma_size(void __iomem *base_addr, u32 width, u32 height)
{
	...
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RDMAOTF_WIDTH], width);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RDMAOTF_HEIGHT], height);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_IMG_SIZE], reg_val);
}

void is_scaler_get_rdma_size(void __iomem *base_addr, u32 *width, u32 *height)
{
	*width = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_IMG_SIZE],
		&mcsc_fields[MCSC_F_RDMAOTF_WIDTH]);
	*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_IMG_SIZE],
...
void is_scaler_set_wdma_size(void __iomem *base_addr, u32 output_id, u32 width, u32 height)
{
	...
	case MCSC_OUTPUT3:
		...
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_WIDTH], width);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_HEIGHT], height);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA4_IMG_SIZE], reg_val);
		break;
	case MCSC_OUTPUT_DS:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_WIDTH], width);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_HEIGHT], height);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMADS_IMG_SIZE], reg_val);
		break;
	default:
		break;
	}
...
void is_scaler_get_wdma_size(void __iomem *base_addr, u32 output_id, u32 *width, u32 *height)
{
	...
	case MCSC_OUTPUT3:
		...
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_IMG_SIZE],
		...
		break;
	case MCSC_OUTPUT4:
		*width = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_IMG_SIZE],
			&mcsc_fields[MCSC_F_WDMA4_WIDTH]);
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_IMG_SIZE],
			&mcsc_fields[MCSC_F_WDMA4_HEIGHT]);
		break;
	case MCSC_OUTPUT_DS:
		*width = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_IMG_SIZE],
			&mcsc_fields[MCSC_F_WDMADS_WIDTH]);
		*height = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_IMG_SIZE],
...
void is_scaler_set_rdma_stride(void __iomem *base_addr, u32 y_stride, u32 uv_stride)
{
	...
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RDMAOTF_Y_STRIDE], y_stride);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RDMAOTF_C_STRIDE], uv_stride);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_STRIDE], reg_val);
}
...
void is_scaler_set_rdma_2bit_stride(void __iomem *base_addr, u32 y_2bit_stride, u32 uv_2bit_stride)
{
	...
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RDMAOTF_2BIT_Y_STRIDE], y_2bit_stride);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_RDMAOTF_2BIT_C_STRIDE], uv_2bit_stride);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_2BIT_STRIDE], reg_val);
}

void is_scaler_get_rdma_stride(void __iomem *base_addr, u32 *y_stride, u32 *uv_stride)
{
	*y_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_STRIDE],
		&mcsc_fields[MCSC_F_RDMAOTF_Y_STRIDE]);
	*uv_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_STRIDE],
...
void is_scaler_set_wdma_stride(void __iomem *base_addr, u32 output_id, u32 y_stride, u32 uv_stride)
{
	...
	case MCSC_OUTPUT2:
		...
		break;
	case MCSC_OUTPUT3:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA3_Y_STRIDE], y_stride);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA3_C_STRIDE], uv_stride);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA3_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_Y_STRIDE], y_stride);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMA4_C_STRIDE], uv_stride);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA4_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT_DS:
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_Y_STRIDE], y_stride);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_WDMADS_C_STRIDE], uv_stride);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMADS_STRIDE], reg_val);
		break;
	default:
		break;
	}
	...
	u32 y_2bit_stride, u32 uv_2bit_stride)
{
	...
	case MCSC_OUTPUT0:
		...
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA0_2BIT_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT1:
		reg_val = is_hw_set_field_value(reg_val,
		...
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA1_2BIT_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT2:
		reg_val = is_hw_set_field_value(reg_val,
		...
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA2_2BIT_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT3:
		reg_val = is_hw_set_field_value(reg_val,
		...
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA3_2BIT_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT4:
		reg_val = is_hw_set_field_value(reg_val,
		...
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_WDMA4_2BIT_STRIDE], reg_val);
		break;
	case MCSC_OUTPUT_DS:
		/* Not supported */
		break;
...
void is_scaler_get_wdma_stride(void __iomem *base_addr, u32 output_id, u32 *y_stride, u32 *uv_stride)
{
	...
	case MCSC_OUTPUT2:
		...
		*uv_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA2_STRIDE],
		...
		break;
	case MCSC_OUTPUT3:
		*y_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_STRIDE],
			&mcsc_fields[MCSC_F_WDMA3_Y_STRIDE]);
		*uv_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA3_STRIDE],
			&mcsc_fields[MCSC_F_WDMA3_C_STRIDE]);
		break;
	case MCSC_OUTPUT4:
		*y_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_STRIDE],
			&mcsc_fields[MCSC_F_WDMA4_Y_STRIDE]);
		*uv_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMA4_STRIDE],
			&mcsc_fields[MCSC_F_WDMA4_C_STRIDE]);
		break;
	case MCSC_OUTPUT_DS:
		*y_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_STRIDE],
			&mcsc_fields[MCSC_F_WDMADS_Y_STRIDE]);
		*uv_stride = is_hw_get_field(base_addr, &mcsc_regs[MCSC_R_WDMADS_STRIDE],
	...
	u32 y_addr, u32 cb_addr, u32 cr_addr, int buf_index)
{
	...
	switch (buf_index) {
	case 0:
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_0], y_addr);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_1], cb_addr);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_2], cr_addr);
		break;
	default:
	...
	u32 y_2bit_addr, u32 cbcr_2bit_addr, int buf_index)
{
	...
	switch (buf_index) {
	case 0:
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_2BIT_0], y_2bit_addr);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_2BIT_1], cbcr_2bit_addr);
		break;
	default:
		/* only index 0 support */
		break;
...
{
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_0], 0x0);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_1], 0x0);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_2], 0x0);

	/* DMA 2bit Y, CR address clear */
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_2BIT_0], 0x0);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_RDMAOTF_BASE_ADDR_2BIT_1], 0x0);
}
	...
	u32 output_id, u32 fmt, u32 plane, u32 dma_idx, u32 width, u32 height)
{
	...
	case MCSC_OUTPUT4:
		...
		val = is_hw_set_field_value(val, &mcsc_fields[MCSC_F_HWFC_ID1_B], dma_idx+1);
		val = is_hw_set_field_value(val, &mcsc_fields[MCSC_F_HWFC_ID2_B], dma_idx+2);
...
/* for YSUM */
void is_scaler_set_ysum_input_sourece_enable(void __iomem *base_addr, u32 output_id, bool ysum_enable)
{
	u32 reg_val = 0;

	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_CHAIN_SEL], output_id);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_ENABLE], ysum_enable);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_CTRL], reg_val);
}

void is_scaler_set_ysum_enable(void __iomem *base_addr, bool ysum_enable)
{
	u32 reg_val = 0;

	reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_CTRL]);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_ENABLE], ysum_enable);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_CTRL], reg_val);
}

void is_scaler_set_ysum_image_size(void __iomem *base_addr, u32 width, u32 height, u32 start_x, u32 start_y)
{
	u32 reg_val = 0;

	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_IMG_SIZE_H], width);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_IMG_SIZE_V], height);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_IMG_SIZE], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_CROP_POS_H], start_x);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_YSUM_CROP_POS_V], start_y);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_CROP_POS], reg_val);
}

void is_scaler_get_ysum_result(void __iomem *base_addr, u32 *luma_sum_msb, u32 *luma_sum_lsb)
{
	*luma_sum_msb = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_LUMA_SUM_MSB]);
	*luma_sum_lsb = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_YSUM_LUMA_SUM_LSB]);
}
...
void is_scaler_set_djag_input_source(void __iomem *base_addr, u32 djag_input_sel)
{
	u32 reg_val = 0;

	reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_DJAG_CTRL]);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DJAG_INPUT_SEL], djag_input_sel);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DJAG_CTRL], reg_val);
}
...
void is_scaler_set_djag_src_size(void __iomem *base_addr, u32 width, u32 height)
{
	...
	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DJAG_PS_SRC_HPOS], 0);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DJAG_PS_SRC_VPOS], 0);
...
void is_scaler_set_djag_tunning_param(void __iomem *base_addr, const struct djag_setfile_contents *djag_tune)
{
	u32 reg_val = 0;
	...
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DJAG_DITHER_SAT_THRES],
...
void is_scaler_set_cac_enable(void __iomem *base_addr, u32 en)
{
	...
	reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_CAC_CTRL]);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_ENABLE], en);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_CAC_CTRL], reg_val);
}

void is_scaler_set_cac_input_source(void __iomem *base_addr, u32 in)
{
	u32 reg_val = 0;

	reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_CAC_CTRL]);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_INPUT_SEL], in);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_CAC_CTRL], reg_val);
}
...
void is_scaler_set_cac_map_crt_thr(void __iomem *base_addr, struct cac_cfg_by_ni *cfg)
{
	...
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_MAP_SPOT_THR_L],
			cfg->map_thr_cfg.map_spot_thr_l);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_MAP_SPOT_THR_H],
			cfg->map_thr_cfg.map_spot_thr_h);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_MAP_SPOT_THR],
			cfg->map_thr_cfg.map_spot_thr);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_MAP_SPOT_NR_STRENGTH],
			cfg->map_thr_cfg.map_spot_nr_strength);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_CAC_MAP_THR], reg_val);


	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_CRT_COLOR_THR_L_DOT],
			cfg->crt_thr_cfg.crt_color_thr_l_dot);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_CRT_COLOR_THR_L_LINE],
			cfg->crt_thr_cfg.crt_color_thr_l_line);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_CAC_CRT_COLOR_THR_H],
			cfg->crt_thr_cfg.crt_color_thr_h);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_CAC_CRT_THR], reg_val);
}

/* for UVSP */
void is_scaler_set_uvsp_enable(void __iomem *base_addr, u32 hw_id, u32 en)
{
	u32 reg_val = 0;

	switch (hw_id) {
	case DEV_HW_MCSC0:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_CTRL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_ENABLE], en);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_CTRL], reg_val);
		break;
	case DEV_HW_MCSC1:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_CTRL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_ENABLE], en);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_CTRL], reg_val);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
void is_scaler_set_uvsp_radial_ctrl(void __iomem *base_addr, u32 hw_id, struct uvsp_ctrl *cfg)
{
	u32 reg_val = 0;

	switch (hw_id) {
	case DEV_HW_MCSC0:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_BINNING]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_BINNING_X],
				cfg->binning_x);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_BINNING_Y],
				cfg->binning_y);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_BINNING], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_CTRL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_EN], 1);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_CENTER_X],
				cfg->radial_center_x);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_CENTER_Y],
				cfg->radial_center_y);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_CTRL], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_BIQUAD_A]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_BIQUAD_A],
				cfg->biquad_a);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_BIQUAD_B],
				cfg->biquad_b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_BIQUAD_B], reg_val);
		break;
	case DEV_HW_MCSC1:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_BINNING]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_BINNING_X],
				cfg->binning_x);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_BINNING_Y],
				cfg->binning_y);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_BINNING], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_CTRL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_EN], 1);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_CENTER_X],
				cfg->radial_center_x);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_CENTER_Y],
				cfg->radial_center_y);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_CTRL], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_BIQUAD_A]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_BIQUAD_A],
				cfg->biquad_a);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_BIQUAD_B],
				cfg->biquad_b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_BIQUAD_B], reg_val);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
void is_scaler_set_uvsp_radial_cfg(void __iomem *base_addr, u32 hw_id, struct uvsp_radial_cfg *cfg)
{
	u32 reg_val = 0;

	switch (hw_id) {
	case DEV_HW_MCSC0:
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_BIQUAD_SHIFT],
			cfg->biquad_shift);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_RANDOM]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_RANDOM_EN],
				cfg->random_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_RANDOM_POWER],
				cfg->random_power);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_RANDOM], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_REFINE]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_REFINE_EN],
				cfg->refine_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_REFINE_LUMA_MIN],
				cfg->refine_luma_min);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_REFINE_DENOM],
				cfg->refine_denom);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_REFINE], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_ALPHA]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_ALPHA_GAIN_ADD_EN],
				cfg->alpha_gain_add_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_ALPHA_GREEN_EN],
				cfg->alpha_green_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_ALPHA_R],
				cfg->alpha_r);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_ALPHA_G],
				cfg->alpha_g);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_RADIAL_ALPHA_B],
				cfg->alpha_b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RADIAL_ALPHA], reg_val);
		break;
	case DEV_HW_MCSC1:
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_BIQUAD_SHIFT],
			cfg->biquad_shift);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_RANDOM]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_RANDOM_EN],
				cfg->random_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_RANDOM_POWER],
				cfg->random_power);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_RANDOM], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_REFINE]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_REFINE_EN],
				cfg->refine_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_REFINE_LUMA_MIN],
				cfg->refine_luma_min);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_REFINE_DENOM],
				cfg->refine_denom);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_REFINE], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_ALPHA]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_ALPHA_GAIN_ADD_EN],
				cfg->alpha_gain_add_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_ALPHA_GREEN_EN],
				cfg->alpha_green_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_ALPHA_R],
				cfg->alpha_r);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_ALPHA_G],
				cfg->alpha_g);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_RADIAL_ALPHA_B],
				cfg->alpha_b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RADIAL_ALPHA], reg_val);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
void is_scaler_set_uvsp_pedestal_cfg(void __iomem *base_addr, u32 hw_id, struct uvsp_pedestal_cfg *cfg)
{
	u32 reg_val = 0;

	switch (hw_id) {
	case DEV_HW_MCSC0:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_PEDESTAL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_PEDESTAL_R], cfg->r);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_PEDESTAL_G], cfg->g);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_PEDESTAL_B], cfg->b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_PEDESTAL], reg_val);
		break;
	case DEV_HW_MCSC1:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_PEDESTAL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_PEDESTAL_R], cfg->r);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_PEDESTAL_G], cfg->g);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_PEDESTAL_B], cfg->b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_PEDESTAL], reg_val);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
void is_scaler_set_uvsp_offset_cfg(void __iomem *base_addr, u32 hw_id, struct uvsp_offset_cfg *cfg)
{
	u32 reg_val = 0;

	switch (hw_id) {
	case DEV_HW_MCSC0:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_OFFSET]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_OFFSET_R], cfg->r);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_OFFSET_G], cfg->g);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_OFFSET_B], cfg->b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_OFFSET], reg_val);
		break;
	case DEV_HW_MCSC1:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_OFFSET]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_OFFSET_R], cfg->r);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_OFFSET_G], cfg->g);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_OFFSET_B], cfg->b);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_OFFSET], reg_val);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
void is_scaler_set_uvsp_desat_cfg(void __iomem *base_addr, u32 hw_id, struct uvsp_desat_cfg *cfg)
{
	u32 reg_val = 0;

	switch (hw_id) {
	case DEV_HW_MCSC0:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_CTRL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_CTRL_EN],
				cfg->ctrl_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_CTRL_SINGLESIDE],
				cfg->ctrl_singleSide);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_CTRL_LUMA_OFFSET],
				cfg->ctrl_luma_offset);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_CTRL_GAIN_OFFSET],
				cfg->ctrl_gain_offset);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_CTRL], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_Y]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_Y_SHIFT],
				cfg->y_shift);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_Y_LUMA_MAX],
				cfg->y_luma_max);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_Y], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_U]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_U_LOW],
				cfg->u_low);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_U_HIGH],
				cfg->u_high);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_U], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_V]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_V_LOW],
				cfg->v_low);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP0_DESAT_V_HIGH],
				cfg->v_high);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_DESAT_V], reg_val);
		break;
	case DEV_HW_MCSC1:
		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_CTRL]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_CTRL_EN],
				cfg->ctrl_en);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_CTRL_SINGLESIDE],
				cfg->ctrl_singleSide);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_CTRL_LUMA_OFFSET],
				cfg->ctrl_luma_offset);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_CTRL_GAIN_OFFSET],
				cfg->ctrl_gain_offset);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_CTRL], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_Y]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_Y_SHIFT],
				cfg->y_shift);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_Y_LUMA_MAX],
				cfg->y_luma_max);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_Y], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_U]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_U_LOW],
				cfg->u_low);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_U_HIGH],
				cfg->u_high);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_U], reg_val);

		reg_val = is_hw_get_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_V]);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_V_LOW],
				cfg->v_low);
		reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_UVSP1_DESAT_V_HIGH],
				cfg->v_high);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_DESAT_V], reg_val);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
void is_scaler_set_uvsp_r2y_coef_cfg(void __iomem *base_addr, u32 hw_id, struct uvsp_r2y_coef_cfg *cfg)
{
	switch (hw_id) {
	case DEV_HW_MCSC0:
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_00], cfg->r2y_coef_00);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_01], cfg->r2y_coef_01);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_02], cfg->r2y_coef_02);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_10], cfg->r2y_coef_10);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_11], cfg->r2y_coef_11);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_12], cfg->r2y_coef_12);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_20], cfg->r2y_coef_20);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_21], cfg->r2y_coef_21);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_22], cfg->r2y_coef_22);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP0_RGB2YUV_COEF_SHIFT], cfg->r2y_coef_shift);
		break;
	case DEV_HW_MCSC1:
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_00], cfg->r2y_coef_00);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_01], cfg->r2y_coef_01);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_02], cfg->r2y_coef_02);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_10], cfg->r2y_coef_10);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_11], cfg->r2y_coef_11);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_12], cfg->r2y_coef_12);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_20], cfg->r2y_coef_20);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_21], cfg->r2y_coef_21);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_22], cfg->r2y_coef_22);
		is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_UVSP1_RGB2YUV_COEF_SHIFT], cfg->r2y_coef_shift);
		break;
	default:
		warn_hw("invalid hw_id(%d) for MCSC api\n", hw_id);
		break;
...
/* DS */
void is_scaler_set_ds_enable(void __iomem *base_addr, u32 ds_enable)
{
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_CTRL], &mcsc_fields[MCSC_F_DS_ENABLE], ds_enable);
}

void is_scaler_set_ds_img_size(void __iomem *base_addr, u32 width, u32 height)
{
	u32 reg_val = 0;

	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_IMG_HSIZE], width);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_IMG_VSIZE], height);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_IMG_SIZE], reg_val);
}

void is_scaler_set_ds_src_size(void __iomem *base_addr, u32 width, u32 height, u32 x_pos, u32 y_pos)
{
	u32 reg_val = 0;

	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_CROP_POS_H], x_pos);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_CROP_POS_V], y_pos);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_CROP_POS], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_SRC_HSIZE], width);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_SRC_VSIZE], height);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_SRC_SIZE], reg_val);
}

void is_scaler_set_ds_dst_size(void __iomem *base_addr, u32 width, u32 height)
{
	u32 reg_val = 0;

	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_DST_HSIZE], width);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_DST_VSIZE], height);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_DST_SIZE], reg_val);
}

void is_scaler_set_ds_scaling_ratio(void __iomem *base_addr, u32 hratio, u32 vratio)
{
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_H_RATIO], &mcsc_fields[MCSC_F_DS_H_RATIO], hratio);
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_V_RATIO], &mcsc_fields[MCSC_F_DS_V_RATIO], vratio);
}

void is_scaler_set_ds_init_phase_offset(void __iomem *base_addr, u32 h_offset, u32 v_offset)
{
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_H_INIT_PHASE_OFFSET],
		&mcsc_fields[MCSC_F_DS_H_INIT_PHASE_OFFSET], h_offset);
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_V_INIT_PHASE_OFFSET],
		&mcsc_fields[MCSC_F_DS_V_INIT_PHASE_OFFSET], v_offset);
}

void is_scaler_set_ds_gamma_table_enable(void __iomem *base_addr, u32 ds_gamma_enable)
{
	u32 reg_val = 0;

	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_EN],
		&mcsc_fields[MCSC_F_DS_GAMMA_EN], ds_gamma_enable);

/* Gamma table : LUT register with default values : it's a value for 10bit */
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_0], 0);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_1], 16);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_01], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_2], 32);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_3], 48);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_23], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_4], 60);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_5], 112);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_45], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_6], 152);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_7], 192);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_67], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_8], 260);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_9], 316);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_89], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_A], 420);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_B], 512);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_AB], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_C], 608);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_D], 708);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_CD], reg_val);

	reg_val = 0;
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_E], 832);
	reg_val = is_hw_set_field_value(reg_val, &mcsc_fields[MCSC_F_DS_GAMMA_TBL_F], 1024);
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_DS_GAMMA_TBL_EF], reg_val);
}

/*
 *void is_scaler_set_ds_dma_enable(void __iomem *base_addr, u32 dma_enable)
 *{
 *	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_DS_DMA_OUT_CTRL],
