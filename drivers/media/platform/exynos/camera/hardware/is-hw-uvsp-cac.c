	...
	struct cac_setfile_contents *cac, u32 cur_ni)
{
	...
	is_scaler_set_cac_enable(hw_ip->regs[REG_SETA], 1);
}
	...
	struct is_frame *frame, u32 instance)
{
	...
	is_hw_mcsc_get_force_block_control(hw_ip, SUBBLK_IP_CAC, cap->max_cac,
	...
	is_scaler_set_cac_input_source(hw_ip->regs[REG_SETA], (hw_mcsc->cac_in - DEV_HW_MCSC0));
...
struct ref_ni hw_mcsc_find_ni_idx_for_uvsp(struct is_hw_ip *hw_ip,
...
void hw_mcsc_calc_uvsp_param_by_ni(struct is_hw_ip *hw_ip,
	struct uvsp_setfile_contents *uvsp, u32 cur_ni)
{
	struct ref_ni ni_range, ni_idx, ni;
	...
	is_scaler_set_uvsp_radial_cfg(hw_ip->regs[REG_SETA], hw_ip->id, &uvsp_cfg.radial_cfg);
	is_scaler_set_uvsp_pedestal_cfg(hw_ip->regs[REG_SETA], hw_ip->id, &uvsp_cfg.pedestal_cfg);
	is_scaler_set_uvsp_offset_cfg(hw_ip->regs[REG_SETA], hw_ip->id, &uvsp_cfg.offset_cfg);
	is_scaler_set_uvsp_r2y_coef_cfg(hw_ip->regs[REG_SETA], hw_ip->id, &uvsp_cfg.r2y_coef_cfg);
	is_scaler_set_uvsp_enable(hw_ip->regs[REG_SETA], hw_ip->id, 1);
}

void hw_mcsc_calc_uvsp_radial_ctrl(struct is_hw_ip *hw_ip,
	struct is_hw_mcsc *hw_mcsc, struct is_frame *frame,
	struct cal_info *cal_info, u32 instance)
{
	...
	if (!frame->shot_ext) {
		...
		is_scaler_set_uvsp_enable(hw_ip->regs[REG_SETA], hw_ip->id, 0);
		return;
	}
	...
	is_scaler_set_uvsp_radial_ctrl(hw_ip->regs[REG_SETA], hw_ip->id, uvsp_ctrl);
...
int is_hw_mcsc_update_uvsp_register(struct is_hw_ip *hw_ip,
	struct is_frame *frame, u32 instance)
{
	int ret = 0;
	struct is_hw_mcsc *hw_mcsc;
	...
	/* The force means that sysfs control has higher priority than scenario. */
	is_hw_mcsc_get_force_block_control(hw_ip, SUBBLK_IP_UVSP, cap->max_uvsp,
		NULL, &uvsp_en);

	if (uvsp_en == false) {
		is_scaler_set_uvsp_enable(hw_ip->regs[REG_SETA], hw_ip->id, uvsp_en);
