		...
		u32 instance)
{
	...
	is_hw_djag_get_input(ischain, &hw_mcsc->djag_in);

	/* The force means that sysfs control has higher priority than scenario. */
	is_hw_mcsc_get_force_block_control(hw_ip, SUBBLK_IP_DJAG, cap->max_djag,
	...
	is_scaler_set_djag_input_source(hw_ip->regs[REG_SETA],
	...
	is_scaler_set_djag_enable(hw_ip->regs[REG_SETA], 1);
