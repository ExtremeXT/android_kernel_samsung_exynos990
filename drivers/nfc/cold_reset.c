...
static void ese_cold_reset_gaurd_timer_callback(struct timer_list *unused)
{
	...
	pn547_dev->state_flags &= ~MASK_ESE_COLD_RESET_GUARD_TIMER;
}
...
static long start_ese_cold_reset_guard_timer(void)
{
	...
	pr_info("starting ese_cold_reset_timer\n");
	...
	setup_timer(&ese_cold_reset_timer,
		ese_cold_reset_gaurd_timer_callback, 0);
#else
	timer_setup(&ese_cold_reset_timer, ese_cold_reset_gaurd_timer_callback, 0);
#endif
	ret = mod_timer(&ese_cold_reset_timer,
...
void rcv_prop_resp_status(const char * const buf)
{
	...
	ret = i2c_master_recv(pn547_dev->client, tmp, rcount);
	if (ret == rcount) {
		prop_nci_rsp[3] = tmp[0];
		pr_info("%s NxpNciR : len = 4 > %02X%02X%02X%02X\n", __func__,
			prop_nci_rsp[0], prop_nci_rsp[1],
			prop_nci_rsp[2], prop_nci_rsp[3]);
	} else {
	...
	if (pn547_dev->state_flags & (PN547_STATE_NFC_ON))
...
static int send_nci_transceive(uint8_t *prop_cmd, size_t prop_cmd_size)
{
	...
	do {
		ret = i2c_master_send(pn547_dev->client, prop_cmd, prop_cmd_size);
	...
	ret = 0x00;
	do {
		if (pn547_dev->state_flags & PN547_STATE_NFC_ON)/* NFC_ON */ {
			/* Read is pending from the NFC service which will
			 * complete the prop_cmd_resp_sema
			 */
 ...
 *    -9           :    INVALID_PARAM  < NFCC rsp to cold reset cmd >
...
int do_reset_protection(bool type)
{
	...
	if (type) {
		...
		if (!(pn547_dev->state_flags & RST_PROTECTION_ENABLED))
			return ret;
	}

	pr_info("%s: NxpNciX: %d > %02X%02X%02X%02X\n", __func__,
		ret, prop_cmd[0], prop_cmd[1], prop_cmd[2], prop_cmd[3]);
...
int ese_cold_reset(enum ese_cold_reset_origin src)
{
	...
	if (status) {
		...
		pn547_dev->state_flags |= src << ESE_COLD_RESET_ORIGIN_FLAGS_POS;
		...
		if (ret) {
			pn547_dev->state_flags &= ~(MASK_ESE_COLD_RESET | MASK_ESE_COLD_RESET_GUARD_TIMER);
			mutex_unlock(&ese_cold_reset_sync_mutex);
			return ret;
		}
		...
		if (IS_RESET_PROTECTION_ENABLED(pn547_dev->state_flags)) {
			...
			ret = -EBUSY;
		} else if (IS_COLD_RESET_REQ_IN_PROGRESS(pn547_dev->state_flags)) {
