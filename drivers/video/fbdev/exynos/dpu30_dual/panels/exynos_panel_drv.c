...
struct exynos_panel_device *exynos_panel_drvdata;
EXPORT_SYMBOL(exynos_panel_drvdata);

static struct exynos_panel_ops *panel_list[MAX_PANEL_SUPPORT];

static struct of_device_id exynos_panel_drv_of_match[] = {
	{ .compatible = "samsung,exynos-panel-drv" },
 	{ }
 };

MODULE_DEVICE_TABLE(of, exynos_panel_drv_of_match);
...
static int exynos_panel_parse_regulators(struct exynos_panel_device *panel)
{
	...
	if(!of_property_read_string(dev->of_node, "regulator_1p6v",
				(const char **)&str_regulator[2])) {
		res->regulator[2] = regulator_get(dev, str_regulator[2]);
		if (IS_ERR(res->regulator[2])) {
			DPU_ERR_PANEL("panel regulator 1.6V get failed\n");
			res->regulator[2] = NULL;
		}
	}
	DPU_INFO_PANEL("get regulator 1.6V");
	return 0;
}
...
static int exynos_panel_set_power(struct exynos_panel_device *panel, bool on)
{
	...
	if (on) {
		...
		if (res->regulator[2] > 0) {
			ret = regulator_enable(res->regulator[2]);
			if (ret) {
				DPU_ERR_PANEL("panel regulator 1.6V enable failed\n");
				return ret;
			}
		}
	} else {
...
{
	panel_list[0] = &panel_s6e3fa7_ops;
}
...
static int exynos_panel_parse_dt(struct exynos_panel_device *panel)
{
	...
	if ((panel->id != 0) && (panel->id != 1))
		of_property_read_u32(panel->dev->of_node, "id", &panel->id);
...
static int exynos_panel_drv_probe(struct platform_device *pdev)
{
	struct exynos_panel_device *panel;
	int ret = 0;
	char name[16];
...
static void exynos_panel_drv_shutdown(struct platform_device *pdev)
{
...
static struct platform_driver exynos_panel_drv = {
	.probe		= exynos_panel_drv_probe,
	.shutdown	= exynos_panel_drv_shutdown,
	.driver		= {
		.name		= "exynos-panel-drv",
		.of_match_table	= of_match_ptr(exynos_panel_drv_of_match),
		.suppress_bind_attrs = true,
	},
};

static int __init exynos_panel_drv_init(void)
{
	int ret = platform_driver_register(&exynos_panel_drv);
	if (ret)
		pr_err("exynos_panel_drv register failed\n");

	return ret;
}
device_initcall_sync(exynos_panel_drv_init);

static void __exit exynos_panel_drv_exit(void)
{
	platform_driver_unregister(&exynos_panel_drv);
}
module_exit(exynos_panel_drv_exit);
