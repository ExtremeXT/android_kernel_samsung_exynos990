...
static int getidx_s6e3fc3_vrr_fps(int vrr_fps)
{
...
static int getidx_s6e3fc3_current_vrr_fps(struct panel_device *panel)
{
	int vrr_fps;
...
static int init_color_blind_table(struct maptbl *tbl)
{
	...
	if (S6E3FC3_SCR_CR_OFS + mdnie->props.sz_scr > sizeof_maptbl(tbl)) {
...
static int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3FC3_NIGHT_MODE_OFS +
...
static int init_mdnie_color_lens_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3FC3_COLOR_LENS_OFS +
...
static int getidx_dyn_ffc_table(struct maptbl *tbl)
{
	...
	row = status->ffc_df;
	if (row >= S6E3FC3_MAX_MIPI_FREQ) {
		panel_warn("ffc out of range %d %d, set to %d\n", row,
			S6E3FC3_MAX_MIPI_FREQ, S6E3FC3_DEFAULT_MIPI_FREQ);
		row = status->ffc_df = S6E3FC3_DEFAULT_MIPI_FREQ;
	}
