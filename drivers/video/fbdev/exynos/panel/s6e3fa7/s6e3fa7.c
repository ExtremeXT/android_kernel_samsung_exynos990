...
static void copy_tpout_center(u8 *output, u32 value, u32 v, u32 color)
{
	...
	if (v == S6E3FA7_NR_TP - 1) {
		...
		if (value > 511)
			panel_err("exceed output range tp:%d c:%d value:%d!!\n",
					index, color, value);
	} else if (v == 0) {
		if (color == RED) {
			output[0] &= ~(0xF << 4);
...
static void copy_irc_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	memcpy(irc_info->ref_tbl,
			&tbl->arr[(brt_tbl->sz_ui_lum - 1) * irc_info->total_len],
			irc_info->total_len);
...
static int getidx_resolution_table(struct maptbl *tbl)
{
	...
	if (props->mres_mode >= mres->nr_resol) {
		...
		panel_info("mres_mode %d (%dx%d)\n",
				props->mres_mode, mres->resol[row].w, mres->resol[row].h);
	}
