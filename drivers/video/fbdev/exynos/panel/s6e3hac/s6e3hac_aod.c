/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3hac/s6e3hac_aod.c
 *
...
void s6e3hac_copy_self_mask_ctrl(struct maptbl *tbl, u8 *dst)
{
	panel_info("was called\n");
	panel_info("%x %x %x\n", dst[0], dst[1], dst[2]);
}

int s6e3hac_init_self_mask_ctrl(struct maptbl *tbl)
{
...
void s6e3hac_copy_digital_pos(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_time(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_timer_rate(struct maptbl *tbl, u8 *dst)
{
	u8 value = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_icon_grid_on_ctrl(struct maptbl *tbl, u8 *dst)
{
	char enable = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_self_move_on_ctrl(struct maptbl *tbl, u8 *dst)
{
	char enable = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_analog_pos(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_analog_en(struct maptbl *tbl, u8 *dst)
{
	char en_reg = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_digital_en(struct maptbl *tbl, u8 *dst)
{
	char en_reg = 0;
	char disp_format = 0;
	struct aod_dev_info *aod = tbl->pdata;
...
int s6e3hac_getidx_self_mode_pos(struct maptbl *tbl)
{
	int row = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_self_move_reset(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_icon_ctrl(struct maptbl *tbl, u8 *dst)
{
	u8 enable = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_digital_color(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_digital_un_width(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
#define ENABLE_PARTIAL_HLPM_VAL 0x03
#define ENABLE_PARTIAL_SCAN_VAL 0x30

void s6e3hac_copy_partial_mode(struct maptbl *tbl, u8 *dst)
{
	u8 enable = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_partial_area(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
	...
	if (props->partial.scan_en) {
		dst[PARTIAL_AREA_SL0_REG] = (char)((props->partial.scan_sl >> 8) & 0xff);
		dst[PARTIAL_AREA_SL1_REG] = (char)(props->partial.scan_sl & 0xff);
		dst[PARTIAL_AREA_EL0_REG] = (char)((props->partial.scan_el >> 8) & 0xff);
...
void s6e3hac_copy_partial_hlpm(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
	...
	if (props->partial.hlpm_scan_en) {
		dst[PARTIAL_HLPM1_L0_REG] = (char)((props->partial.hlpm_area_1 >> 8) & 0xff);
		dst[PARTIAL_HLPM1_L1_REG] = (char)(props->partial.hlpm_area_1 & 0xff);

		dst[PARTIAL_HLPM2_L0_REG] = (char)((props->partial.hlpm_area_2 >> 8) & 0xff);
		dst[PARTIAL_HLPM2_L1_REG] = (char)(props->partial.hlpm_area_2 & 0xff);

		dst[PARTIAL_HLPM3_L0_REG] = (char)((props->partial.hlpm_area_3 >> 8) & 0xff);
		dst[PARTIAL_HLPM3_L1_REG] = (char)(props->partial.hlpm_area_3 & 0xff);

		dst[PARTIAL_HLPM4_L0_REG] = (char)((props->partial.hlpm_area_4 >> 8) & 0xff);
...
int s6e3hac_getidx_self_pattern(struct maptbl *tbl)
{
	int row = 0;
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
#if 0
void s6e3hac_copy_self_move_enable(struct maptbl *tbl, u8 *dst)
{
	struct aod_dev_info *aod = tbl->pdata;
	struct aod_ioctl_props *props = &aod->props;
...
void s6e3hac_copy_self_move_pattern(struct maptbl *tbl, u8 *dst)
{
	int idx;
