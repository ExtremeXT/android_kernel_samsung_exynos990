/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3xa0/s6e3xa0.c
 *
 * S6E3XA0 Dimming Driver
 *
...
{
	int v, c, sign;
	int signed_bit[S6E3XA0_NR_TP] = {
		-1, -1, 7, 7, 7, 7, 7, 7, 7, 7, 7, 9
	};
	int value_mask[S6E3XA0_NR_TP] = {
...
{
	int index = (S6E3XA0_NR_TP - v - 1);

	if (v == S6E3XA0_NR_TP - 1) {
		...
		u8 *dst, enum dim_flash_items item, int vrr_idx, int row, int col, int size)
{
	...
	char *name = s6e3xa0_dim_flash_info[vrr_idx][item].name;
	int nrow = s6e3xa0_dim_flash_info[vrr_idx][item].nrow;
	int ncol = s6e3xa0_dim_flash_info[vrr_idx][item].ncol;
...
static int generate_hbm_gamma_table(struct panel_info *panel_data, int id)
{
	...
	char *hbm_gamma_resource_names[MAX_S6E3XA0_VRR_FPS] = {
		"hbm_gamma",
		"hbm_gamma_120hz",
	};
...
static int generate_gamma_table_using_lut(struct panel_info *panel_data, int id)
{
	...
	struct panel_device *panel = to_panel_device(panel_data);
	struct panel_bl_device *panel_bl = &panel->panel_bl;
	struct dimming_init_info *dim_init_info[2];
	struct brightness_table *brt_tbl;
	struct resinfo *res;
	...
	brt_tbl = &panel_bl->subdev[id].brt_tbl;
	panel_dim_info = panel_data->panel_dim_info[id];
	dim_init_info[0] = &panel_dim_info->dim_init_info;
	dim_init_info[1] = &panel_dim_info->dim_120hz_init_info;
...
static void copy_gamma_inter_control_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	for(i  = 0; i < S6E3XA0_GAMMA_INTER_LEN; i++)
		...
		struct maptbl *gamma_maptbl, u8 *dst)
{
	...
	if (unlikely(!hbm_gamma_res)) {
		panel_warn("panel_bl-%d hbm_gamma not found\n", id);
		return -EIO;
	}
...
static int init_maptbl_from_flash(struct maptbl *tbl, enum dim_flash_items item)
{
	...
	struct dim_flash_info *dim_flash_info;
...
static int getidx_resolution_table(struct maptbl *tbl)
{
	...
	index = search_table_u32(S6E3XA0_SCALER_1440,
			ARRAY_SIZE(S6E3XA0_SCALER_1440), xres);
	if (index < 0)
		row = 0;
...
static int getidx_fps_table(struct maptbl *tbl)
{
	...
	vrr = panel_data->vrrtbl[panel_data->props.vrr_idx];
	index = search_vrr_fps_table(vrr->fps);
...
static int s6e3xa0_getidx_vddm_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_properties *props = &panel->panel_data.props;
...
static int s6e3xa0_getidx_gram_img_pattern_table(struct maptbl *tbl)
{
	...
	props->gct_valid_chksum[0] = S6E3XA0_GRAM_CHECKSUM_VALID;
	props->gct_valid_chksum[1] = S6E3XA0_GRAM_CHECKSUM_VALID;
	props->gct_valid_chksum[2] = S6E3XA0_GRAM_CHECKSUM_VALID;
	props->gct_valid_chksum[3] = S6E3XA0_GRAM_CHECKSUM_VALID;
...
static int s6e3xa0_getidx_tdmb_tune_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_properties *props = &panel->panel_data.props;
...
static void copy_copr_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	reg = &copr->props.reg.v3;

	dst[0] = (reg->copr_mask << 4) | (reg->cnt_re << 3) |
		(reg->copr_ilc << 2) | (reg->copr_gamma << 1) | reg->copr_en;
...
static int init_color_blind_table(struct maptbl *tbl)
{
	...
	if (S6E3XA0_SCR_CR_OFS + mdnie->props.sz_scr > sizeof_maptbl(tbl)) {
		panel_err("invalid size (maptbl_size %d, sz_scr %d)\n",
				sizeof_maptbl(tbl), mdnie->props.sz_scr);
		return -EINVAL;
	}

	memcpy(&tbl->arr[S6E3XA0_SCR_CR_OFS],
...
static int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3XA0_NIGHT_MODE_OFS +
	...
	maptbl_copy(night_maptbl, &tbl->arr[S6E3XA0_NIGHT_MODE_OFS]);
...
static int init_mdnie_color_lens_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3XA0_COLOR_LENS_OFS +
	...
	if (IS_COLOR_LENS_MODE(mdnie))
		maptbl_copy(color_lens_maptbl, &tbl->arr[S6E3XA0_COLOR_LENS_OFS]);
...
static void show_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 err[S6E3XA0_ERR_LEN] = { 0, }, err_15_8, err_7_0;
...
{
	int ret;
	u8 err_fg[S6E3XA0_ERR_FG_LEN] = { 0, };
	struct resinfo *res = info->res;
...
static void show_dsi_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 dsi_err[S6E3XA0_DSI_ERR_LEN] = { 0, };
...
static void show_self_diag(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 self_diag[S6E3XA0_SELF_DIAG_LEN] = { 0, };
