/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3ha9/s6e3ha9.c
 *
 * S6E3HA9 Dimming Driver
 *
...
{
	int v, c, sign;
	int signed_bit[S6E3HA9_NR_TP] = {
		-1, -1, 7, 7, 7, 7, 7, 7, 7, 7, 7, 9
	};
	int value_mask[S6E3HA9_NR_TP] = {
...
{
	int index = (S6E3HA9_NR_TP - v - 1);

	if (v == S6E3HA9_NR_TP - 1) {
		...
		u8 *dst, enum dim_flash_items item, int row, int col, int size)
{
	...
	char *name = s6e3ha9_dim_flash_info[item].name;
	int nrow = s6e3ha9_dim_flash_info[item].nrow;
	int ncol = s6e3ha9_dim_flash_info[item].ncol;

	if (!res[item])
		res[item] = find_panel_resource(panel_data, name);

	if (unlikely(!res[item])) {
		panel_warn("resource(%s) not found\n", name);
		return -EIO;
	}
...
static void print_gamma_table(struct panel_info *panel_data, int id)
{
	...
	for (i = 0; i < brt_tbl->sz_lum; i++) {
		...
		for (j = 0; j < S6E3HA9_GAMMA_CMD_CNT - 1; j++)
...
#ifdef CONFIG_SUPPORT_HMD
			(id == PANEL_BL_SUBDEV_TYPE_HMD) ?
			s6e3ha9_dim_flash_info[DIM_FLASH_HMD_GAMMA].name :
#endif
			s6e3ha9_dim_flash_info[DIM_FLASH_GAMMA].name)) {
		panel_warn("resource(%s) not initialized\n",
#ifdef CONFIG_SUPPORT_HMD
				(id == PANEL_BL_SUBDEV_TYPE_HMD) ?
				s6e3ha9_dim_flash_info[DIM_FLASH_HMD_GAMMA].name :
#endif
...
static void copy_gamma_inter_control_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	for(i  = 0; i < S6E3HA9_GAMMA_INTER_LEN; i++)
...
static int init_maptbl_from_flash(struct maptbl *tbl, enum dim_flash_items item)
{
	...
	panel_data = &panel->panel_data;
	if (!panel_resource_initialized(panel_data,
				s6e3ha9_dim_flash_info[item].name)) {
		panel_warn("resource(%s) not initialized\n",
				s6e3ha9_dim_flash_info[item].name);
	...
	for_each_layer(tbl, layer) {
		for_each_row(tbl, row) {
			if (row >= s6e3ha9_dim_flash_info[item].nrow)
				continue;
...
static int s6e3ha9_getidx_vddm_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_properties *props = &panel->panel_data.props;
...
static int s6e3ha9_getidx_tdmb_tune_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
	struct panel_properties *props = &panel->panel_data.props;
...
static void copy_self_clk_update_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	if (act_info->en) {
		...
		panel_info("active clock enable\n");
...
static void copy_self_clk_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	if (act_info->en) {
		...
		panel_info("active clock pos %d,%d\n",
...
static int init_color_blind_table(struct maptbl *tbl)
{
	...
	if (S6E3HA9_SCR_CR_OFS + mdnie->props.sz_scr > sizeof_maptbl(tbl)) {
		panel_err("invalid size (maptbl_size %d, sz_scr %d)\n",
				sizeof_maptbl(tbl), mdnie->props.sz_scr);
		return -EINVAL;
	}

	memcpy(&tbl->arr[S6E3HA9_SCR_CR_OFS],
...
static int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3HA9_NIGHT_MODE_OFS +
	...
	maptbl_copy(night_maptbl, &tbl->arr[S6E3HA9_NIGHT_MODE_OFS]);
...
static int init_mdnie_color_lens_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (S6E3HA9_COLOR_LENS_OFS +
	...
	if (IS_COLOR_LENS_MODE(mdnie))
		maptbl_copy(color_lens_maptbl, &tbl->arr[S6E3HA9_COLOR_LENS_OFS]);
...
static void show_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 err[S6E3HA9_ERR_LEN] = { 0, }, err_15_8, err_7_0;
...
{
	int ret;
	u8 err_fg[S6E3HA9_ERR_FG_LEN] = { 0, };
	struct resinfo *res = info->res;
...
static void show_dsi_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 dsi_err[S6E3HA9_DSI_ERR_LEN] = { 0, };
...
static void show_self_diag(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 self_diag[S6E3HA9_SELF_DIAG_LEN] = { 0, };
