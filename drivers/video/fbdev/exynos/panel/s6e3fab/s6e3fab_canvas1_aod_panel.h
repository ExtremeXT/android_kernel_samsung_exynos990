/*
 * linux/drivers/video/fbdev/exynos/panel/s6e3fab/s6e3fab_canvas1_aod_panel.h
 *
...
#ifndef __S6E3FAB_CANVAS1_AOD_PANEL_H__
#define __S6E3FAB_CANVAS1_AOD_PANEL_H__

#include "s6e3fab_aod.h"
#include "s6e3fab_aod_panel.h"

#include "s6e3fab_canvas1_self_mask_img.h"
#include "s6e3fab_self_icon_img.h"
#include "s6e3fab_self_analog_clock_img.h"
#include "s6e3fab_self_digital_clock_img.h"

/* CANVAS1 */
static DEFINE_STATIC_PACKET_WITH_OPTION(s6e3fab_canvas1_aod_self_mask_img_pkt,
		DSI_PKT_TYPE_WR_SR, CANVAS1_SELF_MASK_IMG, 0, PKT_OPTION_SR_ALIGN_16);

static void *s6e3fab_canvas1_aod_self_mask_img_cmdtbl[] = {
	&KEYINFO(s6e3fab_aod_l1_key_enable),
	&PKTINFO(s6e3fab_aod_self_mask_sd_path),
	&DLYINFO(s6e3fab_aod_self_spsram_sel_delay),
	&PKTINFO(s6e3fab_canvas1_aod_self_mask_img_pkt),
	&PKTINFO(s6e3fab_aod_reset_sd_path),
	&KEYINFO(s6e3fab_aod_l1_key_disable),
};

static struct seqinfo s6e3fab_canvas1_aod_seqtbl[MAX_AOD_SEQ] = {
	[SELF_MASK_IMG_SEQ] = SEQINFO_INIT("self_mask_img", s6e3fab_canvas1_aod_self_mask_img_cmdtbl),
	...
	[SELF_ICON_IMG_SEQ] = SEQINFO_INIT("SELF_ICON_IMG", s6e3fab_aod_icon_img_cmdtbl),
	...
	[CTRL_ICON_SEQ] = SEQINFO_INIT("CTRL_ICON", s6e3fab_aod_ctrl_icon_cmdtbl),
	[DISABLE_ICON_SEQ] = SEQINFO_INIT("DISABLE_ICON", s6e3fab_aod_disable_cmdtbl),
...
static struct aod_tune s6e3fab_canvas1_aod = {
	.name = "s6e3fab_canvas1_aod",
	.nr_seqtbl = ARRAY_SIZE(s6e3fab_canvas1_aod_seqtbl),
	.seqtbl = s6e3fab_canvas1_aod_seqtbl,
	.nr_maptbl = ARRAY_SIZE(s6e3fab_aod_maptbl),
	.maptbl = s6e3fab_aod_maptbl,
	.self_mask_en = true,
};
#endif //__S6E3FAB_CANVAS1_AOD_PANEL_H__
