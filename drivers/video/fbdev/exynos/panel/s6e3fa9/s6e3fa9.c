...
static int getidx_dyn_ffc_table(struct maptbl *tbl)
{
	...
	if (layer >= MAX_S6E3FA9_OSC) {
		panel_warn("osc out of range %d %d, set to %d\n", layer, MAX_S6E3FA9_OSC, S6E3FA9_OSC_DEFAULT);
		layer = status->current_ddi_osc = S6E3FA9_OSC_DEFAULT;
		status->request_ddi_osc = S6E3FA9_OSC_DEFAULT;
	}
