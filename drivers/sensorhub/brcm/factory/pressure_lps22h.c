	...
	struct device_attribute *attr, char *buf)
{
	...
	temperature = (s32) (data->buf[PRESSURE_SENSOR].temperature);
	float_temperature = ((temperature%100) > 0 ? (temperature%100) : -(temperature%100));
	return sprintf(buf, "%d.%02d\n", (temperature/100), float_temperature);
}

static DEVICE_ATTR(vendor,  0440, pressure_vendor_show, NULL);
static DEVICE_ATTR(name,  0440, pressure_name_show, NULL);
