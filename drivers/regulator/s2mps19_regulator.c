...
#ifdef CONFIG_SEC_PM
	if (of_property_read_bool(iodev->dev->of_node, "ext_en,ldo2,use_tf_sel")) {
		dev_info(&pdev->dev, "%s: LDO2M is controlled by TF_SEL\n",
				__func__);
		ret = s2mps19_update_reg(s2mps19->i2c, S2MPS19_PMIC_REG_EXT_EN,
				1, 1);
		if (ret)
			dev_err(&pdev->dev, "%s: failed to write EXT_EN\n",
					__func__);
	}
#endif /* CONFIG_SEC_PM */
