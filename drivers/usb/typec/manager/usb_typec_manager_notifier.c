			...
			manager_notifier_device_t listener)
{
	...
	if (listener == MANAGER_NOTIFY_CCIC_MUIC || listener == MANAGER_NOTIFY_CCIC_SENSORHUB) {
...
int manager_notifier_unregister(struct notifier_block *nb)
{
	...
	if (nb->priority == MANAGER_NOTIFY_CCIC_MUIC || nb->priority == MANAGER_NOTIFY_CCIC_SENSORHUB) {
