...
static ssize_t reverse_phy_port_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct exynos_usbdrd_phy *phy_drd = dev_get_drvdata(dev);
	int ret = 0;
...
static ssize_t reverse_phy_port_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t n)
{
	struct exynos_usbdrd_phy *phy_drd = dev_get_drvdata(dev);
	int ret = 0, value = 0;
	...
	phy_drd->reverse_phy_port = !!value;

	return n;
}

static DEVICE_ATTR_RW(reverse_phy_port);
