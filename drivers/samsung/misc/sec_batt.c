...
static int __init sec_bat_get_wireless_ic(char *str)
{
	...
	if (get_option(&str, &ic_info)) {
		...
		printk(KERN_ERR "wireless_ic() : ic_info(0x%08X), chip_id(0x%02X), fw_ver(0x%04X), fw_mode(0x%02X)\n",
			ic_info, wireless_chip_id_param, wireless_fw_ver_param, wireless_fw_mode_param);
