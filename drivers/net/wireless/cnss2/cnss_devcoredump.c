...
void cnss_dev_coredumpv(struct device *dev, void *data, size_t datalen,
		   gfp_t gfp)
{
	cnss_dev_coredumpm(dev, NULL, data, datalen, gfp, devcd_readv, devcd_freev);
}
EXPORT_SYMBOL_GPL(cnss_dev_coredumpv);
...
EXPORT_SYMBOL_GPL(cnss_dev_coredumpm);
...
{
	cnss_dev_coredumpm(dev, NULL, table, datalen, gfp, devcd_read_from_sgtable,
		      devcd_free_sgtable);
}
EXPORT_SYMBOL_GPL(cnss_dev_coredumpsg);
